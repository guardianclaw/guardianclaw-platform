/**
 * Basic VoltAgent Example with GuardianClaw Safety Guardrails
 *
 * This example demonstrates how to integrate GuardianClaw's CLAW protocol,
 * OWASP protection, and PII detection into a VoltAgent agent.
 *
 * Prerequisites:
 *   npm install @voltagent/core @guardianclaw/voltagent
 *
 * Run:
 *   npx ts-node examples/basic-agent.ts
 *
 * Note: For development, imports use relative paths.
 * In production, use: import { ... } from '@guardianclaw/voltagent';
 */
...
const customInputGuard = createGuardianClawInputGuardrail({
  enableCLAW: true,
  enableOWASP: true,
  blockUnsafe: true,
  logChecks: true,
...
const customOutputGuard = createGuardianClawOutputGuardrail({
  enablePII: true,
  redactPII: true,
