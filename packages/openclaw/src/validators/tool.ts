/**
 * @guardianclaw/openclaw - Tool Validator
 *
 ...
 * - CLAW validation on parameter content
...
export interface ToolValidationOptions {
  ...
  skipParams?: boolean;
  /** Skip CLAW validation on parameters */
  skipClaw?: boolean;
}
...
export async function validateTool(
  ...
  try {
    ...
    // 6. Validate string parameters for CLAW violations (unless skipped)
    if (!options?.skipClaw) {
      const clawIssues = validateParamContent(params);
...
/**
 * Validate string parameter content for CLAW violations.
 */
