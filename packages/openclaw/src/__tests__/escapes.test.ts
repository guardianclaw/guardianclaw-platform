...
describe('AllowOnceManager', () => {
  ...
  describe('grant', () => {
    it('should grant a token with default limits', () => {
      const token = manager.grant('session-1');

      expect(token.id).toBeDefined();
      expect(token.sessionId).toBe('session-1');
      expect(token.limits).toBe('any');
      expect(token.used).toBe(false);
      expect(token.expiresAt).toBeGreaterThan(Date.now());
    });

    it('should grant a token with specific limits', () => {
      const token = manager.grant('session-1', { limits: 'tool' });

      expect(token.limits).toBe('tool');
    });
    ...
    it('should replace existing token for same session', () => {
      const token1 = manager.grant('session-1', { limits: 'output' });
      const token2 = manager.grant('session-1', { limits: 'tool' });

      expect(token2.limits).toBe('tool');
  ...
  describe('check', () => {
    ...
    it('should check limits correctly', () => {
      manager.grant('session-1', { limits: 'output' });
    ...
    it('should match any limits to all requests', () => {
      manager.grant('session-1', { limits: 'any' });
  ...
  describe('use', () => {
    it('should consume token on use', () => {
      manager.grant('session-1', { limits: 'tool' });
    ...
    it('should fail for wrong limits', () => {
      manager.grant('session-1', { limits: 'output' });
...
describe('EscapeManager', () => {
  ...
  describe('shouldAllowOutput', () => {
    ...
    it('should allow with allow-once', () => {
      manager.grantAllowOnce('session-1', { limits: 'output' });
  ...
  describe('shouldAllowTool', () => {
    ...
    it('should allow with allow-once', () => {
      manager.grantAllowOnce('session-1', { limits: 'tool' });
  ...
  describe('shortcuts', () => {
    ...
    it('should support allow-once shortcuts', () => {
      const token = manager.grantAllowOnce('session-1', { limits: 'output' });
      expect(token.limits).toBe('output');
...
describe('Escape Hatches Integration', () => {
  ...
  it('should handle complete session lifecycle', () => {
    ...
    manager.grantAllowOnce(sessionId, { limits: 'tool' });
