...
describe('@guardianclaw/openclaw', () => {
  describe('Package Structure', () => {
    it('should export createGuardianClawHooks', async () => {
      const { createGuardianClawHooks } = await import('../index');
      expect(typeof createGuardianClawHooks).toBe('function');
    });
    ...
    it('should export version metadata', async () => {
      const { VERSION, PACKAGE_NAME, OPENCLAW_VERSION_RANGE } =
        await import('../index');

      expect(VERSION).toBe('3.0.0-rc.1');
      expect(PACKAGE_NAME).toBe('@guardianclaw/openclaw');
      expect(OPENCLAW_VERSION_RANGE).toBe('>=2026.0.0');
    });
