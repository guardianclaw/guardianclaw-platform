...
describe('End-to-End: Plugin Registration', () => {
  it('should register all hooks with OpenClaw API', () => {
...
describe('End-to-End: Hook Factory', () => {
  ...
  it('should inject seed in beforeAgentStart at guard level', () => {
    ...
    expect(result?.additionalContext).toContain('Credibility');
    expect(result?.additionalContext).toContain('Avoidance');
    expect(result?.additionalContext).toContain('Limits');
    expect(result?.additionalContext).toContain('Worth');
  });

  it('should inject seed in beforeAgentStart at watch level', () => {
    const hooks = createGuardianClawHooks({ level: 'watch' });
    ...
    expect(result).toBeDefined();
    expect(result?.additionalContext).toContain('claw');
  });
...
describe('End-to-End: Escape Manager', () => {
  ...
  it('should grant and consume allow-once token', () => {
    escapes.grantAllowOnce('session-1', { limits: 'output' });

    // Check token exists
    const state = escapes.allowOnce.getState('session-1');
    expect(state.active).toBe(true);

    // Use the token (requires 3 args: sessionId, limits, actionDescription)
...
describe('End-to-End: Alert Manager', () => {
  ...
  beforeEach(() => {
    ...
    alerts = new AlertManager({
      webhooks: [
        {
          url: 'https://webhook.example.com/claw',
          minSeverity: 'medium',
        },
      ],
  ...
  it('should send webhook on alert', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      'https://webhook.example.com/claw',
...
describe('End-to-End: Protection Level Behavior', () => {
  it('should allow everything at off level', async () => {
    const hooks = createGuardianClawHooks({ level: 'off' });
  ...
  it('should block at shield level', async () => {
    const hooks = createGuardianClawHooks({ level: 'shield' });
