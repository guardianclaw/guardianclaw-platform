/**
 * OpenClaw API Simulation Tests
 *
 * These tests simulate the REAL OpenClaw plugin API exactly as documented
 * in openclaw/src/plugins/types.ts to validate our integration is correct.
 *
...
// =============================================================================
// OpenClaw API Types (copied exactly from openclaw/src/plugins/types.ts)
// =============================================================================
...
/**
 * Simulates the real OpenClaw Plugin API.
 * Based on openclaw/src/plugins/types.ts OpenClawPluginApi
 */
function createOpenClawPluginApi(config: Record<string, unknown> = {}) {
  ...
  // Hook runner that mimics OpenClaw's behavior
...
describe('OpenClaw API Simulation Tests', () => {
  describe('Plugin Registration', () => {
    it('should register with OpenClaw API successfully', () => {
      const { api, hooks } = createOpenClawPluginApi({ level: 'guard' });
    ...
    it('should not register hooks when level is off', () => {
      const { api, hooks } = createOpenClawPluginApi({ level: 'off' });
    ...
    it('should log initialization', () => {
      const { api, logger } = createOpenClawPluginApi({ level: 'watch' });

      register(api);

      expect(logger.info).toHaveBeenCalledWith('GuardianClaw initialized', {
        level: 'watch',
      });
    });
  });
  ...
  describe('message_received hook', () => {
    it('should handle OpenClaw message_received event format', async () => {
      const { api, runHook } = createOpenClawPluginApi({ level: 'guard' });
      register(api);

      // Exact OpenClaw event format
    ...
    it('should analyze prompt injection attempts', async () => {
      const { api, runHook, logger } = createOpenClawPluginApi({ level: 'guard' });
      register(api);
  ...
  describe('before_agent_start hook', () => {
    it('should return OpenClaw-compatible result', async () => {
      const { api, runHook } = createOpenClawPluginApi({ level: 'guard' });
      register(api);
      ...
      // Result should match OpenClaw's expected type
      if (result) {
        expect(typeof result.prependSystemContext === 'string' || result.prependSystemContext === undefined).toBe(true);
    ...
    it('should inject seed at guard level', async () => {
      ...
      expect(result?.prependSystemContext).toContain('claw');
    });
  });

  describe('message_sending hook', () => {
    it('should return OpenClaw-compatible result format', async () => {
      const { api, runHook } = createOpenClawPluginApi({ level: 'guard' });
      register(api);
    ...
    it('should block data leaks with OpenClaw-compatible result', async () => {
      const { api, runHook } = createOpenClawPluginApi({ level: 'guard' });
      register(api);
      ...
      // Must return exactly what OpenClaw expects
      expect(result).toBeDefined();
      expect(result?.cancel).toBe(true);
      // Note: cancelReason is extra, OpenClaw ignores it
  ...
  describe('before_tool_call hook', () => {
    ...
    it('should block dangerous commands with OpenClaw-compatible result', async () => {
      const { api, runHook } = createOpenClawPluginApi({ level: 'guard' });
      register(api);
      ...
      // Must match OpenClaw's expected type
      expect(result).toBeDefined();
  ...
  describe('agent_end hook', () => {
    it('should handle OpenClaw agent_end event format', async () => {
      const { api, runHook } = createOpenClawPluginApi({ level: 'guard' });
      register(api);
  ...
  describe('Protection Levels', () => {
    it('watch level should log but never block', async () => {
      const { api, runHook } = createOpenClawPluginApi({ level: 'watch' });
      register(api);
    ...
    it('shield level should block more aggressively', async () => {
      const { api, runHook } = createOpenClawPluginApi({ level: 'shield' });
      register(api);
  ...
  describe('Context Handling', () => {
    ...
    it('should use sessionKey for agent context', async () => {
      ...
      const result = await runHook('before_prompt_build', event, ctx);
      expect(true).toBe(true);
    });
  });
  ...
  describe('Priority Ordering', () => {
    it('should register hooks with high priority', () => {
      ...
      const beforeAgentStart = hooks.find((h) => h.hookName === 'before_prompt_build');
