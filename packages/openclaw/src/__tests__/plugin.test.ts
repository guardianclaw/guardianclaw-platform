 ...
 *
 * Tests for the OpenClaw plugin adapter (plugin.ts).
 * Verifies that the thin adapter correctly connects GuardianClaw hooks
 * to the OpenClaw plugin API.
 */
...
function createMockOpenClawApi(config: Record<string, unknown> = {}) {
  const handlers: MockHandler[] = [];

  const api = {
    id: 'test-plugin',
    name: 'GuardianClaw Test',
    pluginConfig: config,
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
...
describe('register', () => {
  describe('Plugin Initialization', () => {
    it('should register without errors', () => {
      const api = createMockOpenClawApi({ level: 'watch' });

      expect(() => register(api)).not.toThrow();
    });

    it('should log initialization', () => {
      const api = createMockOpenClawApi({ level: 'guard' });

      register(api);

      expect(api.logger.info).toHaveBeenCalledWith(
        'GuardianClaw initialized',
  ...
  describe('Hook Registration', () => {
    it('should register all hooks in non-off mode', () => {
      ...
      expect(api._getHandler('before_prompt_build')).toBeDefined();
    ...
    it('should not register hooks when level is off', () => {
      const api = createMockOpenClawApi({ level: 'off' });

      register(api);

      expect(api.on).not.toHaveBeenCalled();
      expect(api.logger.info).toHaveBeenCalledWith(
        'GuardianClaw is disabled (level: off)'
      );
    });
    ...
    it('should register hooks with correct priorities', () => {
      ...
      expect(api._getHandler('before_prompt_build')?.priority).toBe(100);
  ...
  describe('Level Configurations', () => {
    ...
    it('should accept shield level', () => {
      const api = createMockOpenClawApi({ level: 'shield' });
...
describe('Hook Handlers', () => {
  ...
  describe('before_prompt_build', () => {
    it('should return seed context', () => {
      ...
      const handler = api._getHandler('before_prompt_build')?.handler;
      const event = { prompt: 'Hello' };
      const ctx = { agentId: 'agent-1', sessionKey: 'session-1' };

      const result = handler?.(event, ctx) as { prependSystemContext?: string } | undefined;
...
describe('Full Session Flow', () => {
  it('should handle a complete session lifecycle', async () => {
    ...
    // 1. Agent starts
    const startHandler = api._getHandler('before_prompt_build')?.handler;
    const startResult = startHandler?.({ prompt: 'Hello' }, ctx);
    expect(startResult?.prependSystemContext).toBeDefined();
