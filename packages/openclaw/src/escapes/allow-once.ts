/**
 * @guardianclaw/openclaw - Allow-Once Escape Hatch
 *
 * Provides a mechanism for users to bypass a single blocked action.
 * When GuardianClaw blocks something, the user can invoke "allow once"
...
export interface AllowOnceToken {
  ...
  /** Limits of the token */
  readonly limits: AllowOnceScope;
...
export interface GrantOptions {
  /** Limits of the token (default: 'any') */
  limits?: AllowOnceScope;
...
export class AllowOnceManager {
  ...
  grant(sessionId: string, options: GrantOptions = {}): AllowOnceToken {
    const now = Date.now();
    const limits = options.limits ?? 'any';
   ...
   * @param requiredScope - Required limits (optional, defaults to any match)
  ...
  check(sessionId: string, requiredScope?: AllowOnceScope): AllowOnceCheckResult {
    ...
    if (requiredScope && !this.scopeMatches(token.limits, requiredScope)) {
   ...
   * @param requiredScope - Required limits for the action
  ...
  /**
   * Check if a token limits matches the required limits.
   */
...
/**
 * Check if a limits is valid.
 *
 * @param limits - Limits to check
 * @returns True if valid
 */
export function isValidScope(limits: unknown): limits is AllowOnceScope {
  return limits === 'output' || limits === 'tool' || limits === 'any';
}
