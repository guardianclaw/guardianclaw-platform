/**
 * @guardianclaw/openclaw - Core Hook Handlers
 *
 ...
 * - Framework-agnostic (no OpenClaw dependencies)
...
import type { LevelConfig, GuardianClawOpenClawConfig, DetectedIssue } from '../types';
...
function formatCancelReason(issues: readonly DetectedIssue[]): string {
  if (issues.length === 0) {
    return 'Blocked by GuardianClaw safety validation';
  }
...
function formatToolBlockReason(
  ...
  if (!firstIssue) {
    return `Tool '${toolName}' blocked by GuardianClaw safety validation`;
  }
