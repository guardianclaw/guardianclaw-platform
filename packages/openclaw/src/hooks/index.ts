/**
 * @guardianclaw/openclaw - Hooks Module
 *
 * This module provides OpenClaw hook implementations for
 * integrating GuardianClaw safety monitoring.
 *
 * Architecture:
 ...
 * @example
 * ```typescript
 * import { createGuardianClawHooks } from '@guardianclaw/openclaw/hooks';
 *
 * const hooks = createGuardianClawHooks({ level: 'guard' });
...
import type { GuardianClawOpenClawConfig, LevelConfig } from '../types';
...
export type {
  ...
  BeforeToolCallEvent,
  AgentEndEvent,
  // GuardianClaw result types (rich)
  ...
  // OpenClaw-compatible result types
  OpenClawAgentStartResult,
  OpenClawMessageSendingResult,
...
// =============================================================================
// GuardianClaw Hooks Bundle
// =============================================================================

/**
 * Complete set of GuardianClaw hooks for OpenClaw.
 *
 * These hooks return OpenClaw-compatible result types.
 * Internally, they use the richer GuardianClaw result types for
 * logging, metrics, and decision-making.
 */
export interface GuardianClawHooks {
  ...
  beforeAgentStart: (event: BeforeAgentStartEvent) => OpenClawAgentStartResult | undefined;
  /** Handler for message_sending (can cancel/modify) */
  messageSending: (event: MessageSendingEvent) => Promise<OpenClawMessageSendingResult | undefined>;
  /** Handler for before_tool_call (can block/modify) */
  beforeToolCall: (event: BeforeToolCallEvent) => Promise<OpenClawToolCallResult | undefined>;
...
interface HooksState {
  ...
  readonly config: GuardianClawOpenClawConfig;
...
/**
 * Create GuardianClaw hooks for OpenClaw integration.
 *
 ...
 * const hooks = createGuardianClawHooks({ level: 'guard' });
 *
 * // In OpenClaw plugin registration:
 * export const hooks = {
 ...
 * ```
 */
export function createGuardianClawHooks(
  userConfig: Partial<GuardianClawOpenClawConfig>
): GuardianClawHooks {
 ...
 * Returns OpenClaw-compatible result with additionalContext.
 */
function createBeforeAgentStartHook(state: HooksState) {
  return (event: BeforeAgentStartEvent): OpenClawAgentStartResult | undefined => {
    const context = createContext(state, event.sessionId);
 ...
 * Returns OpenClaw-compatible result with cancel flag.
 */
function createMessageSendingHook(state: HooksState) {
  return async (event: MessageSendingEvent): Promise<OpenClawMessageSendingResult | undefined> => {
    const context = createContext(state, event.sessionId);
 ...
 * Returns OpenClaw-compatible result with block flag.
 */
function createBeforeToolCallHook(state: HooksState) {
  return async (event: BeforeToolCallEvent): Promise<OpenClawToolCallResult | undefined> => {
    const context = createContext(state, event.sessionId);
