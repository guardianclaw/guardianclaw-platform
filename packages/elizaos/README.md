...
> AI Safety Plugin for ElizaOS - CLAW Protocol Validation for Autonomous Agents

[![npm version](https://badge.fury.io/js/@guardianclaw%2Felizaos-plugin.svg)](https://www.npmjs.com/package/@guardianclaw/elizaos-plugin)
...
- **Multi-instance support**: Each `clawPlugin()` call creates an isolated instance registered in a global registry. Use `instanceName` config option for named access.
...
- `clawPreAction`: Validates incoming messages (runs on all messages)
- `clawPostAction`: Reviews outputs before delivery (runs on all responses)
- `clawMemoryIntegrity`: Verifies memory integrity on retrieval (v1.1.0+)
...
1. **Initialization**: When the plugin initializes, it injects the GuardianClaw seed into the agent's system prompt
2. **Pre-action**: Before any message is processed, `clawPreAction` validates the input
3. **Provider**: The `clawSafety` provider adds CLAW context to agent state
4. **Action**: Users can explicitly request safety checks via `GCLAW_SAFETY_CHECK`
5. **Post-action**: Before responses are sent, `clawPostAction` validates outputs
...
- **CREDIBILITY Gate**: Detects deception attempts, role manipulation, fake identity claims
- **AVOIDANCE Gate**: Detects violence, hacking, malware, weapons, dangerous substances
- **LIMITS Gate**: Detects jailbreak attempts, instruction overrides, prompt extraction
- **WORTH Gate**: Detects purposeless destruction patterns
...
2. **WORTH Gate Gaps**: Abstract concepts like "purposeless action" are difficult to detect via regex. Examples:
...
```typescript
import { TextTooLargeError } from '@guardianclaw/elizaos-plugin';
...
import type {
  ...
  ValidationContext,
  // Plugin state types
  GuardianClawLogger,      // For custom logger implementations
...
- [`guardianclaw`](https://www.npmjs.com/package/@guardianclaw/core) - Core GuardianClaw SDK
- [`mcp-server-guardianclaw`](https://www.npmjs.com/package/mcp-server-guardianclaw) - MCP Server
...
Made with care by [GuardianClaw Team](https://guardianclaw.org) | [contact@guardianclaw.org](mailto:contact@guardianclaw.org)
