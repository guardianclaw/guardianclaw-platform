 ...
 *   - Synchronized with Python guardianclaw.memory module
 *
...
export interface IntegrityMetadata {
  claw_integrity_hash: string;
  claw_signed_at: number;
  claw_source: MemorySource;
  claw_integrity_version: string;
}
...
export class MemoryIntegrityChecker {
  ...
  static readonly CONTENT_VALIDATION_KEY = 'claw_content_validation';
  ...
  signMemory(memory: Memory, source: MemorySource = 'unknown'): Memory {
    ...
    const integrityMetadata: IntegrityMetadata = {
      claw_integrity_hash: hash,
      claw_signed_at: Date.now(),
      claw_source: source,
      claw_integrity_version: MemoryIntegrityChecker.VERSION,
    };
  ...
  verifyMemory(memory: Memory): MemoryVerificationResult {
    ...
    if (!metadata?.claw_integrity_hash) {
      return {
        valid: false,
        memoryId: memory.id,
    ...
    const version = metadata.claw_integrity_version;
    if (version !== '1.0' && version !== '2.0') {
      return {
        ...
        trustScore: 0,
        source: metadata.claw_source || 'unknown',
      };
    }

    // Recompute hash and compare
    const source = metadata.claw_source || 'unknown';
    const signableContent = this.getSignableContent(memory, source);
    ...
    try {
      isValid = crypto.timingSafeEqual(
        ...
        Buffer.from(metadata.claw_integrity_hash, 'hex')
      );
    } catch {
      isValid = false;
    }
    ...
    if (metadata.claw_content_validation) {
      trustScore *= metadata.claw_content_validation.trustAdjustment;
    }
  ...
  hasContentSuspicion(memory: Memory): boolean {
    ...
    return !!metadata?.claw_content_validation;
  }

  /**
   * Get content validation info from memory metadata.
   */
  getContentValidationInfo(memory: Memory): ContentValidationMetadata['claw_content_validation'] | undefined {
    const metadata = memory.content?.metadata as ContentValidationMetadata | undefined;
    return metadata?.claw_content_validation;
  }
...
export function createMemoryIntegrityChecker(
  ...
  const key =
    secretKey ||
    process.env.GCLAW_MEMORY_SECRET ||
...
export function hasIntegrityMetadata(memory: Memory): boolean {
  ...
  return !!metadata?.claw_integrity_hash;
}
...
export function getMemorySource(memory: Memory): MemorySource {
  ...
  const metadata = memory.content?.metadata as IntegrityMetadata | undefined;
  return metadata?.claw_source || 'unknown';
}
...
export function getSignedTimestamp(memory: Memory): number | undefined {
  ...
  const metadata = memory.content?.metadata as IntegrityMetadata | undefined;
  return metadata?.claw_signed_at;
}
