...
describe('CLI Formatters', () => {
  ...
  describe('formatStatus', () => {
    it('should format basic status', () => {
      ...
      expect(result).toContain('GuardianClaw Status');
  ...
  describe('formatHelp', () => {
    it('should format command list', () => {
      const commands = [
        ...
        { name: 'level', description: 'Change level', usage: '/claw level [name]' },
      ];

      const result = formatHelp(commands, false);

      expect(result).toContain('GuardianClaw Commands');
    ...
    it('should show examples', () => {
      const commands = [
        {
          ...
          examples: ['/claw test 1', '/claw test 2'],
        },
      ];
      ...
      expect(result).toContain('/claw test 1');
      expect(result).toContain('/claw test 2');
    });
  });
...
describe('CLI Commands', () => {
  ...
  describe('allow-once command', () => {
    ...
    it('should accept limits argument', async () => {
      const result = await executeCommand('allow-once tool', context);

      expect(result.success).toBe(true);
      expect(result.message).toContain('limits: tool');
    });
    ...
    it('should reject invalid limits', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.message).toContain('Invalid limits');
    });
  ...
  describe('help command', () => {
    it('should list all commands', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.message).toContain('GuardianClaw Commands');
      expect(result.message).toContain('status');
