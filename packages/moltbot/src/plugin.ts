/**
 * @guardianclaw/moltbot - Moltbot Plugin Entry Point
 *
 * This file is a thin adapter that connects the GuardianClaw hooks
...
import type { GuardianClawMoltbotConfig } from './types';
...
/**
 * Register the GuardianClaw plugin with Moltbot.
 *
 * This function is called by Moltbot when the plugin is loaded.
 * It creates GuardianClaw hooks and adapts them to Moltbot's API.
 *
 * @param api - Moltbot plugin API
 */
export function register(api: MoltbotPluginApi): void {
  const userConfig = (api.pluginConfig ?? {}) as Partial<GuardianClawMoltbotConfig>;

  // Create GuardianClaw hooks using the factory
  const hooks = createGuardianClawHooks(userConfig);

  // Log initialization
  api.logger.info('GuardianClaw initialized', {
...
function registerMessageReceivedHook(
  ...
  api.on(
    ...
    (event: MoltbotMessageReceivedEvent, ctx: MoltbotMessageContext): void => {
      ...
      // Call GuardianClaw hook (fire-and-forget)
