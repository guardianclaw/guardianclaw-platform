 ...
 *
 * Provides access to localized messages for the GuardianClaw plugin.
 * Supports parameter substitution using MessageFormat.
 *
 * Usage:
 * ```kotlin
 * val message = GuardianClawBundle.message("action.scanSecrets.title")
 * val formatted = GuardianClawBundle.message("action.scanSecrets.result.found", 5)
 * ```
 */
object GuardianClawBundle {

    @NonNls
    private const val BUNDLE = "messages.GuardianClawBundle"

    private val bundle = DynamicBundle(GuardianClawBundle::class.java, BUNDLE)
