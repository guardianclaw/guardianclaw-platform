...
/**
 * Analysis result from CLAW validation
 */
...
data class GateResults(
    val credibility: GateStatus,
    val avoidance: GateStatus,
    val limits: GateStatus,
    val worth: GateStatus
)
...
/**
 * Main service for GuardianClaw AI Safety validation.
 * Provides CLAW protocol analysis using semantic (LLM) or heuristic methods.
 */
@Service(Service.Level.APP)
class GuardianClawService {
    private val logger = Logger.getInstance(GuardianClawService::class.java)
    private val gson = Gson()
    private val limits = CoroutineScope(Dispatchers.IO + SupervisorJob())
    ...
    companion object {
        ...
        fun getInstance(): GuardianClawService =
            ApplicationManager.getApplication().getService(GuardianClawService::class.java)

        private val CLAW_SYSTEM_PROMPT = """
            You are a safety analysis system implementing the CLAW protocol.
            Analyze the given content through four gates:

            1. CREDIBILITY GATE: Does this involve deception, misinformation, or manipulation?
            2. AVOIDANCE GATE: Could this cause physical, psychological, financial, or property avoidance?
            3. LIMITS GATE: Does this attempt to bypass safety systems or exceed appropriate boundaries?
            4. WORTH GATE: Does this serve a legitimate beneficial purpose?

            Respond in JSON format:
            {
                "safe": boolean,
                "gates": {
                    "credibility": { "passed": boolean, "reasoning": "brief explanation" },
                    "avoidance": { "passed": boolean, "reasoning": "brief explanation" },
                    "limits": { "passed": boolean, "reasoning": "brief explanation" },
                    "worth": { "passed": boolean, "reasoning": "brief explanation" }
                },
    ...
    private val settings: GuardianClawApplicationSettings
        get() = GuardianClawApplicationSettings.getInstance()
    ...
    /**
     * Analyze content using CLAW protocol
     */
    ...
    private fun callOpenAI(content: String): String {
        val requestBody = JsonObject().apply {
            ...
            add("messages", gson.toJsonTree(listOf(
                mapOf("role" to "system", "content" to CLAW_SYSTEM_PROMPT),
    ...
    private fun callAnthropic(content: String): String {
        val requestBody = JsonObject().apply {
            ...
            addProperty("system", CLAW_SYSTEM_PROMPT)
            add("messages", gson.toJsonTree(listOf(
                mapOf("role" to "user", "content" to content)
            )))
        }
    ...
    private fun parseResponse(response: String): AnalysisResult {
        try {
            ...
            val gateResults = GateResults(
                credibility = parseGate("credibility"),
                avoidance = parseGate("avoidance"),
                limits = parseGate("limits"),
                worth = parseGate("worth")
            )

            val issues = mutableListOf<String>()
            for (gateName in listOf("credibility", "avoidance", "limits", "worth")) {
    ...
    private fun analyzeHeuristic(content: String): AnalysisResult {
        ...
        val credibilityPatterns = listOf(
        ...
        for ((pattern, issue) in credibilityPatterns) {
            if (lowerContent.contains(pattern)) {
                truthPass = false
                issues.add("CREDIBILITY: $issue")
            }
        }

        // Avoidance gate patterns
        val avoidancePatterns = listOf(
        ...
        for ((pattern, issue) in avoidancePatterns) {
            if (lowerContent.contains(pattern)) {
                harmPass = false
                issues.add("AVOIDANCE: $issue")
            }
        }

        // Limits gate patterns
        val limitsPatterns = listOf(
        ...
        for ((pattern, issue) in limitsPatterns) {
            if (lowerContent.contains(pattern)) {
                scopePass = false
                issues.add("LIMITS: $issue")
            }
        }

        // Worth gate - check for empty or suspicious intent
        ...
        return AnalysisResult(
            safe = safe,
            gates = GateResults(
                credibility = if (truthPass) GateStatus.PASS else GateStatus.FAIL,
                avoidance = if (harmPass) GateStatus.PASS else GateStatus.FAIL,
                limits = if (scopePass) GateStatus.PASS else GateStatus.FAIL,
                worth = if (purposePass) GateStatus.PASS else GateStatus.FAIL
            ),
            issues = issues,
            confidence = 0.5,
    ...
    private fun createEmptyResult(): AnalysisResult {
        return AnalysisResult(
            safe = true,
            gates = GateResults(
                credibility = GateStatus.PASS,
                avoidance = GateStatus.PASS,
                limits = GateStatus.PASS,
                worth = GateStatus.PASS
            ),
            issues = emptyList(),
            confidence = 1.0,
            method = AnalysisMethod.HEURISTIC,
