...
import dev.guardianclaw.jetbrains.services.AnalysisMethod
import dev.guardianclaw.jetbrains.services.AnalysisResult
import dev.guardianclaw.jetbrains.services.GateStatus
...
/**
 * Factory for creating the GuardianClaw tool window
 */
class GuardianClawToolWindowFactory : ToolWindowFactory, DumbAware {

    companion object {
        private val panels = mutableMapOf<Project, GuardianClawPanel>()

        fun showResult(project: Project, result: AnalysisResult) {
            ApplicationManager.getApplication().invokeLater {
                val toolWindow = ToolWindowManager.getInstance(project).getToolWindow("GuardianClaw")
                toolWindow?.show {
 ...
 */
class GuardianClawPanel : JPanel(BorderLayout()) {
    ...
    private fun showWelcome() {
        ...
        val welcomeLabel = JBLabel("<html><b>GuardianClaw AI Safety</b></html>")
        ...
        val infoLabel = JBLabel("<html>" +
                ...
                "or right-click and select <b>GuardianClaw → Analyze Selection</b>.<br/><br/>" +
                "Configure API keys in <b>Settings → Tools → GuardianClaw AI Safety</b><br/>" +
    ...
    fun showResult(result: AnalysisResult) {
        ...
        addGateRow("Credibility", result.gates.credibility)
        addGateRow("Avoidance", result.gates.avoidance)
        addGateRow("Limits", result.gates.limits)
        addGateRow("Worth", result.gates.worth)
