package dev.guardianclaw.jetbrains.inspections
 ...
 */
class GuardianClawInspection : LocalInspectionTool() {
    ...
    override fun getGroupDisplayName(): String = "GuardianClaw AI Safety"

    override fun getShortName(): String = "GuardianClawSafetyInspection"

    override fun isEnabledByDefault(): Boolean = true

    override fun buildVisitor(holder: ProblemsHolder, isOnTheFly: Boolean): PsiElementVisitor {
        val settings = GuardianClawApplicationSettings.getInstance()
        ...
        return object : PsiElementVisitor() {
            override fun visitFile(file: PsiFile) {
                ...
                checkPatterns(file, text, TRUTH_PATTERNS, "Credibility Gate", holder)
                checkPatterns(file, text, HARM_PATTERNS, "Avoidance Gate", holder)
                checkPatterns(file, text, SCOPE_PATTERNS, "Limits Gate", holder)
                checkPatterns(file, text, PURPOSE_PATTERNS, "Worth Gate", holder)
            }
        }
    }
            ...
            while (true) {
                ...
                if (element != null) {
                    holder.registerProblem(
                        element,
                        "GuardianClaw: $gateName violation - $description",
                        ProblemHighlightType.WARNING,
                        GuardianClawQuickFix()
                    )
                }
 ...
 */
class GuardianClawQuickFix : LocalQuickFix {
    override fun getName(): String = "Analyze with GuardianClaw"

    override fun getFamilyName(): String = "GuardianClaw AI Safety"

    override fun applyFix(project: Project, descriptor: ProblemDescriptor) {
        // Open GuardianClaw tool window and suggest full analysis
        val toolWindow = com.intellij.openapi.wm.ToolWindowManager.getInstance(project)
            .getToolWindow("GuardianClaw")
        toolWindow?.show {
            dev.guardianclaw.jetbrains.ui.GuardianClawToolWindowFactory.showMessage(
                project,
