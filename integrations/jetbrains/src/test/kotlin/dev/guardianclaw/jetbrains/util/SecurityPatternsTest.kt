...
class SecurityPatternsTest {
    ...
    inner class PromptInjectionTests {
        ...
        @Test
        @DisplayName("Should map patterns to CLAW gates")
        fun validateGateMapping() {
            val limitsPatterns = SecurityPatterns.PROMPT_INJECTION_PATTERNS.filter { "limits" in it.gates }
            val avoidancePatterns = SecurityPatterns.PROMPT_INJECTION_PATTERNS.filter { "avoidance" in it.gates }

            assertThat(limitsPatterns).isNotEmpty()
            assertThat(avoidancePatterns).isNotEmpty()
        }
    }
