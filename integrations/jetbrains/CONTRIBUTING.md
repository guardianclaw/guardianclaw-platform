# Contributing to GuardianClaw AI Safety JetBrains Plugin

Thank you for your interest in contributing to GuardianClaw! This document provides guidelines for contributing to the JetBrains plugin.
...
src/
├── main/
│   ├── kotlin/dev/guardianclaw/jetbrains/
...
2. Register in `plugin.xml`:
   ```xml
   <action id="GuardianClaw.YourAction"
           class="dev.guardianclaw.jetbrains.actions.YourAction"
           text="Your Action"
...
1. Add English text to `GuardianClawBundle.properties`:
   ```properties
   your.key=Your English text
   ```

2. Add translations to locale files (e.g., `GuardianClawBundle_pt_BR.properties`):
   ```properties
...
3. Use in code:
   ```kotlin
   val message = GuardianClawBundle.message("your.key")
   ```
