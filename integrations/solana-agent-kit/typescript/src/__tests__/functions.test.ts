/**
 * Tests for GuardianClaw functions (methods exposed to SolanaAgentKit)
 *
...
describe("GuardianClaw Functions", () => {
  ...
  describe("validateTransaction", () => {
    ...
    it("should reject transaction without purpose when required", () => {
      ...
      expect(result.concerns.some((c) => c.includes("worth"))).toBe(true);
    });
  });
  ...
  describe("checkSafety", () => {
    it("should return true for safe transactions", () => {
      const isSafe = checkSafety(
        mockAgent,
        "query", // query doesn't require worth
        10,
        VALID_ADDRESS
      );
    ...
    it("should return false when worth is missing for transfer", () => {
      // Transfer requires worth, so without it should fail
      const isSafe = checkSafety(mockAgent, "transfer", 10, VALID_ADDRESS);

      expect(isSafe).toBe(false);
    });

    it("should return true when worth is provided for transfer", () => {
      const isSafe = checkSafety(
        mockAgent,
        "transfer",
        10,
    ...
    it("should accept worth as 4th parameter", () => {
      const isSafe = checkSafety(
        mockAgent,
        "swap",
        25,
  ...
  describe("blockAddress", () => {
    ...
    it("should throw error for invalid address", () => {
      ...
      }).toThrow("[GuardianClaw] Invalid address format");
    });
  ...
  describe("getSafetyStatus", () => {
    ...
    it("should reflect validation history", () => {
      ...
      validateTransaction(mockAgent, { action: "query", worth: VALID_PURPOSE });
      validateTransaction(mockAgent, { action: "test", worth: VALID_PURPOSE });
