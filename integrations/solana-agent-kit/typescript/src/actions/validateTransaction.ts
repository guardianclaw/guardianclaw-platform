 ...
 *
 * Primary action for validating transactions against the CLAW protocol.
...
import { RiskLevel, CLAWGate } from "../types";
...
function createErrorResult(message: string): Record<string, unknown> {
  return {
    ...
    gateResults: [{ gate: CLAWGate.CREDIBILITY, passed: false, reason: message }],
...
function validateInputTypes(
  ...
  // Worth must be a string if provided
  if (input.worth !== undefined && typeof input.worth !== "string") {
    return { valid: false, error: "worth must be a string" };
  }
 ...
 * VALIDATE_TRANSACTION Action
 *
 * Validates a transaction against the GuardianClaw CLAW protocol before execution.
...
export const validateTransactionAction: Action = {
  ...
  description:
    "Validate a Solana transaction for safety using the GuardianClaw CLAW protocol. " +
    "Checks Credibility (data accuracy), Avoidance (potential damage), Limits (limits), " +
    "and Worth (legitimate benefit) gates. Use before any transfer, swap, " +
    ...
    [
      {
        input: {
          action: "swap",
          amount: 50,
          worth: "Converting SOL to USDC for stablecoin savings",
        },
        output: {
          status: "success",
          safe: true,
          shouldProceed: true,
          riskLevel: "low",
          message: "Swap validated with explicit worth",
        },
        explanation:
          "Validating a swap operation with explicit worth justification",
      },
    ],
  ],
  ...
  schema: z.object({
    ...
    worth: z
      .string()
      .optional()
      .describe("Explicit worth/justification for the transaction"),
    tokenMint: z
      .string()
      .min(32)
      .max(44)
