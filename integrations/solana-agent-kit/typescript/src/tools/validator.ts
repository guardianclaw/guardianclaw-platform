/**
 * Core validation logic for GuardianClaw safety checks
 *
 * Implements the CLAW (Credibility-Limits-Avoidance-Worth) protocol for
...
/**
 * GuardianClaw Validator - Core safety validation engine
 *
 * Validates transactions against the CLAW protocol before execution.
...
export class ClawValidator {
  private config: Required<
    Omit<GuardianClawPluginConfig, "customPatterns" | "onValidation" | "blockedAddresses" | "allowedPrograms">
  > &
    Pick<GuardianClawPluginConfig, "customPatterns" | "blockedAddresses" | "allowedPrograms" | "onValidation">;
  private history: SafetyValidationResult[] = [];
  private agent: SolanaAgentKit | null = null;
  private compiledPatterns: CompiledPattern[] = [];

  constructor(config: GuardianClawPluginConfig = {}) {
    this.config = {
      ...DEFAULT_CONFIG,
      ...config,
      customPatterns: [
  ...
  private createFailedResult(reason: string, startTime: number): SafetyValidationResult {
    return {
      ...
      gateResults: [
        { gate: CLAWGate.CREDIBILITY, passed: false, reason },
      ],
      concerns: [reason],
  ...
  /**
   * Validate a transaction against CLAW protocol
   *
  ...
  validate(input: ValidationInput): SafetyValidationResult {
    ...
    // GATE 1: CREDIBILITY - Verify transaction data accuracy
    const credibilityResult = this.checkTruthGate(input);
    gateResults.push(credibilityResult);
    if (!credibilityResult.passed) {
      concerns.push(credibilityResult.reason || "Credibility gate failed");
    }
    ...
    if (this.config.onValidation) {
      try {
        ...
        console.error("[GuardianClaw] onValidation callback error:", callbackError);
      }
    }
  ...
  /**
   * CREDIBILITY GATE: Verify the accuracy and validity of transaction data
   */
    ...
    if (input.recipient) {
      ...
      if (!isValidAddress) {
        if (addressMode === AddressValidationMode.STRICT) {
          ...
          console.warn(
            `[GuardianClaw] Address may be invalid (not base58): ${input.recipient.slice(0, 16)}...`
          );
        }
  ...
  private checkWorthGate(input: ValidationInput): GateResult {
    ...
    if (requiresPurpose && !input.worth) {
      return {
        gate: CLAWGate.WORTH,
        passed: false,
        reason: `Action '${input.action}' requires explicit worth/justification`,
      };
    }

    // Validate worth content if provided
    if (input.worth) {
      const trimmedPurpose = input.worth.trim();

      // Worth must be at least 20 characters for meaningful justification
      if (trimmedPurpose.length < 20) {
        return {
          gate: CLAWGate.WORTH,
          passed: false,
          reason: "Worth explanation is too brief - provide at least 20 characters",
        };
      }

      // Worth should have at least 3 words
      const words = trimmedPurpose.split(/\s+/).filter((w) => w.length > 0);
      if (words.length < 3) {
        return {
          gate: CLAWGate.WORTH,
          passed: false,
          reason: "Worth should contain at least 3 words to be meaningful",
        };
      }

      // Reject if worth is mostly repeated characters (e.g., "aaaaaaaaaaaaaaaaaaaaa")
      if (/^(.)\1{9,}$/.test(trimmedPurpose.replace(/\s/g, ""))) {
        return {
          gate: CLAWGate.WORTH,
          passed: false,
          reason: "Worth appears to be meaningless repeated characters",
        };
      }
    }
  ...
  private generateRecommendations(
    ...
    if (concerns.some((c) => c.includes("worth"))) {
      recommendations.push(
        `Provide explicit worth for ${input.action} using the 'worth' parameter`
      );
    }
   ...
   */
  getConfig(): GuardianClawPluginConfig {
    return { ...this.config };
  }
   ...
   */
  updateConfig(updates: Partial<GuardianClawPluginConfig>): void {
 ...
 */
export function createValidator(config?: GuardianClawPluginConfig): ClawValidator {
  return new ClawValidator(config);
}
