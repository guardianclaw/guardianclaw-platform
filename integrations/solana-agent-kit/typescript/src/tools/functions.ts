/**
 * GuardianClaw validation functions for Solana Agent Kit
 *
 * These functions are exposed as methods on the SolanaAgentKit instance
 * when the GuardianClaw plugin is registered.
 *
 ...
 * by the GuardianClawPlugin during registration.
 */

import type { SolanaAgentKit } from "solana-agent-kit";
import { PublicKey } from "@solana/web3.js";
import { ClawValidator } from "./validator";
import type {
...
let sharedValidator: ClawValidator | null = null;
...
export function initializeValidator(
  ...
  config?: GuardianClawPluginConfig
): void {
  sharedValidator = new ClawValidator(config);
 ...
 */
export function setSharedValidator(validator: ClawValidator): void {
...
function getValidator(): ClawValidator {
  if (!sharedValidator) {
    throw new Error(
      "[GuardianClaw] Validator not initialized. Ensure GuardianClawPlugin is registered before calling validation methods."
    );
  }
 ...
 * against the CLAW protocol (Credibility, Limits, Avoidance, Worth).
 *
 ...
 * @param worth - Optional worth justification (required for some actions)
...
export function checkSafety(
  ...
  const validator = getValidator();
  const result = validator.validate({ action, amount, recipient, worth });
...
export function blockAddress(_agent: SolanaAgentKit, address: string): void {
  if (!isValidSolanaAddress(address)) {
    throw new Error(
      `[GuardianClaw] Invalid address format: ${address?.slice(0, 16) || "empty"}... - must be valid Solana base58 address`
    );
  }
  const validator = getValidator();
...
export function updateSafetyConfig(
  _agent: SolanaAgentKit,
  config: Partial<GuardianClawPluginConfig>
): void {
  const validator = getValidator();
