 ...
 * DeFi Safety Example
 *
 * Demonstrates GuardianClaw protecting DeFi operations like swaps,
...
import { RiskLevel } from "@guardianclaw/solana-agent-kit";
...
async function main() {
  ...
  const agent = new SolanaAgentKit(
    ...
    .use(
      GuardianClawPlugin({
        ...
        // DeFi-specific actions that need worth
        requirePurposeFor: [
          "swap",
          "stake",
          "unstake",
  ...
  const stakeResult = await agent.methods.validateTransaction({
    action: "stake",
    amount: 50,
    worth: "Long-term SOL staking with Marinade for yield",
    metadata: {
      protocol: "Marinade",
      expectedAPY: "7.5%",
    },
  ...
  const riskySwapResult = await agent.methods.validateTransaction({
    action: "swap",
    amount: 100,
    worth: "Quick speculative trade for testing purposes",
    metadata: {
      fromToken: "SOL",
  ...
  const bridgeResult = await agent.methods.validateTransaction({
    action: "bridge",
    amount: 25,
    worth: "Moving funds to Ethereum for NFT purchase on OpenSea",
    metadata: {
      fromChain: "Solana",
      toChain: "Ethereum",
      bridge: "Wormhole",
  ...
  // Example 5: Missing worth rejection
  console.log("\n=== Example 5: Missing Worth Rejection ===");
