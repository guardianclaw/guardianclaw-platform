 ...
 * Basic Usage Example
 *
 * Demonstrates how to integrate GuardianClaw with Solana Agent Kit
...
async function main() {
  // Initialize Solana Agent Kit with GuardianClaw plugin
  ...
  const transferResult = await agent.methods.validateTransaction({
    ...
    worth: "Payment for NFT purchase from verified seller",
  });
  ...
  // Note: checkSafety accepts worth as 4th parameter for actions that require it
  ...
  const blockedResult = await agent.methods.validateTransaction({
    ...
    worth: "Testing transfer to blocked address for validation",
  });
