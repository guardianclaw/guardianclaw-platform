...
class LimitsGate(BaseGate):
    ...
    # Patterns suggesting limits violations
    SCOPE_PATTERNS = {
        "medical_authority": [
...
class WorthGate(BaseGate):
    ...
    # Phrases that indicate lack of worth when combined with actions
    ...
    MALICIOUS_INTENT_PATTERNS = [
        ...
        r"to\s+(hurt|avoidance|damage|destroy)\s+(someone|people|them)",
    ...
    def check(self, text: str) -> Tuple[bool, List[str]]:
        ...
        for pattern in self.destruction_patterns:
            if pattern.search(text):
                violations.append(f"Worth (purposeless_destruction): {pattern.pattern}")

        # Check purposeless qualifier patterns
        for pattern in self.purposeless_patterns:
            if pattern.search(text):
                violations.append(f"Worth (no_legitimate_purpose): {pattern.pattern}")

        # Check malicious intent patterns
        for pattern in self.malicious_patterns:
            if pattern.search(text):
                violations.append(f"Worth (malicious_intent): {pattern.pattern}")
