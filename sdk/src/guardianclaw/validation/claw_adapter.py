"""
GuardianClaw v3.0 Adapter - Bridge between v3.0 core and v2.x interface.

This adapter allows existing integrations to benefit from the improved
GuardianClaw v3.0 detection capabilities while maintaining full backwards
compatibility with the ValidationResult interface.

Architecture:
    Integration calls → GuardianClawV3Adapter.validate() → InputValidator (v3.0)
                                                     ↓
...
Usage:
    ...
    from guardianclaw.validation.claw_adapter import GuardianClawV3Adapter

    adapter = GuardianClawV3Adapter()
...
from guardianclaw.validation.types import (
    ValidationResult,
    ValidationLayer,
    ValidationMode,
    RiskLevel,
)
from guardianclaw.validation.config import ValidationConfig

logger = logging.getLogger("guardianclaw.validation.claw_adapter")


class GuardianClawV3Adapter:
    """
    Adapter that uses GuardianClaw v3.0 internally while exposing v2.x interface.
    ...
    def _ensure_initialized(self) -> None:
        ...
        try:
            ...
            logger.debug(
                f"GuardianClawV3Adapter initialized (embeddings={self._use_embeddings})"
            )

        except ImportError as e:
            logger.error(f"Failed to import v3.0 validators: {e}")
            raise ImportError(
                "GuardianClawV3Adapter requires guardianclaw.detection module"
            ) from e

    def validate(self, content: str) -> ValidationResult:
        """
        Validate content using GuardianClaw v3.0 InputValidator.
...
class AsyncGuardianClawV3Adapter:
    """
    Async version of GuardianClawV3Adapter.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._sync_adapter = GuardianClawV3Adapter(
            config=config,
    ...
    async def validate_action(
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self._sync_adapter.validate_action, action_name, action_args, worth
        )
