...
Layer 1 - Heuristic (CLAWValidator):
...
from guardianclaw.core.exceptions import ValidationError, ConfigurationError


logger = logging.getLogger("guardianclaw.validation.layered")
...
class LayeredValidator:
    ...
    def _init_heuristic(self) -> None:
        ...
        try:
            ...
            self._heuristic = CLAWValidator()
            logger.debug("Initialized CLAWValidator (heuristic layer)")
        except ImportError as e:
            logger.warning(f"Could not import CLAWValidator: {e}")
    ...
    def validate(self, content: str) -> ValidationResult:
        ...
        if self._heuristic:
            try:
                ...
                # CLAWValidator returns Dict with is_safe and violations
        ...
        if self._semantic:
            try:
                ...
                # SemanticValidator returns CLAWResult
    ...
    def validate_action(
        ...
        Example:
            result = validator.validate_action(
                ...
                worth="Notify user of account update",
            )
        """
    ...
    def validate_action_plan(
        ...
        additional checks beyond standard CLAW validation. It combines:
        ...
        - Standard CLAW validation via heuristic/semantic layers
        ...
        # Run standard CLAW validation
        claw_result = self.validate(action_plan)

        # Combine results
        if not claw_result.is_safe:
            concerns.extend([f"Validation: {v}" for v in claw_result.violations])
...
class AsyncLayeredValidator:
    ...
    async def validate_action_plan(
        ...
        # Run standard CLAW validation
        claw_result = await self.validate(action_plan)
