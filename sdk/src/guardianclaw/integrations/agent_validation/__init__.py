...
Usage:
    ...
    from guardianclaw.integrations.agent_validation import safety_check
...
logger = logging.getLogger("guardianclaw.agent_validation")
...
class ValidationResult:
    ...
    @classmethod
    def from_claw(cls, claw_result: CLAWResult, action: str) -> "ValidationResult":
        """Create ValidationResult from CLAWResult."""
        concerns = []
        if not claw_result.truth_passes:
            concerns.append("Failed Credibility gate: potential deception")
        if not claw_result.harm_passes:
            concerns.append("Failed Avoidance gate: could cause harm")
        if not claw_result.scope_passes:
            concerns.append("Failed Limits gate: exceeds boundaries")
        if not claw_result.purpose_passes:
            concerns.append("Failed Worth gate: no legitimate purpose")

        risk_str = (
            claw_result.risk_level.value
            if isinstance(claw_result.risk_level, RiskLevel)
            else claw_result.risk_level
        )

        return cls(
            safe=claw_result.is_safe,
            action=str(action)[:100] if action else "unknown",
            concerns=concerns,
            risk_level=risk_str,
            should_proceed=claw_result.is_safe,
            reasoning=claw_result.reasoning,
            gate_results=claw_result.gate_results,
        )
...
class SafetyValidator(ClawIntegration):
    """
    Core safety validation component using semantic LLM analysis.

    Uses CLAW Protocol (Credibility, Limits, Avoidance, Worth) with real LLM
    semantic analysis - not regex pattern matching.

    Inherits from ClawIntegration for standardized validation via
    LayeredValidator.
    ...
    name = "GuardianClawSafetyValidator"
    description = "AI safety validation using semantic CLAW analysis"
    ...
    def __init__(
        ...
        # GuardianClaw for seed retrieval
        self._claw = GuardianClaw(seed_level=seed_level)
    ...
    def validate_action(
        ...
        Args:
            ...
            worth: Optional stated purpose for the action
        ...
        try:
            ...
            if worth:
                self._validate_text_size(worth, "worth")

            # Combine action and worth for validation
            content = f"{action} {worth}".strip() if worth else action
        ...
        if self.log_checks and not result.should_proceed:
            logger.warning(f"[GUARDIANCLAW] Action blocked: {result.reasoning}")
    ...
    def validate_thought(self, thought: str) -> ValidationResult:
        ...
        try:
            ...
            logger.error(f"[GUARDIANCLAW] Thought validation error: {e}")
            if self.fail_closed:
        ...
        if self.log_checks and not result.should_proceed:
            logger.warning(f"[GUARDIANCLAW] Thought blocked: {result.reasoning}")
    ...
    def validate_output(self, output: str) -> ValidationResult:
        ...
        try:
            ...
            logger.error(f"[GUARDIANCLAW] Output validation error: {e}")
            if self.fail_closed:
        ...
        # Log warning if enabled and blocked
        if self.log_checks and not result.should_proceed:
            logger.warning(f"[GUARDIANCLAW] Output blocked: {result.reasoning}")

        return result

    def get_seed(self) -> str:
        """
        Get GuardianClaw seed for injection into agent system prompt.

        Returns:
            Seed content string
        """
        return self._claw.get_seed()
...
class AsyncSafetyValidator(AsyncClawIntegration):
    """
    Async version of SafetyValidator for use with async frameworks.

    Inherits from AsyncClawIntegration for standardized async validation.
    ...
    def __init__(
        ...
        # Initialize AsyncClawIntegration
        super().__init__(validator=validator)
    ...
    async def validate_action(
        ...
        try:
            ...
            # Combine action and worth for validation (matches sync behavior)
            content = f"{action} {worth}".strip() if worth else action

            # Use inherited async validate method from AsyncClawIntegration
            ...
            logger.error(f"[GUARDIANCLAW] Async validation error: {e}")
            if self.fail_closed:
                result = ValidationResult.error_result(action, e)
            else:
                result = ValidationResult(
    ...
    async def validate_thought(self, thought: str) -> ValidationResult:
        ...
        try:
            ...
            logger.error(f"[GUARDIANCLAW] Async thought validation error: {e}")
            if self.fail_closed:
                result = ValidationResult.error_result(f"thought: {thought[:50]}...", e)
            else:
                result = ValidationResult(
    ...
    async def validate_output(self, output: str) -> ValidationResult:
        ...
        try:
            ...
            logger.error(f"[GUARDIANCLAW] Async output validation error: {e}")
            if self.fail_closed:
                result = ValidationResult.error_result(f"output: {output[:50]}...", e)
            else:
                result = ValidationResult(
    ...
    def get_seed(self) -> str:
        """Get GuardianClaw seed for injection."""
        return self._claw.get_seed()
...
GuardianClawSafetyComponent = SafetyValidator
GuardianClawGuard = ExecutionGuard
