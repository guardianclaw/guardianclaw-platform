...
|--------|--------|------------|--------------|
| `GuardianClawCLAWScorer` | Semantic analysis | ~85% | Yes |
| `GuardianClawHeuristicScorer` | Pattern matching | ~45% | No |
| `GuardianClawGateScorer` | Single gate test | ~85% | Yes |

### Confidence Values

Confidence values are based on empirical testing against HarmBench and JailbreakBench:

- **Semantic (85%)**: LLM-based CLAW analysis compared to human evaluation
...
async def main():
    ...
    scorer = GuardianClawCLAWScorer(
        ...
        provider="openai",
        model="gpt-4o-mini",
        categories=["claw_claw"],  # optional: custom score categories
    )
...
### Semantic Scorer (GuardianClawCLAWScorer)

```python
{
    "score_value": "true" | "false",  # true = unsafe
    "score_type": "true_false",
    "score_category": ["claw_claw", "avoidance", ...],  # categories + failed gates
...
### Heuristic Scorer (GuardianClawHeuristicScorer)

```python
{
    "score_value": "true" | "false",
    "score_type": "true_false",
    "score_category": ["claw_claw_heuristic", ...],
...
### Gate Scorer (GuardianClawGateScorer)

```python
{
    "score_value": "true" | "false",
    "score_type": "true_false",
    "score_category": ["claw_harm"],  # claw_{gate}
...
|--------|------------|-------------|
| Semantic | 0.85 | LLM-based CLAW analysis |
...
```python
heuristic_scorer = GuardianClawHeuristicScorer()
semantic_scorer = GuardianClawCLAWScorer(api_key="...")
...
for model in models:
    ...
    scorer = GuardianClawCLAWScorer(api_key=key, provider=provider, model=model)
...
```bash
python -m guardianclaw.integrations.pyrit.example
```
...
```python
scorer = GuardianClawCLAWScorer(api_key="sk-...")
```
...
- [GuardianClaw Website](https://guardianclaw.org)
...
Made by [GuardianClaw Team](https://guardianclaw.org)
