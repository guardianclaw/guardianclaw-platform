"""
Solana Agent Kit integration for GuardianClaw AI.
...
2. Action wrappers (for Python-based validation flows)
...
Usage patterns:
    ...
    # Pattern 2: Validation in custom actions
...
logger = logging.getLogger("guardianclaw.solana_agent_kit")
...
#
# Crypto-specific patterns only. CLAW jailbreak detection is handled by
# CLAWValidator from guardianclaw.validators.gates (the source of credibility).
# Do not duplicate jailbreak patterns here - use validate_message() for full CLAW check.
DEFAULT_SUSPICIOUS_PATTERNS: List[SuspiciousPattern] = [
    ...
    # which calls CLAWValidator from guardianclaw.validators.gates
]
...
class ClawValidator(ClawIntegration):
    ...
    # Default actions that require explicit worth
    ...
    def __init__(
        ...
        Args:
            ...
            require_purpose_for: Actions that require explicit worth/reason
        ...
        # Keep GuardianClaw instance for get_seed() backwards compatibility
        self.claw = GuardianClaw(seed_level=seed_level)
        ...
        if memory_integrity_check:
            try:
                ...
                warnings.warn(
                    "memory_integrity_check=True but guardianclaw.memory module not available. "
        ...
        if fiduciary_enabled and HAS_FIDUCIARY:
            ...
            warnings.warn(
                "fiduciary_enabled=True but guardianclaw.fiduciary module not available. "
    ...
    def check(
        ...
        **kwargs
        ...
        Args:
            ...
            worth: Explicit worth/reason for the transaction
        ...
        # WORTH GATE: Check if action requires explicit worth
        requires_purpose = any(
            keyword.lower() in action.lower()
            for keyword in self.require_purpose_for
        )
        effective_purpose = worth or kwargs.get("reason", "")
        if requires_purpose and not effective_purpose:
            concerns.append(
                f"Action '{action}' requires explicit worth/reason "
                f"(set worth= or reason= parameter)"
            )
            if risk_level < TransactionRisk.MEDIUM:
                risk_level = TransactionRisk.MEDIUM
            logger.debug(f"Missing worth for action: {action}")
            ...
            if len(trimmed_purpose) < 20:
                concerns.append(
                    "Worth explanation is too brief - provide at least 20 characters"
                )
                if risk_level < TransactionRisk.MEDIUM:
                    risk_level = TransactionRisk.MEDIUM
            ...
            if len(words) < 3:
                concerns.append(
                    "Worth should contain at least 3 words to be meaningful"
                )
                if risk_level < TransactionRisk.MEDIUM:
                    risk_level = TransactionRisk.MEDIUM
            ...
            if len(no_spaces) >= 10 and re.match(r'^(.)\1{9,}$', no_spaces):
                concerns.append(
                    "Worth appears to be meaningless repeated characters"
                )
                if risk_level < TransactionRisk.MEDIUM:
                    risk_level = TransactionRisk.MEDIUM
        ...
        if not validation_result.is_safe:
            claw_concerns = validation_result.violations or []
            concerns.extend(claw_concerns)
            if risk_level < TransactionRisk.HIGH:
                risk_level = TransactionRisk.HIGH
            logger.info(f"GuardianClaw validation failed: {claw_concerns}")
        ...
        if requires_purpose and not worth:
            recommendations.append(f"Provide worth= for {action} actions")
    ...
    def _describe_action(
        ...
        """Create description for GuardianClaw validation (includes memo for attack detection)."""
    ...
    def _check_patterns(
        ...
        text_to_check = " ".join(filter(None, [action, memo, worth]))
...
def create_claw_actions(
    validator: Optional[ClawValidator] = None
) -> Dict[str, Callable]:
    """
    Create GuardianClaw validation actions.
    ...
    def get_safety_seed() -> str:
        """Get GuardianClaw seed for agent system prompt."""
        return validator.claw.get_seed()
...
class TransactionBlockedError(Exception):
    """Raised when a transaction is blocked by GuardianClaw validation."""
    pass


class GuardianClawSafetyMiddleware:
    """
    ...
    def __init__(self, validator: Optional[ClawValidator] = None):
        self.validator = validator or ClawValidator()
