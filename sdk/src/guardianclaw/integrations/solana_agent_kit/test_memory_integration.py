"""
Tests for Memory Integration in Solana Agent Kit ClawValidator.

These tests verify that the ClawValidator correctly uses the core
MemoryIntegrityChecker with content validation (v2.0) support.

Run with: python -m pytest src/guardianclaw/integrations/solana_agent_kit/test_memory_integration.py -v
"""
...
class TestClawValidatorMemoryConfig:
    """Tests for ClawValidator memory configuration."""
    ...
    def test_can_disable_content_validation(self):
        ...
        validator = ClawValidator(memory_content_validation=False)
...
class TestClawValidatorMemoryStats:
    """Tests for ClawValidator memory statistics."""
...
@pytest.mark.skipif(not HAS_MEMORY, reason="Memory module not available")
class TestClawValidatorContentValidation:
    ...
    def test_transaction_recorded_with_content_validation(self):
        ...
        result = validator.check(
            ...
            worth="Test transfer for unit testing",
        )
    ...
    def test_verify_transaction_history_passes(self):
        ...
        validator.check(
            action="transfer",
            amount=1.0,
            worth="Test transfer one",
        )
        validator.check(
            action="swap",
            amount=5.0,
            worth="Test swap operation",
        )
    ...
    def test_content_validation_with_benign_transactions(self):
        ...
        result = validator.check(
            ...
            amount=5.0,
            recipient="7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU",
            worth="Payment for services rendered",
        )
...
@pytest.mark.skipif(not HAS_MEMORY, reason="Memory module not available")
class TestClawValidatorContentValidationDisabled:
    ...
    def test_transactions_still_recorded_when_disabled(self):
        ...
        validator.check(
            action="transfer",
            amount=1.0,
            worth="Test with validation disabled",
        )
