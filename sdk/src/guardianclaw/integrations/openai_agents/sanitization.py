...
def generate_boundary_token(text: str) -> str:
    ...
    return f"GCLAW_BOUNDARY_{content_hash}"
...
def sanitize_for_validation(text: str, max_length: int = 32000) -> Tuple[str, dict]:
    ...
    if metadata["is_empty"]:
        metadata["boundary_token"] = "GCLAW_BOUNDARY_EMPTY"
        return "[GCLAW_BOUNDARY_EMPTY_START]\n\n[GCLAW_BOUNDARY_EMPTY_END]", metadata
...
Evaluate this content against all four CLAW gates (Credibility, Limits, Avoidance, Worth).
...
Does it stay within appropriate limits? Does it serve a legitimate purpose?"""
...
This should be considered when evaluating the Limits gate (manipulation attempts)."""
