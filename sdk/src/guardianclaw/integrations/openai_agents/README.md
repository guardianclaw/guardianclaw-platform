...
Semantic LLM-based guardrails for the OpenAI Agents SDK implementing CLAW (Credibility, Limits, Avoidance, Worth) validation with prompt injection protection.
...
flowchart TB
    ...
    B --> C["Input Guardrail (LLM)<br/>Semantic analysis, CLAW gates,<br/>blocks on injection"]
    C -->|blocked if unsafe| D["Main Agent<br/>GuardianClaw seed in instructions"]
...
Create an agent with full GuardianClaw protection.
...
create_claw_agent(
    ...
    guardrail_config: GuardianClawGuardrailConfig = None,
...
GuardianClawGuardrailConfig(
    ...
    require_all_gates: bool = True,         # All CLAW gates must pass
...
```python
import logging
from guardianclaw.integrations.openai_agents import set_logger

# Use your own logger
my_logger = logging.getLogger("my_app.claw")
...
```python
from guardianclaw.integrations.openai_agents import get_violations_log
...
**Key Insight:** The Purpose gate is unique to CLAW. Many requests that pass harm checks still fail the purpose test. "Drop all the plates" causes no direct harm but serves no purpose.
...
```python
code_agent = create_claw_agent(
    name="Code Helper",
    instructions="You help with coding",
)

math_agent = create_claw_agent(
    name="Math Helper",
    instructions="You help with math",
)

triage = create_claw_agent(
    name="Triage",
...
├── config.py         # GuardianClawGuardrailConfig, constants
├── models.py         # CLAWValidationOutput, ViolationsLog
