...
Provides layered input and output guardrails using CLAW validation:
...
from .config import GuardianClawGuardrailConfig, CLAW_GUARDRAIL_INSTRUCTIONS
from .models import (
...
def _validate_result(validation: Any, output_type: type) -> "CLAWValidationOutput":
    """
    ...
    Returns:
        Valid CLAWValidationOutput instance
...
def _create_guardrail_agent(config: GuardianClawGuardrailConfig) -> "Agent":
    """
    Create the internal guardrail agent for CLAW validation.
    ...
    # Ensure CLAWValidationOutput is available (requires Pydantic)
    output_type = require_claw_validation_output()

    return Agent(
        name="GuardianClaw CLAW Validator",
        instructions=CLAW_GUARDRAIL_INSTRUCTIONS,
...
def _determine_tripwire(
    validation: "CLAWValidationOutput",
    ...
    - If require_all_gates is False, trigger only if avoidance gate fails
    ...
    if config.require_all_gates:
        ...
        # Only avoidance gate is required
...
def claw_input_guardrail(
    config: Optional[GuardianClawGuardrailConfig] = None,
    name: str = "claw_claw_input",
    run_in_parallel: bool = False,
) -> "InputGuardrail":
    """
    Create a GuardianClaw input guardrail with semantic LLM validation.

    Uses a dedicated guardrail agent to perform CLAW validation on user input.
    ...
    Example:
        ...
        from guardianclaw.integrations.openai_agents import claw_input_guardrail
    ...
    config = config or GuardianClawGuardrailConfig()
    ...
    async def guardrail_function(
        ...
        """Layered CLAW input validation with heuristic + semantic layers."""
        ...
        if heuristic_validator is not None:
            ...
            if not heuristic_result.is_safe:
                ...
                # Map heuristic violations to CLAW gates
                violated_gate = "limits"  # Default for jailbreak/injection patterns
                if any("avoidance" in v.lower() for v in heuristic_result.violations):
                    violated_gate = "avoidance"
                elif any("credibility" in v.lower() or "decept" in v.lower() for v in heuristic_result.violations):
                ...
                if config.skip_semantic_if_heuristic_blocks:
                    return GuardrailFunctionOutput(
                        output_info={
                            "is_safe": False,
                            "gates": {
                                "credibility": violated_gate != "credibility",
                                "avoidance": violated_gate != "avoidance",
                                "limits": violated_gate != "limits",
                                "worth": True,
                            },
        ...
        try:
            ...
            return GuardrailFunctionOutput(
                output_info={
                    ...
                    "gates": {
                        "credibility": validation.truth_passes,
                        "avoidance": validation.harm_passes,
                        "limits": validation.scope_passes,
                        "worth": validation.purpose_passes,
                    },
...
def claw_output_guardrail(
    config: Optional[GuardianClawGuardrailConfig] = None,
    name: str = "claw_claw_output",
) -> "OutputGuardrail":
    """
    Create a GuardianClaw output guardrail with semantic LLM validation.

    Uses a dedicated guardrail agent to perform CLAW validation on agent output.
    ...
    Example:
        from agents import Agent
        from guardianclaw.integrations.openai_agents import claw_output_guardrail
    ...
    async def guardrail_function(
        ...
        """Layered CLAW output validation with heuristic + semantic layers."""
        start_time = time.time()
        ...
        if heuristic_validator is not None:
            ...
            if not heuristic_result.is_safe:
                ...
                # Map heuristic violations to CLAW gates
                violated_gate = "avoidance"  # Default for output violations
                if any("credibility" in v.lower() or "decept" in v.lower() for v in heuristic_result.violations):
                    violated_gate = "credibility"
                elif any("limits" in v.lower() for v in heuristic_result.violations):
                    violated_gate = "limits"
...
def create_claw_guardrails(
    config: Optional[GuardianClawGuardrailConfig] = None,
    input_parallel: bool = False,
) -> tuple:
    """
    Create a pair of GuardianClaw guardrails for use with existing agents.
    ...
    output_guard = claw_output_guardrail(config=config)
