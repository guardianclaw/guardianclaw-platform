"""
Examples for OpenAI Agents SDK integration with GuardianClaw.

These examples demonstrate semantic CLAW validation using LLM guardrail agents.
...
async def example_basic_agent():
    """
    Example 1: Basic GuardianClaw Agent with Semantic Validation

    Creates an agent with:
    - GuardianClaw seed in instructions
    - LLM-based input guardrail (CLAW validation)
    - LLM-based output guardrail (CLAW validation)
    - Prompt injection protection
    """
    print("\n" + "=" * 60)
    print("Example 1: Basic GuardianClaw Agent (Semantic Validation)")
    print("=" * 60)
    ...
    # Safe query - should pass all CLAW gates
    print("\nTesting safe query...")
...
async def example_guardrail_blocking():
    ...
    requests and blocks those that fail CLAW gates.
    """
    print("\n" + "=" * 60)
    ...
    try:
        ...
        print("Request blocked by semantic CLAW validation!")
...
async def example_worth_gate():
    """
    Example 4: Worth Gate in Action

    Demonstrates the Worth gate (P in CLAW) - blocking requests
    that may not cause direct harm but serve no legitimate purpose.
    """
    print("\n" + "=" * 60)
    print("Example 4: Worth Gate (Teleological Validation)")
    print("=" * 60)

    agent = create_claw_agent(
        name="Worth-Aware Agent",
        instructions="You help users with meaningful tasks.",
        model="gpt-4o-mini",
        guardrail_config=GuardianClawGuardrailConfig(
            guardrail_model="gpt-4o-mini",
            require_all_gates=True,  # All gates including Worth must pass
            log_violations=True,
        ),
    )
    ...
    try:
        ...
        print("Request blocked - failed Worth gate!")
...
async def example_add_guardrails_to_existing():
    ...
    You can add GuardianClaw's LLM-based guardrails to any existing agent.
    """
    print("\n" + "=" * 60)
...
async def main():
    ...
    print("=" * 60)
    print("OpenAI Agents SDK + GuardianClaw Integration Examples")
    print("=" * 60)
    print("\nFeatures demonstrated:")
    print("- LLM-based CLAW semantic validation")
