...
class PydanticNotAvailableError(ImportError):
    ...
    def __init__(self):
        super().__init__(
            "Pydantic is required for CLAWValidationOutput but is not installed. "
            "This usually means openai-agents is not installed. "
            "Install with: pip install openai-agents"
        )


def _create_claw_validation_output():
    """
    Create the Pydantic model for CLAW validation output.
    ...
    class CLAWValidationOutput(BaseModel):
        """
        Output model for CLAW semantic validation.

        This structured output is returned by the guardrail agent
        after analyzing content against the four CLAW gates.
        """

        is_safe: bool = Field(
            description="Whether the content passes all required CLAW gates"
        )

        truth_passes: bool = Field(
            description="Whether content passes the Credibility gate (no deception/misinformation)"
        )

        harm_passes: bool = Field(
            description="Whether content passes the Avoidance gate (no dangerous content)"
        )

        scope_passes: bool = Field(
            description="Whether content passes the Limits gate (appropriate boundaries)"
        )

        purpose_passes: bool = Field(
            description="Whether content passes the Worth gate (legitimate benefit)"
        )

        violated_gate: Optional[str] = Field(
            default=None,
            description="Which gate was violated first, if any (credibility/avoidance/limits/worth)"
        )
    ...
    return CLAWValidationOutput


# Create the model class at module level
CLAWValidationOutput = _create_claw_validation_output()


def require_claw_validation_output():
    """
    Get CLAWValidationOutput class, raising if not available.

    Raises:
        PydanticNotAvailableError: If Pydantic/CLAWValidationOutput is not available
    """
    if CLAWValidationOutput is None:
