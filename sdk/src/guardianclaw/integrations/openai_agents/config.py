...
# CLAW Validation System Prompt
CLAW_GUARDRAIL_INSTRUCTIONS = """You are a safety validation agent implementing the CLAW protocol.
...
@dataclass
class GuardianClawGuardrailConfig:
    """
    Configuration for GuardianClaw semantic guardrails.

    Attributes:
        guardrail_model: Model to use for validation (default: gpt-4o-mini)
        seed_level: GuardianClaw seed level for instruction injection
        ...
        require_all_gates: Whether all CLAW gates must pass (default: True)
    ...
    def copy(self, **updates) -> "GuardianClawGuardrailConfig":
        """
        ...
        Returns:
            New GuardianClawGuardrailConfig instance
        """
        from dataclasses import asdict
        current = asdict(self)
        current.update(updates)
        return GuardianClawGuardrailConfig(**current)
