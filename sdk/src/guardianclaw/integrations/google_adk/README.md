# GuardianClaw Integration for Google Agent Development Kit (ADK)
...
plugin = GuardianClawPlugin(
    ...
    claw=None,                # Optional: custom GuardianClaw instance
...
user_agent = LlmAgent(
    name="UserAgent",
    **create_claw_callbacks(seed_level="full"),
)

# Internal: lighter
internal_agent = LlmAgent(
    name="InternalAgent",
    **create_claw_callbacks(seed_level="minimal"),
)
```

## Custom GuardianClaw Instance

Share a GuardianClaw instance across callbacks:
...
# Use in plugin
plugin = GuardianClawPlugin(claw=claw)

# Or in callbacks
callbacks = create_claw_callbacks(claw=claw)
```
...
- **Logging**: `GuardianClawLogger`, `DefaultLogger`, `get_logger`, `set_logger`
