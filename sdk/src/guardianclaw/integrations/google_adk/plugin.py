"""GuardianClaw Plugin for Google ADK.

This module provides a global plugin that integrates GuardianClaw's CLAW
...
class GuardianClawPlugin(_BASE_CLASS, ClawIntegration):
    """Global GuardianClaw safety plugin for Google ADK.

    This plugin integrates GuardianClaw's CLAW (Credibility, Limits, Avoidance, Worth)
    ...
    Attributes:
        name: Plugin identifier ("claw").
        claw: The GuardianClaw instance used for validation.
    ...
    def __init__(
        ...
        blocked_message: str = "Request blocked by GuardianClaw safety validation.",
        validator: Optional[LayeredValidator] = None,
    ) -> None:
        """Initialize the GuardianClaw plugin.

        Args:
            claw: Optional GuardianClaw instance. If not provided, a new
            ...
            validation_timeout: Maximum time in seconds for CLAW validation.
            ...
            blocked_message: Message returned when content is blocked.
                Defaults to "Request blocked by GuardianClaw safety validation."
        ...
        if ADK_AVAILABLE and _BASE_CLASS is not object:
            _BASE_CLASS.__init__(self, name="claw")
        ClawIntegration.__init__(self, validator=validator)

        # Initialize GuardianClaw (for seed access)
        if claw is not None:
            self._claw = claw
        else:
            from guardianclaw import GuardianClaw
        ...
        if not fail_closed:
            log_fail_open_warning("GuardianClawPlugin")

        _logger.debug(
            "GuardianClawPlugin initialized: seed_level=%s, block=%s, fail_closed=%s",
            seed_level,
            block_on_failure,
            fail_closed,
        )
    ...
    @property
    def claw(self) -> GuardianClaw:
        """The GuardianClaw instance used for validation."""
        return self._claw
    ...
    async def after_model_callback(
        ...
        *,
        ...
        to the user. It validates the response text against CLAW.
        ...
        try:
            ...
            if self._block_on_failure:
                return create_blocked_response(
                    "Response blocked by GuardianClaw safety validation."
                )
        ...
        *,
        ...
        try:
            ...
            if self._block_on_failure:
                ...
                return {
                    "status": "blocked",
                    "error": f"Tool '{tool_name}' blocked by GuardianClaw safety validation.",
        ...
        *,
        ...
        try:
            ...
            if self._block_on_failure:
                return {
                    "status": "blocked",
                    "error": "Tool result blocked by GuardianClaw safety validation.",
    ...
    async def close(self) -> None:
        ...
        """
        _logger.debug("GuardianClawPlugin closing")
    ...
    async def _validate_content_async(
        ...
        # CLAW validation using inherited validate() method
        try:
        ...
        for concern in concerns:
            ...
            if "credibility" in concern_lower or "factual" in concern_lower:
                gate_failures["credibility"] = True
            if "avoidance" in concern_lower or "dangerous" in concern_lower or "violence" in concern_lower:
                gate_failures["avoidance"] = True
            if "limits" in concern_lower or "instruction" in concern_lower or "override" in concern_lower:
                gate_failures["limits"] = True
            if "worth" in concern_lower or "justification" in concern_lower:
                gate_failures["worth"] = True
        ...
        return {
            "reason": f"CLAW validation failed: {', '.join(str(c)[:50] for c in concerns[:3])}",
            "concerns": concerns,
            "risk_level": risk_level,
...
def create_claw_plugin(
    seed_level: str = DEFAULT_SEED_LEVEL,
    block_on_failure: bool = True,
    fail_closed: bool = False,
    **kwargs: Any,
) -> GuardianClawPlugin:
    """Factory function to create a GuardianClawPlugin.

    This is a convenience function that creates a properly configured
    GuardianClawPlugin instance.
    ...
    Args:
        ...
        **kwargs: Additional arguments passed to GuardianClawPlugin.

    Returns:
        Configured GuardianClawPlugin instance.

    Example:
        plugin = create_claw_plugin(seed_level="full", fail_closed=True)
        session_service = InMemorySessionService()
        runner = Runner(
            app_name="my_app",
            agent=my_agent,
