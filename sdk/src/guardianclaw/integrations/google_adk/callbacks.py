...
on ADK agents for CLAW validation. Unlike the GuardianClawPlugin which
...
def _get_claw(claw: Optional[GuardianClaw], seed_level: str) -> GuardianClaw:
    """Get or create a GuardianClaw instance.

    Args:
        claw: Optional existing GuardianClaw instance.
        seed_level: Seed level to use if creating new instance.

    Returns:
        GuardianClaw instance.
    """
    if claw is not None:
        return claw

    from guardianclaw import GuardianClaw
    return GuardianClaw(seed_level=seed_level)
...
def create_before_model_callback(
    ...
    # Initialize GuardianClaw once
    _claw = _get_claw(claw, seed_level)
    ...
    def before_model_callback(
        ...
        try:
            ...
            result = _validate_content_sync(
                _claw, content, max_text_size, validation_timeout, fail_closed
            )
...
def create_after_model_callback(
    ...
    validation_timeout: float = DEFAULT_VALIDATION_TIMEOUT,
    fail_closed: bool = False,
    blocked_message: str = "Response blocked by GuardianClaw safety validation.",
) -> AfterModelCallback:
    ...
    Args:
        claw: Optional GuardianClaw instance.
        seed_level: Safety level (minimal, standard, full).
    ...
    def before_tool_callback(
        ...
        try:
            ...
            if block_on_failure:
                return {
                    "status": "blocked",
                    "error": "Tool arguments blocked by GuardianClaw validation.",
    ...
    def after_tool_callback(
        ...
        try:
            ...
            if block_on_failure:
                return {
                    "status": "blocked",
                    "error": "Tool result blocked by GuardianClaw validation.",
...
def create_claw_callbacks(
    ...
    **kwargs: Any,
    ...
    """Create a complete set of GuardianClaw callbacks for an agent.
    ...
    Args:
        claw: Optional GuardianClaw instance (shared across callbacks).
        seed_level: Safety level (minimal, standard, full).
    ...
    # Create shared GuardianClaw instance
    _claw = _get_claw(claw, seed_level)
