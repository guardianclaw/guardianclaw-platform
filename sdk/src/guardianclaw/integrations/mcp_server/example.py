#!/usr/bin/env python3
"""
GuardianClaw MCP Server Integration Examples.

Demonstrates how to create and use MCP servers with GuardianClaw safety tools.
...
def example_create_server():
    """Example 1: Create a standalone GuardianClaw MCP server."""
    print("\n" + "=" * 60)
    ...
    print("\nTools available:")
    print("  - claw_validate: Validate text through CLAW gates")
    print("  - claw_check_action: Check if action is safe")
    print("  - claw_check_request: Validate user requests")
    print("  - claw_get_seed: Get seed content")
    print("  - claw_batch_validate: Batch validation")
    print("\nResources available:")
    print("  - claw://seed/{level}: Get seed by level")
    print("  - claw://config: Current configuration")
    print("\nTo run the server:")
    print("  mcp.run()")


def example_add_to_existing():
    """Example 2: Add GuardianClaw tools to an existing MCP server."""
    print("\n" + "=" * 60)
    print("Example 2: Add GuardianClaw Tools to Existing Server")
    print("=" * 60)
    ...
    try:
        ...
        # Add GuardianClaw safety tools
        add_claw_tools(mcp)

        # You can add your own tools too
        @mcp.tool()
        def my_custom_tool(text: str) -> str:
            """A custom tool alongside GuardianClaw."""
            return f"Custom processing: {text[:50]}..."

        print("\nGuardianClaw tools added to custom server!")
        print("Server now has both GuardianClaw and custom tools.")
...
def example_configuration():
    ...
    from guardianclaw.integrations.mcp_server import MCPConfig, __version__
...
def main():
    ...
    print("  python -m guardianclaw.integrations.mcp_server")
    print()
