"""
MCP Server integration for GuardianClaw AI.

Provides a Model Context Protocol server that exposes GuardianClaw safety
...
Usage:
    ...
    # Option 3: Add GuardianClaw tools to existing MCP server
...
Tools provided:
    - claw_validate: Validate any text through CLAW gates
    - claw_check_action: Check if an action is safe to execute
    - claw_check_request: Validate a user request for safety
    - claw_get_seed: Get the GuardianClaw seed for injection
    - claw_batch_validate: Validate multiple items in batch

Resources provided:
    - claw://seed/{level}: Get seed content by level
    - claw://config: Current GuardianClaw configuration
"""
...
logger = logging.getLogger("guardianclaw.mcp_server")
...
def create_claw_mcp_server(
    name: str = "claw-safety",
    claw: Optional[GuardianClaw] = None,
    seed_level: str = "standard",
) -> Any:
    """
    Create a GuardianClaw MCP server with safety validation tools.
    ...
    Returns:
        FastMCP server instance with GuardianClaw tools
    ...
    # Add GuardianClaw tools to the server
    add_claw_tools(mcp, claw)
...
def add_claw_tools(
    ...
    """
    Add GuardianClaw safety tools to an existing MCP server.

    This allows integration of GuardianClaw validation into any MCP server.

    Args:
        mcp: FastMCP server instance
        claw: GuardianClaw instance (creates default if None) - used for get_seed
    ...
    # Tool: Validate text through CLAW gates
    @mcp.tool()
    def claw_validate(
        text: str,
        ...
        """
        Validate text through GuardianClaw CLAW (Credibility, Limits, Avoidance, Worth) gates.
        ...
        try:
            ...
            logger.warning(f"claw_validate: {e}")
            return {
                "safe": False,
                "violations": [str(e)],
        ...
        logger.debug(f"claw_validate called: check_type={check_type}, text_len={len(text)}")
        ...
        logger.debug(f"claw_validate result: safe={result.is_safe}, violations={len(result.violations)}")
    ...
    def claw_check_action(action: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"claw_check_action: {e}")
            return {
                "safe": False,
        ...
        logger.debug(f"claw_check_action called: action_len={len(action)}")
        ...
        logger.debug(f"claw_check_action result: safe={result.is_safe}, risk={risk_level}")
        return {
            "safe": result.is_safe,
    ...
    @mcp.tool()
    def claw_check_request(request: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.warning(f"claw_check_request: {e}")
            return {
                "should_proceed": False,
                "risk_level": "critical",
                "concerns": [str(e)],
        ...
        logger.debug(f"claw_check_request called: request_len={len(request)}")
        ...
        logger.debug(f"claw_check_request result: proceed={result.is_safe}, risk={risk_level}")
        return {
            "should_proceed": result.is_safe,
            "risk_level": risk_level,
            "concerns": result.violations,
    ...
    # Tool: Get GuardianClaw seed
    @mcp.tool()
    def claw_get_seed(level: str = "standard") -> str:
        """
        Get the GuardianClaw safety seed for injection into system prompts.

        Use this to retrieve the CLAW safety guidelines that can be
        ...
        """
        logger.debug(f"claw_get_seed called: level={level}")

        # Validate level
        valid_levels = ("minimal", "standard", "full")
        if level not in valid_levels:
            logger.warning(f"claw_get_seed: invalid level '{level}', using 'standard'")
            level = "standard"

        temp_claw = GuardianClaw(seed_level=level)
        seed = temp_claw.get_seed()

        logger.debug(f"claw_get_seed result: seed_len={len(seed)}")
        return seed
    ...
    def claw_batch_validate(
        ...
        """
        logger.debug(f"claw_batch_validate called: items={len(items)}, check_type={check_type}")

        # Enforce size limits to prevent memory exhaustion
        max_items = min(max_items, MCPConfig.MAX_BATCH_ITEMS)
        if len(items) > max_items:
            logger.info(f"claw_batch_validate: truncating from {len(items)} to {max_items} items")
        ...
        logger.debug(
            f"claw_batch_validate result: total={len(items)}, "
    ...
    @mcp.resource("claw://seed/{level}")
    def get_seed_resource(level: str) -> str:
        """
        Get GuardianClaw seed content by level.

        Available levels: minimal, standard, full
        """
        try:
            temp_claw = GuardianClaw(seed_level=level)
            return temp_claw.get_seed()
        except ValueError:
            return f"Invalid seed level: {level}. Use 'minimal', 'standard', or 'full'."

    # Resource: Configuration
    @mcp.resource("claw://config")
    def get_config_resource() -> Dict[str, Any]:
        """Get current GuardianClaw configuration."""
        return {
            "version": __version__,
            "default_seed_level": claw.seed_level if hasattr(claw, 'seed_level') else "standard",
            "protocol": "CLAW",
...
class GuardianClawMCPClient:
    """
    Client for connecting to GuardianClaw MCP servers.
    ...
    Example (HTTP):
        ...
        async with GuardianClawMCPClient("http://localhost:8000/mcp") as client:
    ...
    Example (with timeout):
        async with GuardianClawMCPClient(url="http://localhost:8000/mcp", timeout=10.0) as client:
    ...
    def __init__(
        ...
        Args:
            ...
            args: Arguments for stdio command (e.g., ["-m", "guardianclaw.integrations.mcp_server"])
        ...
        logger.debug(f"GuardianClawMCPClient initialized: url={url}, command={command}, timeout={timeout}")
...
def run_server():
    """Run GuardianClaw MCP server as standalone process."""
    if not MCP_AVAILABLE:
    ...
    mcp = create_claw_mcp_server()
    print("Starting GuardianClaw MCP Server...")
    print("Tools available:")
    print("  - claw_validate")
    print("  - claw_check_action")
    print("  - claw_check_request")
    print("  - claw_get_seed")
    print("  - claw_batch_validate")
    mcp.run()
