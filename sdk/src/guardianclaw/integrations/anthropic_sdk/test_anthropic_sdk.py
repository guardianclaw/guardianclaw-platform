...
Run with: python -m pytest src/guardianclaw/integrations/anthropic_sdk/test_anthropic_sdk.py -v
"""
...
class TestCreateBlockedResponse:
    ...
    def test_blocked_response_structure(self):
        response = _create_blocked_response("Blocked message", gate="avoidance")
...
class TestBlockedStreamIterator:
    ...
    def test_iterator_yields_once(self):
        iterator = BlockedStreamIterator("Blocked", gate="avoidance")

        # First iteration should return blocked response
        result = next(iterator)
        assert result["claw_blocked"] is True
        assert result["claw_gate"] == "avoidance"
...
class TestAsyncBlockedStreamIterator:
    ...
    def test_async_iterator_yields_once(self):
        ...
        async def _test():
            iterator = AsyncBlockedStreamIterator("Blocked", gate="avoidance")
...
class TestGuardianClawMessagesInternal:
    """Tests for internal _GuardianClawMessages class using LayeredValidator."""

    @pytest.fixture
    def mock_claw(self):
        claw = Mock()
        claw.get_seed.return_value = "SEED CONTENT"
        return claw

    @pytest.fixture
    def mock_layered_validator_safe(self):
        """Mock LayeredValidator that returns safe ValidationResult."""
        from guardianclaw.validation import ValidationResult, ValidationLayer, RiskLevel
        validator = Mock()
    ...
    def mock_layered_validator_unsafe(self):
        ...
        validator.validate.return_value = ValidationResult(
            is_safe=False,
            violations=["Avoidance (violence): harmful content detected"],
            layer=ValidationLayer.HEURISTIC,
    ...
    def test_validate_content_size_check(self, mock_claw, mock_messages_api):
        ...
        messages = _GuardianClawMessages(
    ...
    def test_validate_content_heuristic_pass(
        self, mock_claw, mock_messages_api, mock_layered_validator_safe
    ):
        from guardianclaw.integrations.anthropic_sdk import _GuardianClawMessages
    ...
    def test_validate_content_heuristic_fail(
        self, mock_claw, mock_messages_api, mock_layered_validator_unsafe
    ):
        from guardianclaw.integrations.anthropic_sdk import _GuardianClawMessages
    ...
    def test_create_blocks_unsafe_output(
        self, mock_claw, mock_messages_api
    ):
        from guardianclaw.integrations.anthropic_sdk import _GuardianClawMessages
        from guardianclaw.validation import ValidationResult, ValidationLayer, RiskLevel
        ...
        validator.validate.side_effect = [
            ...
            ValidationResult(
                is_safe=False,
                violations=["Avoidance (output): harmful output"],
                layer=ValidationLayer.HEURISTIC,
                risk_level=RiskLevel.HIGH,
            ),
        ]
    ...
    def test_stream_returns_blocked_iterator_on_unsafe(
        ...
        item = next(result)
        assert item["claw_blocked"] is True
...
class TestFailClosedBehavior:
    ...
    def mock_claw(self):
        claw = Mock()
        claw.get_seed.return_value = "SEED"
        return claw
    ...
    def test_fail_open_on_semantic_error(self, mock_claw, mock_messages_api):
    ...
    def test_fail_closed_on_error(self, mock_claw, mock_messages_api):
...
class TestWithAnthropicSDK:
    ...
    def test_claw_anthropic_import(self):
        from guardianclaw.integrations.anthropic_sdk import GuardianClawAnthropic
        assert GuardianClawAnthropic is not None

    def test_claw_async_anthropic_import(self):
        from guardianclaw.integrations.anthropic_sdk import GuardianClawAsyncAnthropic
        assert GuardianClawAsyncAnthropic is not None
...
class TestModuleExports:
    ...
    def test_all_exports_available(self):
        from guardianclaw.integrations.anthropic_sdk import __all__
