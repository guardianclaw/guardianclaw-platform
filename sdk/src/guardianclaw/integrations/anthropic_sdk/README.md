...
| `GuardianClawAnthropicWrapper` | Wrap existing client instance |
...
### GuardianClawAnthropic / GuardianClawAsyncAnthropic

```python
GuardianClawAnthropic(
    api_key=None,                          # Defaults to ANTHROPIC_API_KEY env var
    claw=None,                          # Custom GuardianClaw instance
...
```python
{
    ...
    "claw_blocked": True,
    "claw_gate": "avoidance",  # Which CLAW gate failed
}
```
...
client = GuardianClawAnthropic(logger=MyLogger())
...
| Class | Description |
|-------|-------------|
| `GuardianClawAnthropic` | Sync client with CLAW safety |
| `GuardianClawAsyncAnthropic` | Async client with CLAW safety |
| `GuardianClawAnthropicWrapper` | Generic wrapper for existing clients |
| `GuardianClawLogger` | Protocol for custom loggers |
...
if response.get("claw_blocked"):
    print(f"Blocked: {response['claw_gate']}")
...
The `inject_seed()` function does not require the Anthropic SDK to be installed. It only needs the base `guardianclaw` package, making it useful for other integrations or custom implementations.
