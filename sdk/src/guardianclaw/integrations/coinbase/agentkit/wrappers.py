"""
Action Wrappers for GuardianClaw AgentKit Integration.

Provides decorators and utilities to wrap AgentKit actions with
GuardianClaw safety validation automatically.

This enables transparent safety validation without modifying
existing action code.

Example:
    from guardianclaw.integrations.coinbase.agentkit import safe_action
...
from ..config import ChainType, GuardianClawCoinbaseConfig, get_default_config
from ..validators.transaction import TransactionValidator

logger = logging.getLogger("guardianclaw.coinbase.wrappers")
...
class ActionBlockedError(Exception):
    """
    Raised when an action is blocked by GuardianClaw validation.
...
class GuardianClawActionWrapper:
    """
    Wrapper class for adding GuardianClaw validation to actions.

    Can be used to wrap individual functions or entire classes.

    Example:
        wrapper = GuardianClawActionWrapper()
    ...
    def wrap(
        ...
        """
        Wrap a function with GuardianClaw validation.
        ...
        def sync_wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if self.log_validations:
                ...
                logger.info(
                    f"GuardianClaw [{status}] {action_type}: "
...
_default_wrapper: Optional[GuardianClawActionWrapper] = None


def _get_default_wrapper() -> GuardianClawActionWrapper:
    """Get or create the default wrapper."""
    global _default_wrapper
    if _default_wrapper is None:
        _default_wrapper = GuardianClawActionWrapper()
...
def safe_action(
    ...
    """
    Decorator to wrap a function with GuardianClaw safety validation.
    ...
    def decorator(func: F) -> F:
        if config:
            wrapper = GuardianClawActionWrapper(config=config)
        else:
...
def create_safe_action_wrapper(
    ...
    Returns:
        Configured GuardianClawActionWrapper
    ...
    return GuardianClawActionWrapper(
        config=config,
