"""
GuardianClaw ActionProvider for Coinbase AgentKit.
...
using CLAW Protocol and EVM-specific checks.
...
Example:
    ...
    from guardianclaw.integrations.coinbase.agentkit import claw_action_provider
...
logger = logging.getLogger("guardianclaw.coinbase.agentkit")


# Build the base classes tuple dynamically
_PROVIDER_BASES = (_AgentKitActionProvider, ClawIntegration) if AGENTKIT_AVAILABLE else (ClawIntegration,)


class GuardianClawActionProvider(*_PROVIDER_BASES):
    """
    GuardianClaw security ActionProvider for Coinbase AgentKit.

    Inherits from ActionProvider (Coinbase) and ClawIntegration for
    standardized validation via LayeredValidator.

    Provides the following security actions:
    - claw_validate_transaction: Validate before any transfer
    - claw_validate_address: Validate address format and status
    - claw_check_action_safety: Check if any action is safe
    - claw_get_spending_summary: Get spending limits and usage
    - claw_assess_defi_risk: Assess DeFi operation risk
    - claw_configure_guardrails: Adjust security settings
    - claw_block_address: Block a malicious address
    - claw_unblock_address: Remove address from blocklist
    - claw_get_validation_history: Get validation audit log

    Example:
        provider = GuardianClawActionProvider()
    ...
    def __init__(
        self,
        config: Optional[GuardianClawCoinbaseConfig] = None,
        wallet_address: Optional[str] = None,
        validator: Optional[LayeredValidator] = None,
    ):
        """
        Initialize the GuardianClaw ActionProvider.
        ...
        ClawIntegration.__init__(self, validator=validator)
        ...
        logger.info(
            f"GuardianClawActionProvider initialized with profile: {self.config.security_profile.value}"
        )
    ...
    def supports_network(self, network: Network) -> bool:
        ...
        GuardianClaw supports all networks (validation is network-agnostic).
        """
        return True
    ...
    @create_action(
        name="claw_validate_transaction",
        description=(
    ...
    def validate_transaction(self, args: dict[str, Any]) -> str:
        ...
        try:
            ...
            result = self.transaction_validator.validate(
                ...
                worth=validated.worth,
            )

            # Also run CLAW validation using inherited validate() if worth provided
            claw_concerns = []
            if validated.worth:
                claw_result: ValidationResult = self.validate(validated.worth)
                if not claw_result.is_safe:
                    claw_concerns = claw_result.violations or []
            ...
            response = {
                ...
                "concerns": result.concerns + claw_concerns,
    ...
    def check_action_safety(self, args: dict[str, Any]) -> str:
        ...
        try:
            ...
            if is_high_risk:
                ...
                if self.config.require_purpose_for_transfers and not validated.worth:
                    concerns.append("High-risk actions require a stated purpose")

            # Run CLAW validation on purpose if provided using inherited validate()
            if validated.worth:
                claw_result: ValidationResult = self.validate(validated.worth)
                if not claw_result.is_safe:
                    concerns.extend(claw_result.violations or [])

            # Check args if provided
            if validated.action_args:
                args_str = json.dumps(validated.action_args)
                claw_result: ValidationResult = self.validate(args_str)
                if not claw_result.is_safe:
    ...
    @create_action(
        name="claw_get_spending_summary",
        description=(
    ...
    @create_action(
        name="claw_configure_guardrails",
        description=(
    ...
    def configure_guardrails(self, args: dict[str, Any]) -> str:
        ...
        try:
            ...
            if validated.require_purpose is not None:
                ...
                changes.append(f"Require worth: {validated.require_purpose}")
    ...
    @create_action(
        name="claw_get_validation_history",
...
def claw_action_provider(
    security_profile: str = "standard",
    wallet_address: Optional[str] = None,
    **kwargs: Any,
) -> GuardianClawActionProvider:
    """
    Create a GuardianClawActionProvider with the specified security profile.
    ...
    Returns:
        Configured GuardianClawActionProvider
    ...
    return GuardianClawActionProvider(
        config=config,
