...
- **AgentKit Guardrails**: ActionProvider with CLAW validation for all AgentKit actions
- **x402 Payment Validation**: CLAW-based validation for HTTP 402 payments
...
block-beta
    ...
    block:Framework["GUARDIANCLAW COINBASE PROTECTION FRAMEWORK"]
        columns 1
...
flowchart TB
    ...
    subgraph Pay["4. PAYMENT LAYER"]
        direction LR
        D1[CLAW Gates] --> D2[Tracking] --> D3[Profiles]
    end
...
How GuardianClaw validates AgentKit actions before execution:

```mermaid
flowchart TB
    LLM([AI Agent<br/>OpenAI / Anthropic / Google ADK])
    ...
    subgraph GuardianClaw["GuardianClaw ActionProvider"]
        direction TB
    ...
    Actions -->|Before Execute| GuardianClaw
    Validate --> Checks
    Check --> Checks
    DeFi --> Checks
    ...
    style GuardianClaw fill:#1a1a2e,stroke:#e94560,stroke-width:2px
...
How GuardianClaw intercepts and validates x402 payments before they reach the blockchain:

```mermaid
sequenceDiagram
    participant Client as AI Agent
    participant GuardianClaw as GuardianClaw Middleware
    ...
    Note over Client,GuardianClaw: GuardianClaw Intercepts Here

    Client->>GuardianClaw: 3. Payment Request

    rect rgb(50, 50, 80)
        Note over GuardianClaw: CLAW Validation Gates
        GuardianClaw->>GuardianClaw: TRUTH: Valid format?
        GuardianClaw->>GuardianClaw: HARM: Blocked address?
        GuardianClaw->>GuardianClaw: SCOPE: Within limits?
        GuardianClaw->>GuardianClaw: PURPOSE: Legitimate?
    end

    alt All Gates Pass
        GuardianClaw->>Server: 4. Forward with Payment Signature
        ...
        GuardianClaw-->>Client: BLOCKED: Reason
        Note over Client: Payment prevented
    end
```

### CLAW Gate Decision Flow (x402)
...
# - claw_get_spending_summary
# - claw_assess_defi_risk
# - claw_configure_guardrails
# - claw_block_address
# - claw_unblock_address
```
...
```python
from guardianclaw.integrations.coinbase import get_default_config
...
```python
from guardianclaw.integrations.coinbase import safe_action
...
```bash
python -m guardianclaw.integrations.coinbase.example
```
