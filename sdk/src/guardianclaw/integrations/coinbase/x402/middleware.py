"""GuardianClaw x402 payment validation middleware.

This module provides the main middleware class that integrates GuardianClaw
...
from .validators import CLAWPaymentValidator

logger = logging.getLogger(__name__)


class GuardianClawX402Middleware:
    """Main GuardianClaw middleware for x402 payment validation.

    This class provides comprehensive payment validation using the CLAW
    framework, with support for spending limits, rate limiting, and
    audit logging.

    Attributes:
        config: The middleware configuration
        validator: The CLAW validator instance
    ...
    Example:
        ...
        >>>
        >>> # With custom config
        >>> from guardianclaw.integrations.coinbase.x402.config import get_default_config
    ...
    def validate_payment(
        ...
        """Validate a payment request through CLAW gates.
    ...
    def _build_result(
        ...
        blocked_reason = None
        if decision == PaymentDecision.BLOCK:
            avoidance_result = gate_results.get(CLAWGate.AVOIDANCE)
            if avoidance_result and not avoidance_result.passed:
                blocked_reason = avoidance_result.reason
...
class PaymentBlockedError(Exception):
    """Raised when a payment is blocked by GuardianClaw."""
...
class PaymentRejectedError(Exception):
    """Raised when a payment is rejected by GuardianClaw."""
...
def create_claw_x402_middleware(
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        **kwargs: Additional arguments passed to GuardianClawX402Middleware

    Returns:
        Configured GuardianClawX402Middleware instance
    ...
    return GuardianClawX402Middleware(config=config, **kwargs)
