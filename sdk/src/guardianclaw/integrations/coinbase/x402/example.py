"""Example usage of GuardianClaw x402 payment validation.

This example demonstrates how to integrate GuardianClaw safety validation
...
def example_blocked_address() -> None:
    ...
    from guardianclaw.integrations.coinbase.x402 import CLAWGate
    avoidance_gate = result.gates.get(CLAWGate.AVOIDANCE)
    if avoidance_gate:
        print(f"\nAVOIDANCE Gate: {'PASS' if avoidance_gate.passed else 'FAIL'}")
        if avoidance_gate.reason:
            print(f"  Reason: {avoidance_gate.reason}")
...
def example_agentkit_provider() -> None:
    ...
    # Create provider
    provider = claw_x402_action_provider(security_profile="standard")
...
if __name__ == "__main__":
    print("=" * 60)
    print("GuardianClaw x402 Payment Validation Examples")
    print("=" * 60)
