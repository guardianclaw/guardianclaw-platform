"""GuardianClaw x402 Action Provider for Coinbase AgentKit.

This module provides an ActionProvider that adds GuardianClaw safety validation
to x402 payment flows in AgentKit-powered AI agents.

The provider wraps the standard x402 provider to add CLAW gates before
...
Example:
    ...
    >>> agent = AgentKit(
    ...     action_providers=[
    ...         claw_x402_action_provider(strict_mode=True),
    ...     ]
    ... )
"""
...
from .middleware import GuardianClawX402Middleware
from .schemas import (
...
class GuardianClawX402ActionProvider(*_PROVIDER_BASES):
    """GuardianClaw safety provider for x402 payments in AgentKit.

    Inherits from ActionProvider (Coinbase) and ClawIntegration for
    standardized validation via LayeredValidator.

    This provider adds CLAW validation gates to x402 payment flows,
    ensuring AI agents validate payments before execution.

    Actions:
        - claw_x402_validate_payment: Validate a payment before execution
        - claw_x402_get_spending_summary: Get spending statistics
        - claw_x402_configure_limits: Configure spending limits
        - claw_x402_check_endpoint: Check if endpoint is safe
        - claw_x402_safe_request: Make a safe x402 request
    ...
    def __init__(
        self,
        config: GuardianClawX402Config | None = None,
        ...
        if AGENTKIT_AVAILABLE and _AgentKitActionProvider is not None:
            _AgentKitActionProvider.__init__(self, "claw_x402", [])
        ClawIntegration.__init__(self, validator=validator)

        # Ensure name is always defined (consistency with GuardianClawActionProvider)
        self.name = "claw_x402"
        ...
        self.middleware = GuardianClawX402Middleware(config=self.config)
    ...
    @create_action(
        name="claw_x402_validate_payment",
        description="""Validate an x402 payment request before execution using CLAW gates.

This action should be called BEFORE any x402 payment to ensure it's safe.

Checks performed:
- CREDIBILITY: Payment request is well-formed and legitimate
- AVOIDANCE: Recipient/endpoint is not malicious
- LIMITS: Amount is within spending limits
- WORTH: Payment serves a legitimate purpose
...
Example: Before paying $5 to api.weather.io, validate with this action first.""",
    ...
    @create_action(
        name="claw_x402_get_spending_summary",
...
Use this to check how much spending capacity remains before making payments.""",
    ...
    @create_action(
        name="claw_x402_configure_limits",
...
def claw_x402_action_provider(
    config: GuardianClawX402Config | None = None,
    security_profile: Literal["permissive", "standard", "strict", "paranoid"] = "standard",
) -> GuardianClawX402ActionProvider:
    """Create a GuardianClaw x402 action provider for AgentKit.
    ...
    Returns:
        Configured GuardianClawX402ActionProvider
    ...
    Example:
        ...
        >>> provider = claw_x402_action_provider()
        >>>
    ...
    """
    return GuardianClawX402ActionProvider(
        config=config,
