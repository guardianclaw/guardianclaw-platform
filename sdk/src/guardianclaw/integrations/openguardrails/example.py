...
Demonstrates how to use GuardianClaw with OpenGuardrails for comprehensive
...
def example_openguardrails_as_backend():
    ...
    OpenGuardrails provides enterprise-grade detection while GuardianClaw
    adds CLAW protocol validation.
    """
...
def example_claw_as_scanner():
    """
    Example 2: Register GuardianClaw as OpenGuardrails custom scanner

    This integrates GuardianClaw's CLAW protocol directly into the
    OpenGuardrails pipeline.
    """
    print("\n=== Example 2: GuardianClaw as OpenGuardrails Scanner ===\n")

    # Register GuardianClaw scanner
    ...
    try:
        ...
        print(f"Registered GuardianClaw scanner as: {tag}")
        print("GuardianClaw CLAW validation is now available in OpenGuardrails!")
...
def example_combined_validation():
    """
    Example 3: Combined GuardianClaw + OpenGuardrails validation
...
def example_manual_combined():
    ...
    def validate_with_priority(content: str) -> dict:
        """
        Custom validation that prioritizes GuardianClaw for alignment
        and OpenGuardrails for enterprise concerns.
        """
        # GuardianClaw check first (alignment focus)
        claw_result = claw.validate(content)

        # If GuardianClaw blocks, don't even check OpenGuardrails
        if not claw_result.get("safe", True):
            return {
                "safe": False,
                "blocked_by": "claw",
                "reason": claw_result.get("concerns", []),
            }
...
def example_quick_setup():
    """
    Example 5: Quickest way to add OpenGuardrails to existing GuardianClaw setup
    """
    print("\n=== Example 5: Quick Setup ===\n")

    # One-liner to register GuardianClaw as scanner
    try:
        tag = register_claw_scanner(
            openguardrails_url="http://localhost:5000",
            jwt_token="your-token",
        )
        print(f"GuardianClaw registered as scanner {tag}")
