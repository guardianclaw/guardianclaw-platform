...
Run with: python -m pytest src/guardianclaw/integrations/openguardrails/test_openguardrails.py -v
"""
...
class TestGuardianClawOpenGuardrailsScannerInit:
    """Tests for GuardianClawOpenGuardrailsScanner.__init__ parameter validation"""

    def test_risk_level_invalid_string_raises(self):
        """C002: risk_level must be valid"""
        with pytest.raises(ValueError, match="Invalid risk_level"):
            GuardianClawOpenGuardrailsScanner(risk_level="invalid")
    ...
    def test_risk_level_int_raises(self):
        ...
        with pytest.raises(TypeError, match="RiskLevel enum or string"):
            GuardianClawOpenGuardrailsScanner(risk_level=123)

    def test_risk_level_valid_string_accepted(self):
        """C002: valid string should convert to RiskLevel"""
        scanner = GuardianClawOpenGuardrailsScanner(risk_level="high_risk")
        assert scanner.risk_level == RiskLevel.HIGH

    def test_risk_level_enum_accepted(self):
        scanner = GuardianClawOpenGuardrailsScanner(risk_level=RiskLevel.CRITICAL)
        assert scanner.risk_level == RiskLevel.CRITICAL

    def test_timeout_negative_raises(self):
        """M003: timeout must be positive"""
        with pytest.raises(ValueError, match="positive"):
            GuardianClawOpenGuardrailsScanner(timeout=-1)

    def test_timeout_zero_raises(self):
        with pytest.raises(ValueError, match="positive"):
            GuardianClawOpenGuardrailsScanner(timeout=0)

    def test_scan_prompt_string_raises(self):
        """M008: scan_prompt must be bool"""
        with pytest.raises(TypeError, match="bool"):
            GuardianClawOpenGuardrailsScanner(scan_prompt="yes")

    def test_scan_response_int_raises(self):
        """M008: scan_response must be bool"""
        with pytest.raises(TypeError, match="bool"):
            GuardianClawOpenGuardrailsScanner(scan_response=1)

    def test_jwt_token_int_raises(self):
        """M007: jwt_token must be string"""
        with pytest.raises(TypeError, match="string or None"):
            GuardianClawOpenGuardrailsScanner(jwt_token=123)


class TestGuardianClawGuardrailsWrapperValidation:
    """Tests for GuardianClawGuardrailsWrapper.validate() security fixes

    These tests use LayeredValidator mocks to test error handling,
    since GuardianClawGuardrailsWrapper uses LayeredValidator internally.
    """
    ...
    def test_validator_exception_returns_safe_false(self):
        ...
        wrapper = GuardianClawGuardrailsWrapper(validator=mock_validator)
        result = wrapper.validate("test content")

        assert result["safe"] is False
        assert "claw_error" in result["blocked_by"]
    ...
    def test_validator_returns_unsafe_returns_safe_false(self):
        ...
        assert result["safe"] is False
        assert "claw" in result["blocked_by"]
    ...
    def test_claw_passes_returns_safe_true(self):
        """Normal case: GuardianClaw passing returns safe=True"""
        class PassingGuardianClaw:
            def validate(self, content):
                return {"safe": True}

        wrapper = GuardianClawGuardrailsWrapper(claw=PassingGuardianClaw())
        result = wrapper.validate("test content")
...
class TestRequireBothLogic:
    ...
    def test_require_both_false_claw_fails_blocks(self):
        """require_both=False (default): if claw fails, block"""
        from unittest.mock import MagicMock
        from guardianclaw.validation import ValidationResult
        from guardianclaw.validation.types import ValidationLayer, RiskLevel
    ...
    def test_require_both_true_only_claw_fails_allows(self):
        """require_both=True (permissive): if only claw fails, allow"""
        from unittest.mock import MagicMock
        from guardianclaw.validation import ValidationResult
        from guardianclaw.validation.types import ValidationLayer, RiskLevel
        ...
        # No OpenGuardrails configured, so only claw runs
        wrapper = GuardianClawGuardrailsWrapper(
            validator=mock_validator,
            openguardrails=None,
        ...
        # Only claw failed, so with require_both=True should allow
        assert result["safe"] is True
        assert "claw" in result["blocked_by"]
    ...
    def test_require_both_true_only_openguardrails_fails_allows(self):
        ...
        assert result["safe"] is True
        assert "openguardrails" in result["blocked_by"]
        assert "claw" not in result["blocked_by"]
...
class TestScannerUrlValidation:
    ...
    def test_openguardrails_url_empty_raises(self):
        ...
        with pytest.raises(ValueError, match="non-empty string"):
            GuardianClawOpenGuardrailsScanner(openguardrails_url="")

    def test_openguardrails_url_none_raises(self):
        """None openguardrails_url should raise ValueError"""
        with pytest.raises(ValueError, match="non-empty string"):
            GuardianClawOpenGuardrailsScanner(openguardrails_url=None)
...
class TestWrapperTypeValidation:
    """Tests for GuardianClawGuardrailsWrapper parameter validation"""
    ...
    def test_openguardrails_wrong_type_raises(self):
        ...
        with pytest.raises(TypeError, match="callable 'validate' method"):
            GuardianClawGuardrailsWrapper(openguardrails="not a validator")

    def test_openguardrails_dict_raises(self):
        """openguardrails must have validate method"""
        with pytest.raises(TypeError, match="callable 'validate' method"):
            GuardianClawGuardrailsWrapper(openguardrails={"api_url": "http://test"})
    ...
    def test_require_both_string_raises(self):
        ...
        with pytest.raises(TypeError, match="require_both must be bool"):
            GuardianClawGuardrailsWrapper(require_both="yes")

    def test_require_both_int_raises(self):
        """require_both must be bool"""
        with pytest.raises(TypeError, match="require_both must be bool"):
            GuardianClawGuardrailsWrapper(require_both=1)


class TestRequireBothWithErrors:
    """Tests for require_both behavior with errors"""

    def test_require_both_true_with_claw_error_blocks(self):
        """require_both=True: claw error still blocks (fail-closed)"""
        from unittest.mock import MagicMock

        mock_validator = MagicMock()
        mock_validator.validate.side_effect = RuntimeError("GuardianClaw crashed!")
