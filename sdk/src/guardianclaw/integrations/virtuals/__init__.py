"""
GuardianClaw Safety Plugin for Virtuals Protocol GAME SDK

This integration provides safety guardrails for AI agents built with the GAME framework.
It implements the CLAW Protocol (Credibility, Limits, Avoidance, Worth) to validate agent
...
1. Function wrappers that add CLAW validation before execution
...
Usage:
    ...
    safety_worker = GuardianClawSafetyWorker.create_worker_config()

    # Option 2: Wrap individual functions
    safe_fn = create_claw_function(my_function, config)

    # Option 3: Wrap all functions in a worker's action space
    safe_action_space = wrap_functions_with_claw(action_space)
...
logger = logging.getLogger("guardianclaw.virtuals")

# Import CLAW types from validators (canonical source)
...
class GuardianClawValidationError(Exception):
    """Raised when an action fails GuardianClaw safety validation."""
...
# CLAWGate is imported from guardianclaw.validators.semantic (canonical source)
# Kept in __all__ for backwards compatibility


@dataclass
class ValidationResult:
    """Result of CLAW validation."""
    passed: bool
...
@dataclass
class ClawConfig:
    """Configuration for GuardianClaw safety validation."""
...
class ClawValidator(ClawIntegration):
    """
    Core validation engine implementing CLAW Protocol for Virtuals/GAME agents.

    Uses LayeredValidator for content validation (security patterns,
    jailbreak detection, etc.) via ClawIntegration inheritance,
    ...
    The four CLAW gates:
    - CREDIBILITY: Is the action based on accurate information?
    - AVOIDANCE: Could this action cause harm?
    - LIMITS: Is this action within appropriate limits?
    - WORTH: Does this action serve a legitimate benefit?
    ...
    def __init__(
        self,
        config: Optional[ClawConfig] = None,
        ...
        """
        Initialize ClawValidator with optional Fiduciary validation.

        Args:
            config: GuardianClaw configuration for CLAW gates.
    ...
    def validate(
        ...
        """
        Validate an action through all CLAW gates.

        First uses the global CLAWValidator for content validation (detects
        ...
        Args:
            ...
            context: Optional context (worker state, worth, etc.)
        ...
        # Validate action_name - empty or whitespace-only names fail CREDIBILITY gate
        ...
        if not layered_result.is_safe:
            # LayeredValidator failed - mark avoidance gate as failed
        ...
        # Gate 1: CREDIBILITY - Check for misleading action names
        truth_passed, truth_concerns = self._check_credibility_gate(
            action_name, action_args, context
        )
        if not truth_passed:
            gate_results["credibility"] = False
        ...
        # Gate 2: AVOIDANCE - Check blocked functions and crypto patterns
        harm_passed, harm_concerns = self._check_avoidance_gate(
            action_name, action_args, context
        )
        if not harm_passed:
            gate_results["avoidance"] = False
        ...
        # Gate 3: LIMITS - Check transaction limits and whitelists
        scope_passed, scope_concerns = self._check_limits_gate(
            action_name, action_args, context
        )
        if not scope_passed:
        ...
        # Gate 4: WORTH - Check for explicit worth on sensitive actions
        purpose_passed, purpose_concerns = self._check_worth_gate(
            action_name, action_args, context
        )
        if not purpose_passed:
            gate_results["worth"] = False
        ...
        if self._fiduciary is not None:
            ...
            # Include worth in the action string so FiduciaryValidator can analyze it
            amount = action_args.get("amount", action_args.get("value", 0))
            worth = context.get("worth", action_args.get("worth", ""))
            ...
            if worth:
                fid_action_parts.append(f"- {worth}")
            ...
            if not fid_result.compliant:
                ...
                if self._strict_fiduciary and fiduciary_blocked:
                    ...
                    gate_results["worth"] = False  # Associate with purpose gate
    ...
    def _build_content_string(
        ...
        """
        Build a content string for CLAWValidator from action data.

        Converts action name, arguments, and context into a single string
        that the CLAWValidator can analyze for security patterns.
        ...
        if context:
            ...
            relevant_keys = ["worth", "reason", "user_request", "message"]
    ...
    def _check_credibility_gate(
        self,
        action_name: str,
        action_args: Dict[str, Any],
        context: Dict[str, Any],
    ) -> Tuple[bool, List[str]]:
        """CREDIBILITY gate: Verify factual correspondence and detect deception."""
        concerns = []
        ...
        misleading_patterns = [
            (r"(?i)safe.*(?:drain|steal|hack)", "Action name misleading: contains 'safe' but suggests avoidance"),
    ...
    def _check_avoidance_gate(
        self,
        action_name: str,
        action_args: Dict[str, Any],
        context: Dict[str, Any],
    ) -> Tuple[bool, List[str]]:
        """AVOIDANCE gate: Assess potential for avoidance."""
        concerns = []
    ...
    def _check_worth_gate(
        self,
        action_name: str,
        action_args: Dict[str, Any],
        context: Dict[str, Any],
    ) -> Tuple[bool, List[str]]:
        """WORTH gate: Require teleological justification."""
        concerns = []

        # Check for worth/reason in context or args
        has_purpose = any([
            context.get("worth"),
            context.get("reason"),
            context.get("user_request"),
            action_args.get("worth"),
            action_args.get("reason"),
        ])

        # Check if action requires worth based on config
        ...
        if requires_purpose and not has_purpose:
            concerns.append(
                f"Action '{action_name}' requires explicit worth/reason "
...
def create_claw_function(
    original_function: "Function",
    config: Optional[ClawConfig] = None,
    validator: Optional[ClawValidator] = None,
) -> "Function":
    """
    Wrap a GAME SDK Function with GuardianClaw validation.

    This creates a new Function that validates through CLAW gates
    before executing the original function's executable.

    Args:
        original_function: The GAME SDK Function to wrap
        config: Optional GuardianClaw configuration
    ...
    Example:
        ...
        safe_transfer_fn = create_claw_function(transfer_fn)
    """
    ...
    val = validator or ClawValidator(cfg)
    ...
    def safe_executable(**kwargs) -> Tuple["FunctionResultStatus", str, dict]:
        """Wrapped executable with GuardianClaw validation."""
        ...
        if cfg.log_validations:
            ...
            logger.info(f"GuardianClaw [{status}] {fn_name}: gates={result.gate_results}")

        if not result.passed:
            if cfg.block_unsafe:
                return (
                    FunctionResultStatus.FAILED,
                    f"GuardianClaw blocked: {', '.join(result.concerns)}",
                    {"claw_blocked": True, "gate": result.blocked_gate},
                )
            else:
                logger.warning(f"GuardianClaw: {fn_name} would be blocked: {result.concerns}")
...
def wrap_functions_with_claw(
    functions: List["Function"],
    config: Optional[ClawConfig] = None,
) -> List["Function"]:
    """
    Wrap a list of Functions with GuardianClaw validation.
    ...
    cfg = config or ClawConfig()
    validator = ClawValidator(cfg)

    return [
        create_claw_function(fn, config=cfg, validator=validator)
        for fn in functions
    ]


class GuardianClawSafetyWorker:
    """
    ...
    def __init__(self, config: Optional[ClawConfig] = None):
        self.config = config or ClawConfig()
        self.validator = ClawValidator(self.config)
        ...
        if self.config.memory_integrity_check:
            if not MEMORY_INTEGRITY_AVAILABLE:
                logger.warning(
                    "Memory integrity requested but guardianclaw.memory module not available. "
                    "Make sure guardianclaw is installed correctly."
                )
            else:
    ...
    def check_action_safety(
        ...
        Args:
            ...
            worth: Stated worth for the action
        ...
        result = self.validator.validate(
            action_name=action_name,
            action_args=args,
            context={"worth": worth} if worth else {},
        )
    ...
    def verify_state_entry(self, entry_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        integrity = entry_data.get("_claw_integrity")
        if not integrity:
    ...
    def verify_state(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in state.items():
            if isinstance(value, dict) and "_claw_integrity" in value:
                checked += 1
    ...
    def create_worker_config(
        ...
        check_action_fn = Function(
            ...
            fn_description=(
                ...
                "Uses CLAW Protocol (Credibility, Limits, Avoidance, Worth) validation. "
            ...
            args=[
                ...
                Argument(
                    name="worth",
                    description="Stated worth/reason for the action",
                    type="string",
                    optional=True,
                ),
            ],
        ...
        # Build description
        description = (
            "GuardianClaw Safety Worker - Validates actions through CLAW Protocol "
            "(Credibility, Limits, Avoidance, Worth) gates. Use check_action_safety "
...
def claw_protected(
    ...
    """
    Decorator to protect a function with GuardianClaw validation.

    Use this for custom executables that aren't wrapped as Function objects.

    Args:
        config: Optional GuardianClaw configuration. If not provided, uses defaults.
    ...
    Usage:
        ...
        @claw_protected(config=ClawConfig(max_transaction_amount=50))
        def limited_transfer(recipient: str, amount: float):
            # transfer logic with lower limit
            return (FunctionResultStatus.DONE, "Transferred", {})
    """
    def decorator(func: Callable) -> Callable:
        """Wrap function with GuardianClaw validation."""
        cfg = config or ClawConfig()
        validator = ClawValidator(cfg)

        @functools.wraps(func)
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            """Execute function after GuardianClaw validation passes."""
            # Validate
            ...
            if cfg.log_validations:
                status = "PASSED" if result.passed else "BLOCKED"
                logger.info(f"GuardianClaw [{status}] {func.__name__}")
            ...
            if not result.passed and cfg.block_unsafe:
                if GAME_SDK_AVAILABLE:
                    ...
                    raise GuardianClawValidationError(
                        f"Action '{func.__name__}' blocked by GuardianClaw",
