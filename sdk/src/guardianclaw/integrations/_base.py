"""
Base class for all GuardianClaw integrations.

This module provides ClawIntegration and AsyncClawIntegration,
...
1. GuardianClaw v3.0 validation (InputValidator + OutputValidator)
...
Usage for integration developers:
    ...
    class MyFrameworkIntegration(ClawIntegration):
...
Usage for end users:
    ...
    from guardianclaw.integrations._base import ValidationConfig
...
from guardianclaw.validation import (
    ...
    ValidationResult,
    ValidationLayer,
    GuardianClawV3Adapter,
    AsyncGuardianClawV3Adapter,
)
from guardianclaw.validation.types import RiskLevel

if TYPE_CHECKING:
    from guardianclaw.core.interfaces import Validator, AsyncValidator


logger = logging.getLogger("guardianclaw.integrations")
...
class ClawIntegration:
    ...
    Example:
        class GoogleADKPlugin(ClawIntegration):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            use_v3: Use GuardianClaw v3.0 adapter (default: True). If False, uses
        ...
        Priority:
            ...
            2. use_v3=True - creates GuardianClawV3Adapter (recommended)
        ...
        if validator is not None:
            ...
            # Use GuardianClaw v3.0 adapter (recommended)
            from guardianclaw.validation import GuardianClawV3Adapter

            self._validator = GuardianClawV3Adapter(
                config=validation_config,
                use_embeddings=use_embeddings,
            )
            logger.debug(
                f"[{self._integration_name}] Created GuardianClawV3Adapter "
    ...
    def get_seed(self, level: Optional[str] = None) -> str:
        ...
        claw = GuardianClaw(seed_level=target_level)
        return claw.get_seed(target_level)
    ...
    def prepare_system_prompt(
        ...
        prompt. The seed provides CLAW (Credibility, Limits, Avoidance, Worth) gates
    ...
    def validate_action(
        ...
        Args:
            ...
            worth: Stated worth for the action (for CLAW Worth gate)
        ...
        Example:
            result = self.validate_action(
                ...
                worth="Fetch user data for report",
            )
            if not result.is_safe:
                raise ActionBlockedError(result.violations)
        """
        return self._validator.validate_action(action_name, action_args, worth)
    ...
    def validate_output(
        ...
        Returns:
            ...
            - gates_failed: CLAW gates that failed
...
class AsyncClawIntegration:
    """
    Async version of ClawIntegration for async frameworks.
    ...
    Example:
        class AsyncGoogleADKPlugin(AsyncClawIntegration):
            _integration_name = "google_adk"
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """
        Initialize with AsyncGuardianClawV3Adapter if no validator provided.

        Args:
            validator: Custom async validator instance
            validation_config: Configuration for validator
            use_v3: Use GuardianClaw v3.0 adapter (default: True)
        ...
        if validator is not None:
            ...
            # Use GuardianClaw v3.0 async adapter (recommended)
            from guardianclaw.validation import AsyncGuardianClawV3Adapter

            self._validator = AsyncGuardianClawV3Adapter(
                config=validation_config,
                use_embeddings=use_embeddings,
            )
            logger.debug(
                f"[{self._integration_name}] Created AsyncGuardianClawV3Adapter "
                f"(embeddings={use_embeddings})"
            )
        else:
    ...
    async def avalidate_action(
        ...
        Returns:
            ValidationResult
        """
        return await self._validator.validate_action(action_name, action_args, worth)
