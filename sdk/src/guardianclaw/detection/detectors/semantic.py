"""
Semantic Detector - LLM-based attack detection using CLAW validation.

This module provides SemanticDetector, a detector that uses the SemanticValidator
(LLM-based CLAW validation) to detect attacks that bypass heuristic detection.
...
Architecture:
    ...
    SemanticValidator (LLM)
        ...
        └── SemanticDetector (adapter)
            ...
            └── Maps CLAW gates to AttackType

Usage:
    from guardianclaw.detection.detectors.semantic import SemanticDetector
...
logger = logging.getLogger("guardianclaw.detection.detectors.semantic")
...
# CLAW gate to AttackType mapping
GATE_TO_ATTACK_TYPE: Dict[str, AttackType] = {
    "credibility": AttackType.MANIPULATION,
    "avoidance": AttackType.HARMFUL_REQUEST,
    "limits": AttackType.JAILBREAK,
    "worth": AttackType.MANIPULATION,
...
class SemanticDetector(BaseDetector):
    ...
    Uses SemanticValidator (CLAW validation via LLM) to detect attacks
    that bypass heuristic pattern matching.

    The detector:
    - Sends text to LLM for CLAW gate analysis
    ...
    def detect(
        ...
        """
        Detect attacks using LLM-based CLAW validation.
        ...
        try:
            ...
            claw_result = validator.validate(text)

            # Convert CLAWResult to DetectionResult
            result = self._convert_claw_result(claw_result)
    ...
    def _convert_claw_result(self, claw_result) -> DetectionResult:
        """
        Convert CLAWResult to DetectionResult.

        Maps CLAW gate failures to appropriate AttackType categories.
...
class AsyncSemanticDetector:
    ...
    async def detect(
        ...
        """Async detect attacks using LLM-based CLAW validation."""
        if not text or not text.strip():
            return DetectionResult.nothing_detected(self.name, self.version)
        ...
        try:
            ...
            claw_result = await self._validator.validate(text)
