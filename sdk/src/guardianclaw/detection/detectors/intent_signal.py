...
Design Philosophy:
    ...
    2. Intent combinations - action + target patterns that indicate avoidance
...
logger = logging.getLogger("guardianclaw.detection.intent_signal")
...
EVASION_PATTERNS: List[Tuple[str, re.Pattern, float]] = [
    ...
    (
        ...
        re.compile(
            ...
            # Requires harmful context: attack, avoidance, steal, hack, etc.
            ...
            r".*\b(attack|hack|steal|avoidance|access\s+their|break\s+into)\b",
            re.IGNORECASE,
        ),
        0.75,
    ),
...
INTENT_COMBOS: List[Tuple[str, re.Pattern, float, str]] = [
    ...
    (
        "encourage_harmful",
        re.compile(
            r"\b(encourage|promoting|urging)\b.*\b(young people|children|teens|minors)\b.*\b(sex|drug|alcohol|avoidance)\b",
            re.IGNORECASE,
        ),
        0.85,
        "harmful_content",
