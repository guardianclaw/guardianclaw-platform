...
How It Works:
    ...
    4. Return CheckFailureType.HARMFUL_CONTENT for CLAW AVOIDANCE gate
...
logger = logging.getLogger("guardianclaw.detection.checkers.embedding")
...
# Category mapping: toxic vector categories to CLAW CheckFailureType
# Toxic content primarily maps to AVOIDANCE gate
TOXIC_CATEGORY_TO_FAILURE_TYPE: Dict[str, CheckFailureType] = {
    # Primary toxicity categories -> AVOIDANCE gate
...
class EmbeddingChecker(BaseChecker):
    ...
    - Maps toxic categories to CLAW failure types
    ...
    def _map_category_to_failure_type(self, category: str) -> CheckFailureType:
        """
        Map toxic vector category to CLAW CheckFailureType.

        Most toxic content maps to HARMFUL_CONTENT (AVOIDANCE gate), but some
        categories like misinformation map to DECEPTIVE_CONTENT (CREDIBILITY gate).
