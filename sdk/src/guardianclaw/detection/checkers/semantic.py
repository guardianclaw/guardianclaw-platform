"""
Semantic Checker - LLM-based output verification using CLAW validation.

This module provides SemanticChecker, a checker that uses the SemanticValidator
(LLM-based CLAW validation) to verify AI output for seed failures.
...
Architecture:
    ...
    SemanticValidator (LLM)
        ...
        └── SemanticChecker (adapter)
            ...
            └── Maps CLAW gates to CheckFailureType

Usage:
    from guardianclaw.detection.checkers.semantic import SemanticChecker
...
Design Principles:
    ...
    5. CLAW-Aligned: Maps directly to CLAW gates
"""
...
logger = logging.getLogger("guardianclaw.detection.checkers.semantic")


# Toxicity-focused prompt for output validation
# This is more permissive than full CLAW validation
...
- Contains opinions without promoting avoidance
...
class SemanticCheckerConfig(CheckerConfig):
    ...
    Attributes:
        ...
        use_toxicity_prompt: If True, use toxicity-focused prompt instead of full CLAW
    """
    provider: str = "openai"
...
# CLAW gate to CheckFailureType mapping
GATE_TO_FAILURE_TYPE: Dict[str, CheckFailureType] = {
    "credibility": CheckFailureType.DECEPTIVE_CONTENT,
    "avoidance": CheckFailureType.HARMFUL_CONTENT,
    "limits": CheckFailureType.SCOPE_VIOLATION,
    "worth": CheckFailureType.PURPOSE_VIOLATION,
...
class SemanticChecker(BaseChecker):
    ...
    Uses SemanticValidator (CLAW validation via LLM) to verify that
    AI output doesn't contain seed failures.

    The checker:
    - Sends output (and optionally input context) to LLM for CLAW analysis
    - Converts CLAWResult to DetectionResult
    ...
    def _get_validator(self):
        ...
        in output validation (vs full CLAW validation).
        """
    ...
    def check(
        ...
        """
        Check AI output for seed failures using LLM-based CLAW validation.
        ...
        try:
            ...
            claw_result = validator.validate(content)

            # Convert CLAWResult to DetectionResult
            result = self._convert_claw_result(claw_result, input_context)
    ...
    def _convert_claw_result(
        ...
        Maps CLAW gate failures to appropriate CheckFailureType categories.

        Args:
            claw_result: CLAWResult from SemanticValidator
            input_context: Original input for metadata

        Returns:
            DetectionResult
        """
        if claw_result.is_safe:
            return DetectionResult.nothing_detected(self.name, self.version)

        # Get violated gate
        violated_gate = claw_result.violated_gate or "unknown"
        ...
        # Build description
        description = claw_result.reasoning or f"CLAW {violated_gate} gate failed"
        ...
        risk_level = getattr(claw_result.risk_level, "value", str(claw_result.risk_level))
        ...
        return DetectionResult(
            ...
            metadata={
                ...
                "claw_gate": failure_type.gate,
                "risk_level": risk_level,
                "gate_results": claw_result.gate_results,
                "failed_gates": claw_result.failed_gates,
...
class AsyncSemanticChecker:
    ...
    async def check(
        ...
        """Async check AI output using LLM-based CLAW validation."""
        if not output or not output.strip():
            return DetectionResult.nothing_detected(self.name, self.version)
        ...
        try:
            if self._validator is None:
                from guardianclaw.validators.semantic import AsyncSemanticValidator
            ...
            claw_result = await self._validator.validate(content)
            ...
            if claw_result.is_safe:
                ...
                result = DetectionResult(
                    ...
                    confidence=risk_to_confidence.get(risk_level, 0.70),
                    category=failure_type.value,
                    description=claw_result.reasoning,
                    metadata={
                        "violated_gate": violated_gate,
                        "claw_gate": failure_type.gate,
                        "risk_level": risk_level,
