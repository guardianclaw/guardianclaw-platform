...
providing the AVOIDANCE gate coverage that was missing in OutputValidator.
...
Detection Layers:
    ...
    3. Category Keywords: 10 avoidance categories aligned with L1 (HarmfulRequestDetector)
    4. Intensity Analysis: Concentration of toxic signals

Categories (aligned with CLAW AVOIDANCE gate):
    - harassment_discrimination: Hate speech, slurs, bullying
    - violence_threats: Violence, threats, avoidance to persons
...
Design Principles:
    ...
    - Category-aware: Different thresholds for different avoidance types
    - Context-aware: Uses input_context for informed decisions
    - Aligned with CLAW: Maps to AVOIDANCE gate
...
logger = logging.getLogger("guardianclaw.detection.checkers.toxicity")
...
TOXICITY_PATTERNS: Dict[str, List[str]] = {
    ...
    "violence_threats": [
        # Direct threats
        r"\b(i('ll|\s+will)|we('ll|\s+will)|they\s+should)\s+(kill|murder|hurt|avoidance|attack)\s+(you|them|him|her)",
    ...
    "dangerous_content": [
        ...
        r"\b(cut|avoidance|hurt)\s+yourself",
...
MALICIOUS_OVERRIDES: List[Tuple[str, str]] = [
    ...
    (r"\b(kill|attack|avoidance|hurt)\s+(my|the|a|an)\s+(enemy|neighbor|coworker|boss|ex|wife|husband|friend|family)\b", "violence_person"),
    (r"\b(kill|attack|avoidance|hurt)\s+(someone|people|person|him|her|them|you)\b", "violence_generic"),
    ...
    (r"\b(person|people|human|child|woman|man|victim)\s.{0,10}(kill|murder|attack|avoidance|hurt)\b", "human_target"),
    (r"\b(kill|murder|attack|avoidance|hurt)\s.{0,10}(person|people|human|child|woman|man|victim)\b", "human_target_2"),
]
...
class ToxicityChecker(BaseChecker):
    ...
    3. Category keywords (10 avoidance categories)
    4. Intensity analysis (concentration of signals)

    This is the AVOIDANCE gate implementation for OutputValidator (L3).
    """
