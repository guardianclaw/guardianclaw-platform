...
GCLAW_SRC = DETECTION_DIR.parent
GCLAW_ROOT = GCLAW_SRC.parent.parent

sys.path.insert(0, str(GCLAW_SRC))
...
def main():
    ...
    parser.add_argument(
        "--jailbreakbench",
        type=Path,
        default=GCLAW_ROOT / "evaluation" / "benchmarks" / "jailbreakbench" / "data" / "jailbreakbench_cache.json",
        help="JailbreakBench data file",
    )
    parser.add_argument(
        "--harmbench",
        type=Path,
        default=GCLAW_ROOT / "evaluation" / "benchmarks" / "harmbench" / "data" / "harmbench_behaviors_text_all.csv",
