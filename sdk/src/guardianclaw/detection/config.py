...
Environment Variables:
    GCLAW_API_KEY: API key for semantic validation
    GCLAW_PROVIDER: LLM provider ("openai", "anthropic")
    GCLAW_MODEL: Model to use for semantic validation
    GCLAW_MODE: Validation mode ("auto", "heuristic", "semantic")
    GCLAW_MIN_CONFIDENCE: Minimum confidence to block (0.0-1.0)
    GCLAW_EMBEDDING_MODEL: Model for embedding-based detection
...
Example:
    ...
    from guardianclaw.detection import InputValidatorConfig
...
class DetectionConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.api_key is None:
            self.api_key = os.environ.get("GCLAW_API_KEY")
...
class InputValidatorConfig(DetectionConfig):
    ...
    def __post_init__(self) -> None:
        ...
        env_embedding_model = os.environ.get("GCLAW_EMBEDDING_MODEL")
        if env_embedding_model:
            self.embedding_model = env_embedding_model

        # Load min_confidence from environment
        env_min_confidence = os.environ.get("GCLAW_MIN_CONFIDENCE")
    ...
    def from_env(cls) -> "InputValidatorConfig":
        ...
        Reads:
            GCLAW_API_KEY, GCLAW_PROVIDER, GCLAW_MODEL,
            GCLAW_MODE, GCLAW_EMBEDDING_MODEL, GCLAW_MIN_CONFIDENCE

        Returns:
            InputValidatorConfig populated from environment

        Example:
            os.environ["GCLAW_API_KEY"] = "sk-..."
            config = InputValidatorConfig.from_env()
        """
        return cls(
            api_key=os.environ.get("GCLAW_API_KEY"),
            provider=os.environ.get("GCLAW_PROVIDER", "openai"),
            model=os.environ.get("GCLAW_MODEL"),
            mode=os.environ.get("GCLAW_MODE", "auto"),
            embedding_model=os.environ.get(
                "GCLAW_EMBEDDING_MODEL", "all-MiniLM-L6-v2"
            ),
            min_confidence_to_block=float(
                os.environ.get("GCLAW_MIN_CONFIDENCE", "0.7")
            ),
        )
...
class OutputValidatorConfig(DetectionConfig):
    ...
    def from_env(cls) -> "OutputValidatorConfig":
        ...
        return cls(
            ...
            model=os.environ.get("GCLAW_MODEL"),
            mode=os.environ.get("GCLAW_MODE", "auto"),
            context_type=os.environ.get("GCLAW_CONTEXT_TYPE", "general"),
        )
