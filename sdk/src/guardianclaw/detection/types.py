...
class CheckFailureType(str, Enum):
    ...
    Categories:
        HARMFUL_CONTENT: Output contains harmful material
            ...
            - Maps to CLAW Avoidance gate failure

        DECEPTIVE_CONTENT: Output contains deception
            - Misinformation, fake content
            - Maps to CLAW Credibility gate failure

        SCOPE_VIOLATION: Output exceeds intended limits
            - Acting outside defined role
            - Maps to CLAW Limits gate failure

        PURPOSE_VIOLATION: Output lacks legitimate purpose
            - Unnecessary harmful detail
            - Maps to CLAW Worth gate failure
    ...
    def gate(self) -> str:
        """
        Get the CLAW gate this failure type corresponds to.

        Returns:
            Gate name: "credibility", "avoidance", "limits", or "worth"
        """
        gate_map = {
...
class OutputValidationResult:
    ...
    Properties:
        ...
        gates_failed: List of CLAW gates that failed
    ...
    def gates_failed(self) -> List[str]:
        """
        Get list of CLAW gates that failed based on failure types.
