...
class AdversarialBehaviorDetector(BaseBehaviorDetector):
    ...
    def _init_patterns(self):
        ...
        self._patterns[BehaviorType.AB_INSTRUCTION_OVERRIDE] = [
            ...
            re.compile(r"my\s+(real|true|new)\s+(instructions?|worth)\s+(is|are)", re.I),
        ]
        ...
        self._patterns[BehaviorType.AB_ROLEPLAY_EXPLOITATION] = [
            re.compile(r"\*?(in\s+character|IC|as\s+the\s+character)\*?:?\s*.*(avoidance|kill|illegal|weapon)", re.I),
            re.compile(r"(the\s+character|my\s+character)\s+(would|will)\s+(avoidance|kill|attack|make\s+weapons?)", re.I),
