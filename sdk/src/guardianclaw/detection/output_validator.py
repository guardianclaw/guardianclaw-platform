...
the CLAW protocol. This is the second layer of the 360 validation:
...
Design Principles:
    ...
    3. CLAW-Aligned: Failure types map to CLAW gates
    4. Configurable: Thresholds and behavior via OutputValidatorConfig
    5. Non-Invasive: Checks without altering the output

Failure Types Detected (mapped to CLAW gates):
    - HARMFUL_CONTENT → Avoidance gate (violence, malware, fraud)
    - DECEPTIVE_CONTENT → Credibility gate (jailbreak acceptance, impersonation)
    - SCOPE_VIOLATION → Limits gate (authority claims, overstepping)
    - PURPOSE_VIOLATION → Worth gate (purposeless destruction)
    - BYPASS_INDICATOR → Limits gate (signs of successful jailbreak)
    - POLICY_VIOLATION → Limits gate (custom policy violations)
...
from guardianclaw.detection.registry import CheckerRegistry

logger = logging.getLogger("guardianclaw.detection.output_validator")
...
class OutputValidator:
    ...
    def _init_default_checkers(self) -> None:
        ...
        try:
            from guardianclaw.detection.checkers import (
            ...
            # Register ToxicityChecker (weight 1.3 - covers AVOIDANCE gate for toxicity)
        ...
        # 5. SemanticChecker - LLM-based CLAW validation (optional)
    ...
    def _init_semantic_checker(self) -> None:
        ...
        This checker uses the SemanticValidator (CLAW validation via LLM)
    ...
    def validate(
        ...
        Example:
            ...
            if result.seed_failed:
                print(f"CLAW gates failed: {result.gates_failed}")
        """
