"""
GuardianClaw Core Types - TypedDicts for type-safe function returns.

This module defines TypedDicts for the various return types used throughout
the GuardianClaw library. Using TypedDicts instead of Dict[str, Any] provides:
...
class ChatResponse(TypedDict):
    """
    Response from GuardianClaw.chat() method.
...
class CLAWResultDict(TypedDict):
    """
    Dictionary representation of CLAWResult for serialization.

    Attributes:
        is_safe: Overall safety assessment
        truth_passes: Whether content passes Credibility gate
        harm_passes: Whether content passes Avoidance gate
        scope_passes: Whether content passes Limits gate
        purpose_passes: Whether content passes Worth gate
