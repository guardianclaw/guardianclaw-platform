"""
GuardianClaw v3.0 Configuration.

Unified configuration for the 3-gate architecture as defined in
GCLAW_V3_ARCHITECTURE.md.

Example:
    from guardianclaw.core import ClawConfig, BlockMessages, Gate4Fallback
...
class Gate4Fallback(Enum):
    """
    Behavior when Gate 4 (L4 GuardianClaw Observer) is unavailable.
...
class ClawConfig:
    """
    Configuration for ClawValidator.

    The 3-gate validation flow (Gates 1 → 2 → 4):
    - Gate 1 (Input): Heuristic detection of attacks
    - Gate 2 (Output): Heuristic + Embedding detection of failures
    - Gate 4 (Observer): LLM-based transcript analysis (L4 component)

    Note: Gate 4 uses "gate4_*" config fields to reflect its L4 position
    in the L1-L2-L3-L4 architecture. Legacy "gate3_*" aliases are provided
    ...
    Attributes:
        ...
        gate2_confidence_threshold: Confidence threshold to skip Gate 4

        gate4_enabled: Enable Gate 4 (ClawObserver — L4)
        gate4_provider: LLM provider ("openai", "anthropic", "groq", "together", "deepseek", "mistral")
        gate4_model: Model to use for Gate 4
        gate4_timeout: Timeout in seconds for Gate 4 API call
        gate4_api_key: Optional API key (uses env var if not set)
        gate4_base_url: Optional custom base URL for API (auto-configured for known providers)
    ...
    gate2_confidence_threshold: float = 0.95  # High threshold to escalate more to Gate 4 (L4)

    # Gate 4 (L4 GuardianClaw Observer) - LLM
...
DEFAULT_CONFIG = ClawConfig()

MINIMAL_CONFIG = ClawConfig(
    gate1_embedding_enabled=False,
    gate2_embedding_enabled=False,
    gate4_enabled=False,
)

FULL_CONFIG = ClawConfig(
