"""
GuardianClaw v3.0 Result Types.

Result dataclasses for the 3-gate architecture as defined in
GCLAW_V3_ARCHITECTURE.md.

Example:
    from guardianclaw.core import ObservationResult, GuardianClawResult

    # Gate 4 (L4 Observer) result
    ...
    # Final result
    result = GuardianClawResult.blocked_by_gate4(
...
if TYPE_CHECKING:
    from guardianclaw.core.claw_config import BlockMessages


@dataclass
class ObservationResult:
    """
    Result from ClawObserver (Gate 4 — L4 component).

    The observer analyzes a transcript of user-AI interaction
    and evaluates against the CLAW protocol (Credibility, Limits, Avoidance, Worth).
    ...
    Attributes:
        ...
        gates_violated: List of CLAW gates violated (CREDIBILITY, AVOIDANCE, LIMITS, WORTH)
...
@dataclass
class GuardianClawResult:
    """
    Unified result from ClawValidator.
    ...
    gate3_result: Optional[ObservationResult] = None  # L4 observer result (field name kept for compat)
    ...
    # Metadata
    latency_ms: float = 0.0
    gate3_was_called: bool = False  # True when Gate 4 (L4) was invoked (field name kept for compat)
    ...
    def with_user_message(self, message: str) -> "GuardianClawResult":
        """
        ...
        Returns:
            New GuardianClawResult with the message set.
        """
    ...
    def blocked_by_gate4(
        ...
        This is the L4 GuardianClaw Observer - the final semantic analysis layer.
    ...
    def allowed_by_gate3(
        ...
        """Factory for Gate 3 allow (observer confirmed safe). Deprecated: use allowed_by_gate4."""
        return cls(
            blocked=False,
            allowed=True,
            decided_by="gate3",
    ...
    def allowed_by_gate4(
        ...
        """Factory for Gate 4 allow (L4 observer confirmed safe)."""
        return cls(
            blocked=False,
            allowed=True,
            decided_by="gate4",
