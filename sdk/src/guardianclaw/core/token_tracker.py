...
DESIGN: This module is OBSERVABILITY-ONLY.
It records and reports token usage but does NOT enforce rate limits.
TPM (tokens per minute) and RPM (requests per minute) enforcement is
the responsibility of the application layer. Use provider-level rate
limiting (e.g., OpenAI's request queuing, Anthropic's retry headers)
or an external token bucket implementation for actual enforcement.
"""
...
class TokenTracker:
    """
    Centralized token usage tracker (observability-only).

    Records token consumption across all GuardianClaw components and
    provides aggregate statistics with per-component breakdown.

    This class is a METRICS ACCUMULATOR. It does NOT:
    - Enforce TPM (tokens per minute) limits
    - Enforce RPM (requests per minute) limits
    - Block or throttle API calls when limits are exceeded

    Rate limiting must be implemented at the application layer.
    Use ``get_stats()`` to feed usage data into your own limiter,
    or rely on provider SDK retry/backoff logic (e.g., openai-python
    ...
    Example:
        ...
        # Track L4 usage
        tracker.track_l4_claw(prompt=800, completion=200, cost=0.001)

        # Get stats (for logging, dashboards, or external rate limiters)
    ...
    def __init__(self):
        ...
        self._components: Dict[str, ComponentUsage] = {
            ...
            "l4_claw": ComponentUsage(name="L4 GuardianClaw Observer"),
    ...
    def track_l4_claw(
        ...
        """
        Track token usage from L4 GuardianClaw Observer.
        ...
        if cost is None:
            cost = self._estimate_cost(prompt + completion, model)
        self._components["l4_claw"].add(prompt, completion, cost)
...
def call_openai_with_tracking(
    ...
    **kwargs,
    ...
    Example:
        ...
        from guardianclaw.core.token_tracker import call_openai_with_tracking
...
def call_anthropic_with_tracking(
    ...
    **kwargs,
    ...
    Example:
        ...
        from guardianclaw.core.token_tracker import call_anthropic_with_tracking
