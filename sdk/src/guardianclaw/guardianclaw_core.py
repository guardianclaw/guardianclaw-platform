"""
Core GuardianClaw class and seed management.

This module provides the main GuardianClaw class, which is the primary entry point
for using the guardianclaw library. It combines alignment seeds with CLAW
(Credibility, Limits, Avoidance, Worth) validation through LayeredValidator.
"""
...
if TYPE_CHECKING:
    from guardianclaw.core.types import ChatResponse, ValidationInfo


class SeedLevel(Enum):
    """Available seed levels with different size/coverage trade-offs."""
    MINIMAL = "minimal"      # ~360 tokens - essential CLAW gates only
...
class GuardianClaw:
    """
    Main class for GuardianClaw AI alignment toolkit.

    Provides:
    - Access to alignment seeds (minimal, standard, full)
    - Chat wrapper with automatic seed injection
    - Response validation through CLAW gates (Credibility, Limits, Avoidance, Worth)
    ...
    Example:
        ...
        # Get a seed
        seed = claw.get_seed(SeedLevel.STANDARD)

        # Or use chat directly
        response = claw.chat("Help me write a Python function")

        # Validate a response
        is_safe, violations = claw.validate("I'll help you hack...")

        # Get detailed validation result
        result = claw.get_validation_result("some text")
    ...
    def _init_validator(self, use_semantic: Optional[bool]) -> None:
        ...
        # Note: Legacy code expecting CLAWValidator.validate() dict format should
    ...
    def _get_seed_path(self, level: SeedLevel) -> Path:
        ...
        raise FileNotFoundError(
            ...
            f"  1. {package_seed}\n"
            f"  2. {project_seed}\n"
            f"Ensure seed files are installed correctly or check your guardianclaw installation."
        )
    ...
    def chat(
        ...
        **kwargs: Any
        ...
        Args:
            ...
            validate_response: Whether to run CLAW validation on response
        ...
        if self.provider == "openai":
            from guardianclaw.providers.openai import OpenAIProvider
            provider = OpenAIProvider(
                model=self.model,
                api_key=self._api_key
            )
        elif self.provider == "anthropic":
            from guardianclaw.providers.anthropic import AnthropicProvider
    ...
    def validate_action(self, action_plan: str) -> tuple:
        ...
        Combines physical safety pattern checking with CLAW validation.
    ...
    def __repr__(self) -> str:
        return f"GuardianClaw(seed_level={self.seed_level.value}, provider={self.provider}, model={self.model})"
