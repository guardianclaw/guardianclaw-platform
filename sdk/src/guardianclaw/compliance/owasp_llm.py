"""
OWASP LLM Top 10 (2025) Compliance Checker using GuardianClaw CLAW.
...
CLAW gates provide behavioral-level controls for applicable vulnerabilities:
- Limits Gate: LLM01, LLM06, LLM07
- Credibility Gate: LLM02, LLM05, LLM09
- Avoidance Gate: LLM02, LLM05
- Worth Gate: LLM06
...
logger = logging.getLogger("guardianclaw.compliance.owasp_llm")
...
class CoverageLevel(str, Enum):
    """CLAW coverage level for OWASP vulnerabilities."""
    STRONG = "strong"
    MODERATE = "moderate"
    INDIRECT = "indirect"
...
# OWASP vulnerability to CLAW Gate mapping
VULNERABILITY_GATE_MAPPING: Dict[OWASPVulnerability, Dict[str, Any]] = {
    ...
    OWASPVulnerability.LLM07_SYSTEM_PROMPT_LEAKAGE: {
        ...
        "claw_protection": "Limits gate detects extraction attempts",
    },
    OWASPVulnerability.LLM08_VECTOR_EMBEDDING: {
        ...
        "claw_protection": "Infrastructure-level concern, not behavioral",
    },
    OWASPVulnerability.LLM09_MISINFORMATION: {
        "gates": ["credibility"],
        "coverage": CoverageLevel.STRONG,
        "description": "LLMs generating false information",
        "claw_protection": "Credibility gate enforces epistemic humility",
    },
    OWASPVulnerability.LLM10_UNBOUNDED_CONSUMPTION: {
        "gates": [],
        "coverage": CoverageLevel.NOT_APPLICABLE,
        "description": "Resource exhaustion attacks",
        "claw_protection": "Infrastructure-level rate limiting required",
    },
}
...
class VulnerabilityFinding:
    ...
    Attributes:
        ...
        coverage_level: CLAW coverage level for this vulnerability
        gates_checked: Which CLAW gates were evaluated
...
class OWASPLLMChecker:
    ...
    Uses CLAW gates to detect:
    - LLM01: Prompt Injection (Limits gate)
    - LLM02: Sensitive Info Disclosure (Credibility + Avoidance gates)
    - LLM05: Improper Output Handling (Credibility + Avoidance gates)
    ...
    # Vulnerabilities with CLAW support
    ...
    def _init_validator(self):
        ...
        if self._validator is None:
            try:
                ...
                self._validator = CLAWValidator()
                logger.info("Using CLAWValidator (heuristic) for OWASP LLM compliance")
            except ImportError:
    ...
    def _run_claw_validation(
        self, content: str
    ) -> Tuple[Dict[str, bool], bool, List[str]]:
        """Run content through CLAW gates."""
        if self._validator is None:
    ...
    def check_input(
        ...
        gates, is_safe, failed_gates = self._run_claw_validation(content)
        findings = []
    ...
    def _get_vulnerability_recommendation(self, vuln: OWASPVulnerability) -> str:
        ...
        recommendations = {
            OWASPVulnerability.LLM01_PROMPT_INJECTION:
                ...
                "Consider using GuardianClaw's Limits gate for pre-inference validation.",
            OWASPVulnerability.LLM02_SENSITIVE_INFO_DISCLOSURE:
            ...
            OWASPVulnerability.LLM05_IMPROPER_OUTPUT_HANDLING:
                ...
                "Use GuardianClaw's output validation in your pipeline.",
            OWASPVulnerability.LLM06_EXCESSIVE_AGENCY:
                "Limit agent capabilities and require explicit user confirmation "
                "for high-impact actions. Implement Worth gate validation.",
            OWASPVulnerability.LLM07_SYSTEM_PROMPT_LEAKAGE:
...
def check_owasp_llm_compliance(
    ...
    else:  # both - validate as input and output perspectives
        result = checker.check_pipeline(content, content)
