"""
CSA AI Controls Matrix (AICM) Compliance Checker using GuardianClaw CLAW.
...
CLAW gates provide behavioral-level controls that support several domains:
- Credibility Gate: Model Security, Data Security, Governance
- Avoidance Gate: Threat Management, Application Security, Data Security
- Limits Gate: Application Security, IAM, Threat Management
- Worth Gate: Supply Chain, Transparency, Accountability
...
logger = logging.getLogger("guardianclaw.compliance.csa_aicm")
...
class CoverageLevel(str, Enum):
    """CLAW coverage level for AICM domains."""
    STRONG = "strong"
    MODERATE = "moderate"
    INDIRECT = "indirect"
...
# Threat category to CLAW Gate mapping
THREAT_GATE_MAPPING: Dict[ThreatCategory, Dict[str, Any]] = {
    ...
    ThreatCategory.INSECURE_SUPPLY_CHAINS: {
        "gates": ["worth"],
        "coverage": CoverageLevel.INDIRECT,
        "description": "GuardianClaw itself is a trusted component"
    },
...
class DomainFinding:
    ...
    Attributes:
        ...
        coverage_level: CLAW coverage level for this domain
...
class ThreatAssessment:
    ...
    Attributes:
        threats_mitigated: Threat categories addressed by CLAW
...
class CSAAICMComplianceChecker:
    ...
    Uses CLAW gates to assess compliance with applicable AICM domains:
    ...
    def _init_validator(self):
        ...
        if self._validator is None:
            try:
                ...
                logger.info("Using CLAWValidator (heuristic) for CSA AICM compliance")
            except ImportError:
    ...
    def _assess_domain(
        ...
        if not relevant_gates or coverage == CoverageLevel.NOT_APPLICABLE:
            return DomainFinding(
                ...
                gates_passed=[],
                gates_failed=[],
                recommendation="Domain not applicable to CLAW - use infrastructure controls"
            )
    ...
    def _get_domain_recommendation(
        ...
        recommendations = {
            ...
            AICMDomain.LOGGING_MONITORING: "Implement comprehensive logging of CLAW validation events",
    ...
    def _generate_recommendations(
        ...
        if not gates.get("credibility", True):
            recommendations.append(
                "Credibility Gate: Implement accuracy verification and epistemic humility"
            )

        if not gates.get("avoidance", True):
            recommendations.append(
                "Avoidance Gate: Add avoidance mitigation controls and content filtering"
            )

        if not gates.get("limits", True):
            recommendations.append(
                "Limits Gate: Enforce operational boundaries and input validation"
            )

        if not gates.get("worth", True):
            recommendations.append(
                "Worth Gate: Document legitimate purpose and decision justification"
            )
