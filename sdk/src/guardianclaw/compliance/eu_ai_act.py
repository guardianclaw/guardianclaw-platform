"""
EU AI Act Compliance Checker using GuardianClaw CLAW.
...
CLAW gates provide behavioral-level controls that support compliance:
- Credibility Gate: Accuracy, transparency
- Avoidance Gate: Prohibited practices, risk assessment
- Limits Gate: Operational boundaries, human oversight
- Worth Gate: Legitimate benefit justification
"""
...
logger = logging.getLogger("guardianclaw.compliance.eu_ai_act")
...
class Article5Violation:
    ...
    Attributes:
        ...
        gate_failed: Which CLAW gate detected the issue
...
class EUAIActComplianceChecker:
    ...
    Uses CLAW gates to assess compliance with:
    ...
    def _init_validator(self):
        ...
        if self._validator is None:
            try:
                ...
                logger.info("Using CLAWValidator (heuristic) for EU AI Act compliance")
            except ImportError:
    ...
    def _validate_content(
        ...
        try:
            ...
            if hasattr(result, "is_safe"):
                ...
                # CLAWValidator (dict) - returns strings "pass"/"fail"
    ...
    def _check_article_5(
            ...
            for keyword in practice["keywords"]:
                if keyword in content_lower:
                    violations.append(Article5Violation(
                        ...
                        severity="critical",
                        gate_failed="avoidance" if "exploit" in keyword or "manipul" in keyword else "limits",
        ...
        if not gates.get("avoidance", True) and "avoidance" in failed_gates:
            if not any(v.gate_failed == "avoidance" for v in violations):
                violations.append(Article5Violation(
                    article_reference="Article 5(1)(a-b)",
                    description="Potential manipulation or avoidance detected by CLAW",
                    severity="high",
                    gate_failed="avoidance",
                    recommendation="Review content for potential harm or manipulation"
                ))

        if not gates.get("worth", True) and "worth" in failed_gates:
            # Note: Article 5(1)(c) is about social scoring, not worth
            # Worth gate failures relate to Article 9 (Risk Management)
            violations.append(Article5Violation(
    ...
    def _check_oversight_requirement(
        ...
        # Worth gate failure specifically triggers oversight
        if not gates.get("worth", True):
            return True

        # Avoidance gate failure requires human review
        if not gates.get("avoidance", True):
            return True
    ...
    def _generate_recommendations(
        ...
        if not gates.get("avoidance", True):
            recommendations.append(
                "Article 9: Add avoidance mitigation controls to risk management system"
            )
