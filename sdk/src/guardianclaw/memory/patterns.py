"""
Memory Injection Pattern Definitions for GuardianClaw Memory Shield v2.0
...
Background:
    ...
    - Address redirection ("treasury moved to 0x..." or "<solana_base58_address>")
...
References:
    ...
    - GuardianClaw browser extension: packages/browser/src/agent-shield/memory-scanner.ts

Synchronization:
    This file is the Python source of credibility. To sync with TypeScript:
...
INSTRUCTION_OVERRIDE_PATTERNS: List[InjectionPattern] = [
    InjectionPattern(
        pattern=r"\b(always|never|must|should)\s+(send|transfer|swap|approve|sign)\s+.*(to|for)\s+(?:0x[a-fA-F0-9]{6,}|[1-9A-HJ-NP-Za-km-z]{32,44})",
        category=InjectionCategory.INSTRUCTION_OVERRIDE,
        confidence=95,
        reason="Wallet address instruction injection (EVM or Solana)",
...
ADDRESS_REDIRECTION_PATTERNS: List[InjectionPattern] = [
    InjectionPattern(
        pattern=r"\b(redirect|forward|send|route)\s+(all|any|every)?\s*(funds?|tokens?|eth|sol|crypto|money|payments?)\s+to\s+(?:0x[a-fA-F0-9]{6,}|[1-9A-HJ-NP-Za-km-z]{32,44})",
        category=InjectionCategory.ADDRESS_REDIRECTION,
        confidence=95,
        reason="Fund redirection instruction (EVM or Solana address)",
        name="fund_redirection",
    ),
    InjectionPattern(
        pattern=r"\b(treasury|vault|wallet|safe)\s*(address)?\s*(updated?|changed?|moved?|is now|now at)\s*(to|at)?\s*:?\s*(?:0x[a-fA-F0-9]{6,}|[1-9A-HJ-NP-Za-km-z]{32,44})",
        category=InjectionCategory.ADDRESS_REDIRECTION,
        confidence=95,
        reason="Fake treasury address update (EVM or Solana address)",
        name="treasury_address_update",
    ),
    InjectionPattern(
        pattern=r"\b(official|verified|new)\s+(wallet|address)\s*:?\s*(?:0x[a-fA-F0-9]{6,}|[1-9A-HJ-NP-Za-km-z]{32,44})",
        category=InjectionCategory.ADDRESS_REDIRECTION,
        confidence=90,
        reason="Fake official address claim (EVM or Solana address)",
...
def compile_patterns(
    ...
    for p in patterns:
        try:
            ...
            import logging
            logging.getLogger("guardianclaw.memory.patterns").warning(
