...
# In production, use: os.environ.get("GCLAW_MEMORY_SECRET")
...
```python
from guardianclaw.memory import MemoryIntegrityChecker, MemorySource
...
```python
from guardianclaw.memory import MemorySource
...
```python
from guardianclaw.memory import InjectionCategory
...
```python
from guardianclaw.memory import MemoryContentUnsafe
...
|----------|-------------|
| `GCLAW_MEMORY_SECRET` | Default secret key for HMAC signing |
...
See `@guardianclaw/elizaos-plugin` for native ElizaOS integration with automatic memory signing and verification.

### With Solana Agent Kit

See `@guardianclaw/solana-agent-kit` for Solana-specific memory integrity features.
...
|---------|----------|--------|
| `guardianclaw` | Python | ✅ Source of truth |
| `@guardianclaw/core` | TypeScript | ✅ Synced |
| `@guardianclaw/elizaos-plugin` | TypeScript | ✅ Synced |
...
- [GuardianClaw Documentation](https://guardianclaw.org/docs/memory-integrity) - Full documentation
...
**GuardianClaw Team** - Practical AI Safety for Developers
