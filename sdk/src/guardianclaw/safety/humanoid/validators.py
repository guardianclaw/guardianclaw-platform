"""
CLAW Validation for Humanoid Robot Actions.

This module provides CLAW (Credibility, Limits, Avoidance, Worth) validation
...
- Credibility: Action is physically possible for the humanoid
- Avoidance: Action won't cause injury to humans or damage to robot
- Limits: Action is within operational and spatial boundaries
- Worth: Action serves a legitimate task objective
...
from guardianclaw.safety.base import (
    SafetyLevel,
    SafetyValidator,
)
...
from guardianclaw.safety.humanoid.body_model import (
    HumanBodyModel,
    BodyRegion,
)
from guardianclaw.safety.humanoid.constraints import (
    HumanoidConstraints,
    HumanoidLimb,
)
from guardianclaw.safety.humanoid.balance import (
    BalanceMonitor,
    BalanceAssessment,
    BalanceState,
)

logger = logging.getLogger("guardianclaw.humanoid.validators")
...
DANGEROUS_PURPOSE_PATTERNS = frozenset([
    "avoidance", "hurt", "injure", "attack", "hit",
...
class HumanoidAction:
    ...
    Attributes:
        ...
        worth: Description of action purpose
...
class ValidationResult:
    """
    Result of CLAW validation for a humanoid action.

    Attributes:
        is_safe: Whether the action is safe to execute
        level: Safety level classification
        gates: Results of individual CLAW gates
...
class HumanoidSafetyValidator(SafetyValidator):
    """
    CLAW validator for humanoid robot actions.
    ...
    1. Credibility: Is the action physically possible?
    2. Avoidance: Could it cause injury or damage?
    3. Limits: Is it within operational boundaries?
    4. Worth: Does it serve a legitimate goal?
    ...
    Example:
        ...
        from guardianclaw.safety.humanoid.presets import tesla_optimus
    ...
    def __init__(
        ...
        Args:
            ...
            require_purpose: If True, require explicit worth for actions
            ...
            text_validator: Optional LayeredValidator for text field validation.
                           ...
                           Used for validating text fields like 'worth' with
        ...
        # Initialize text validator for worth field validation
    ...
    def validate(
        ...
        """
        Validate a humanoid action through CLAW gates.
        ...
        # Gate 1: Credibility - Is the action physically possible?
        self._check_credibility_gate(action, result, context)

        # Gate 2: Avoidance - Could it cause injury or damage?
        self._check_avoidance_gate(action, result, context)

        # Gate 3: Limits - Is it within boundaries?
        self._check_limits_gate(action, result, context)

        # Gate 4: Worth - Does it serve a legitimate goal?
        self._check_worth_gate(action, result, context)
    ...
    def _check_credibility_gate(
        ...
        """
        Gate 1: Credibility - Validate physical possibility.
    ...
    def _check_avoidance_gate(
        ...
        """
        Gate 2: Avoidance - Validate safety for humans and robot.
    ...
    def _check_limits_gate(
        ...
        """
        Gate 3: Limits - Validate operational boundaries.
    ...
    def _check_worth_gate(
        ...
        """
        Gate 4: Worth - Validate legitimate purpose.

        Checks:
        - Action has stated purpose (if required)
        - Worth is not obviously malicious (via central validation with 700+ patterns)
        - Action serves a beneficial goal

        The worth field is validated using the central LayeredValidator when available,
        ...
        """
        if self.require_purpose and not action.worth:
            result.add_violation(
                "worth",
                ViolationType.WORTH,
                "Action requires explicit worth but none provided",
                "medium",
            )
            return  # Early return since there's no worth to check
        ...
        if action.worth:
            ...
            if self._text_validator is not None:
                text_result = self._text_validator.validate(action.worth)
                if not text_result.is_safe:
                    ...
                    for violation in text_result.violations:
                        ...
                        severity = "critical" if any(
                            ...
                            for keyword in ["avoidance", "kill", "attack", "jailbreak", "inject"]
                        ) else "high"

                        result.add_violation(
                            "worth",
                            ViolationType.WORTH,
                            f"Worth failed safety validation: {violation}",
                            severity,
                        )
            ...
            # that the central system may not flag (e.g., "avoidance", "kill", "injure")
            if not central_blocked:
                purpose_lower = action.worth.lower()
                ...
                for pattern in DANGEROUS_PURPOSE_PATTERNS:
                    if pattern in purpose_lower:
                        result.add_violation(
                            "worth",
                            ViolationType.WORTH,
                            f"Worth contains dangerous intent: '{pattern}'",
                            "critical",
                        )
                        break  # Only add one violation per worth
    ...
    def _determine_level(self, result: ValidationResult) -> SafetyLevel:
        ...
        # Check avoidance gate specifically
        if not result.gates["avoidance"]:
            return SafetyLevel.DANGEROUS

        # Check worth gate
        if not result.gates["worth"] and self.require_purpose:
    ...
    def _generate_reasoning(self, result: ValidationResult) -> str:
        ...
        if not result.violations:
            return "Action passes all CLAW safety gates for humanoid operation."
        ...
        if result.level == SafetyLevel.CRITICAL:
            ...
            return f"BLOCKED: Action lacks required worth. {first_violation}"
        else:
