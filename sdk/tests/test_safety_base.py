...
class TestViolationType:
    ...
    def test_claw_gate_values(self):
        """Test CLAW gate violation types."""
        assert ViolationType.TRUTH_VIOLATION.value == "credibility"
        assert ViolationType.HARM_VIOLATION.value == "avoidance"
        assert ViolationType.SCOPE_VIOLATION.value == "limits"
        assert ViolationType.PURPOSE_VIOLATION.value == "worth"
    ...
    def test_gate_property_claw_violations(self):
        """Test gate property for CLAW violation types."""
        assert ViolationType.TRUTH_VIOLATION.gate == "credibility"
        assert ViolationType.HARM_VIOLATION.gate == "avoidance"
        assert ViolationType.SCOPE_VIOLATION.gate == "limits"
        assert ViolationType.PURPOSE_VIOLATION.gate == "worth"

    def test_gate_property_physical_violations(self):
        """Test gate property for physical violations."""
        assert ViolationType.JOINT_POSITION.gate == "limits"
        assert ViolationType.JOINT_VELOCITY.gate == "limits"
        assert ViolationType.FORCE.gate == "avoidance"
        assert ViolationType.TORQUE.gate == "avoidance"

    def test_gate_property_input_violations(self):
        """Test gate property for input validation violations."""
        assert ViolationType.INVALID_VALUE.gate == "credibility"
        assert ViolationType.INVALID_TYPE.gate == "credibility"
        assert ViolationType.MISSING_REQUIRED.gate == "credibility"
...
class TestSafetyResult:
    ...
    def test_custom_gates(self):
        ...
        assert result.gates["avoidance"] is False
        assert result.gates["credibility"] is True
    ...
    def test_failed_gates_property(self):
        ...
        result = SafetyResult(
            is_safe=False,
            level=SafetyLevel.DANGEROUS,
            gates={"credibility": True, "avoidance": False, "limits": False, "worth": True},
        )
        assert "avoidance" in result.failed_gates
        assert "limits" in result.failed_gates
        assert "credibility" not in result.failed_gates
    ...
    def test_passed_gates_property(self):
        ...
        assert "credibility" in result.passed_gates
        assert "limits" in result.passed_gates
        assert "avoidance" not in result.passed_gates
    ...
    def test_to_dict(self):
        ...
        assert d["gates"]["avoidance"] is False
    ...
    def test_from_dict(self):
        ...
        d = {
            "is_safe": False,
            "level": "warning",
            "gates": {"credibility": True, "avoidance": True, "limits": False, "worth": True},
        ...
        assert result.gates["limits"] is False
    ...
    def test_unsafe_factory(self):
        ...
        assert result.gates["limits"] is False
        assert result.gates["credibility"] is True  # Not in failed_gates
    ...
    def test_merge_with_both_unsafe(self):
        ...
        assert merged.gates["limits"] is False
        assert merged.gates["avoidance"] is False
...
class TestSafetyValidator:
    ...
    def test_init_stats_structure(self):
        ...
        assert "credibility" in stats["gate_failures"]
        assert "avoidance" in stats["gate_failures"]
        assert "limits" in stats["gate_failures"]
        assert "worth" in stats["gate_failures"]
