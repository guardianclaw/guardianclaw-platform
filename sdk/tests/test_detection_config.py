"""
Tests for guardianclaw.detection.config module.
...
class TestInputValidatorConfigEnvironment:
    ...
    def test_from_env_defaults(self):
        ...
        for key in ["GCLAW_API_KEY", "GCLAW_PROVIDER", "GCLAW_MODE"]:
    ...
    def test_from_env_with_api_key(self):
        """from_env() reads GCLAW_API_KEY."""
        os.environ["GCLAW_API_KEY"] = "test-key"
        try:
            config = InputValidatorConfig.from_env()
            assert config.api_key == "test-key"
        finally:
            del os.environ["GCLAW_API_KEY"]

    def test_from_env_with_provider(self):
        """from_env() reads GCLAW_PROVIDER."""
        os.environ["GCLAW_PROVIDER"] = "anthropic"
        try:
            config = InputValidatorConfig.from_env()
            assert config.provider == LLMProvider.ANTHROPIC
        finally:
            del os.environ["GCLAW_PROVIDER"]

    def test_from_env_with_mode(self):
        """from_env() reads GCLAW_MODE."""
        os.environ["GCLAW_MODE"] = "heuristic"
        try:
            config = InputValidatorConfig.from_env()
            assert config.mode == ValidationMode.HEURISTIC
        finally:
            del os.environ["GCLAW_MODE"]
...
class TestOutputValidatorConfigEnvironment:
    ...
    def test_from_env_defaults(self):
        """from_env() works with no environment variables."""
        for key in ["GCLAW_API_KEY", "GCLAW_CONTEXT_TYPE"]:
            os.environ.pop(key, None)
    ...
    def test_from_env_with_context_type(self):
        """from_env() reads GCLAW_CONTEXT_TYPE."""
        os.environ["GCLAW_CONTEXT_TYPE"] = "healthcare"
        try:
            config = OutputValidatorConfig.from_env()
            assert config.context_type == "healthcare"
        finally:
            del os.environ["GCLAW_CONTEXT_TYPE"]
