"""Tests for guardianclaw.integrations.anthropic_sdk module."""
...
class TestInjectSeed:
    ...
    def test_inject_seed_with_custom_claw(self):
        """Should work with custom GuardianClaw instance."""
        from guardianclaw.integrations.anthropic_sdk import inject_seed
        from guardianclaw import GuardianClaw

        claw = GuardianClaw(seed_level="minimal")
        result = inject_seed("Custom test", claw=claw)
...
class TestBlockedStreamIterator:
    ...
    def test_yields_single_blocked_response(self):
        ...
        from guardianclaw.integrations.anthropic_sdk import BlockedStreamIterator

        iterator = BlockedStreamIterator("Test blocked message", gate="avoidance")
        results = list(iterator)

        assert len(results) == 1
        assert results[0]["claw_blocked"] is True
        assert results[0]["claw_gate"] == "avoidance"
    ...
    def test_context_manager_support(self):
        ...
        with BlockedStreamIterator("Test", gate="credibility") as stream:
            for event in stream:
                assert event["claw_blocked"] is True
...
class TestAsyncBlockedStreamIterator:
    ...
    def test_yields_single_blocked_response(self):
        """Should yield exactly one blocked response."""
        from guardianclaw.integrations.anthropic_sdk import AsyncBlockedStreamIterator

        async def run_test():
            iterator = AsyncBlockedStreamIterator("Test blocked", gate="limits")
            results = []
        ...
        assert len(results) == 1
        assert results[0]["claw_blocked"] is True
        assert results[0]["claw_gate"] == "limits"
    ...
    def test_async_context_manager_support(self):
        ...
        async def run_test():
            async with AsyncBlockedStreamIterator("Test", gate="worth") as stream:
...
class TestLogger:
    ...
    def test_set_and_get_logger(self):
        ...
        from guardianclaw.integrations.anthropic_sdk import set_logger, get_logger
    ...
    def test_default_logger_exists(self):
        ...
        from guardianclaw.integrations.anthropic_sdk import get_logger
...
class TestConstants:
    ...
    def test_default_validation_model_exists(self):
        ...
        from guardianclaw.integrations.anthropic_sdk import DEFAULT_VALIDATION_MODEL
    ...
    def test_anthropic_available_flag(self):
        ...
        from guardianclaw.integrations.anthropic_sdk import ANTHROPIC_AVAILABLE
    ...
    def test_semantic_validator_available_flag(self):
        ...
        from guardianclaw.integrations.anthropic_sdk import SEMANTIC_VALIDATOR_AVAILABLE
...
class TestIsAsyncClient:
    ...
    def test_detects_sync_client(self):
        ...
        from guardianclaw.integrations.anthropic_sdk import _is_async_client
...
class TestCreateBlockedResponse:
    ...
    def test_creates_correct_structure(self):
        ...
        from guardianclaw.integrations.anthropic_sdk import _create_blocked_response

        response = _create_blocked_response("Test message", gate="avoidance")

        assert response["id"] == "blocked"
        assert response["type"] == "message"
        assert response["role"] == "assistant"
        assert response["model"] == "claw-blocked"
        assert response["stop_reason"] == "claw_blocked"
        assert response["claw_blocked"] is True
        assert response["claw_gate"] == "avoidance"
    ...
    def test_handles_none_gate(self):
        ...
        assert response["claw_gate"] is None
...
class TestHeuristicValidation:
    ...
    def test_heuristic_validator_blocks_harmful_content(self):
        ...
        assert result["gates"]["avoidance"] == "fail"
    ...
    def test_heuristic_validator_blocks_deception(self):
        ...
        assert result["safe"] is False
        assert result["gates"]["credibility"] == "fail"


class TestGuardianClawMessagesWrapper:
    """Tests for _GuardianClawMessages wrapper (with mocks)."""
    ...
    def setup_method(self):
        ...
        self.claw = GuardianClaw(seed_level="minimal")
    ...
    def test_create_blocks_harmful_input(self):
        ...
        from guardianclaw.integrations.anthropic_sdk import _GuardianClawMessages

        wrapper = _GuardianClawMessages(
        ...
        assert response["claw_blocked"] is True
        assert response.get("claw_gate") == "avoidance"
    ...
    def test_stream_blocks_harmful_input(self):
        ...
        from guardianclaw.integrations.anthropic_sdk import (
            _GuardianClawMessages,
...
class TestAsyncGuardianClawMessagesWrapper:
    """Tests for _GuardianClawAsyncMessages wrapper."""

    def setup_method(self):
        """Setup mocks for each test."""
        from guardianclaw import GuardianClaw
        from guardianclaw.validation import AsyncLayeredValidator, ValidationConfig
    ...
    def test_create_blocks_harmful_input(self):
        ...
        from guardianclaw.integrations.anthropic_sdk import _GuardianClawAsyncMessages

        wrapper = _GuardianClawAsyncMessages(
            messages_api=self.mock_messages_api,
            claw=self.claw,
            enable_seed_injection=True,
            validate_input=True,
...
class TestWrapAnthropicClient:
    ...
    def test_wrap_returns_wrapper_instance(self):
        """Should return GuardianClawAnthropicWrapper."""
        from guardianclaw.integrations.anthropic_sdk import (
        ...
        assert isinstance(wrapped, GuardianClawAnthropicWrapper)
...
class TestExports:
    ...
    def test_all_exports_available(self):
        ...
        from guardianclaw.integrations import anthropic_sdk
