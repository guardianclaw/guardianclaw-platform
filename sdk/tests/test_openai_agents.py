...
These tests verify the functionality of the GuardianClaw integration
...
from guardianclaw.integrations.openai_agents.config import (
    GuardianClawGuardrailConfig,
    VALID_SEED_LEVELS,
    CLAW_GUARDRAIL_INSTRUCTIONS,
)
from guardianclaw.integrations.openai_agents.utils import (
    DefaultLogger,
    get_logger,
    set_logger,
    truncate_text,
...
from guardianclaw.integrations.openai_agents.sanitization import (
    escape_xml_chars,
...
from guardianclaw.integrations.openai_agents.models import (
    ViolationRecord,
    ViolationsLog,
    ValidationMetadata,
    get_violations_log,
...
class TestGuardianClawGuardrailConfig:
    """Tests for GuardianClawGuardrailConfig."""
    ...
    def test_custom_model_warning(self):
        ...
        config = GuardianClawGuardrailConfig(guardrail_model="custom-model-v1")
...
class TestBoundaryGeneration:
    ...
    def test_boundary_format(self):
        ...
        assert boundary.startswith("GCLAW_BOUNDARY_")
        assert len(boundary) > len("GCLAW_BOUNDARY_")
...
class TestViolationsLog:
    ...
    def test_count_by_gate(self):
        ...
        gates = ["avoidance", "avoidance", "limits", "worth"]
        ...
        assert counts["avoidance"] == 2
        assert counts["limits"] == 1
        assert counts["worth"] == 1
...
class TestTripwireDetermination:
    ...
    def test_block_disabled_never_triggers(self):
        ...
        from guardianclaw.integrations.openai_agents.guardrails import _determine_tripwire

        config = GuardianClawGuardrailConfig(block_on_violation=False)
    ...
    def test_harm_only_mode_harm_passes(self):
        """Test that avoidance gate passing allows request in avoidance-only mode."""
        from guardianclaw.integrations.openai_agents.guardrails import _determine_tripwire

        config = GuardianClawGuardrailConfig(
            block_on_violation=True,
            require_all_gates=False,  # Only check avoidance
        )
    ...
    def test_harm_only_mode_harm_fails(self):
        """Test that avoidance gate failing blocks in avoidance-only mode."""
        from guardianclaw.integrations.openai_agents.guardrails import _determine_tripwire
...
class TestSafeCalculator:
    ...
    def test_basic_addition(self):
        """Test basic addition."""
        from guardianclaw.integrations.openai_agents.example import safe_calculate
...
class TestAgentCreation:
    ...
    def test_inject_claw_instructions_with_base(self):
        """Test seed injection with base instructions."""
        from guardianclaw.integrations.openai_agents.agents import inject_claw_instructions

        result = inject_claw_instructions(
    ...
    def test_inject_claw_instructions_without_base(self):
        """Test seed injection without base instructions."""
        from guardianclaw.integrations.openai_agents.agents import inject_claw_instructions

        result = inject_claw_instructions(seed_level="minimal")
    ...
    def test_inject_invalid_seed_level(self):
        ...
        with pytest.raises(ValueError):
            inject_claw_instructions(seed_level="invalid")
...
class TestLoggerConfiguration:
    ...
    def test_get_default_logger(self):
        ...
        # Reset logger
        import guardianclaw.integrations.openai_agents.utils as utils_module
...
class TestSDKIntegration:
    ...
    def test_agents_sdk_available_false_when_not_installed(self):
        ...
        from guardianclaw.integrations.openai_agents import AGENTS_SDK_AVAILABLE
...
class TestReasoningHelpers:
    ...
    def test_get_reasoning_safe_none_validation(self):
        ...
        from guardianclaw.integrations.openai_agents.models import get_reasoning_safe
    ...
    def test_truncate_reasoning_empty(self):
        ...
        from guardianclaw.integrations.openai_agents.models import truncate_reasoning
...
class TestConfigValidation:
    ...
    def test_invalid_validation_timeout(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            GuardianClawGuardrailConfig(validation_timeout=0)
        assert "validation_timeout must be positive" in str(exc_info.value)

        with pytest.raises(ValueError):
            GuardianClawGuardrailConfig(validation_timeout=-5)

    def test_valid_timeout(self):
        """Test that valid timeout is accepted."""
        config = GuardianClawGuardrailConfig(validation_timeout=15.0)
...
class TestExceptions:
    ...
    def test_validation_timeout_error(self):
        ...
        from guardianclaw.integrations.openai_agents import ValidationTimeoutError
    ...
    def test_validation_parse_error(self):
        ...
        from guardianclaw.integrations.openai_agents import ValidationParseError
    ...
    def test_pydantic_not_available_error(self):
        ...
        from guardianclaw.integrations.openai_agents import PydanticNotAvailableError
...
class TestValidateResult:
    ...
    def test_validate_result_none(self):
        ...
        from guardianclaw.integrations.openai_agents.guardrails import _validate_result, ValidationParseError
    ...
    def test_validate_result_valid(self):
        ...
        from guardianclaw.integrations.openai_agents.guardrails import _validate_result
