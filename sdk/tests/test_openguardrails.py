...
- GuardianClawOpenGuardrailsScanner configuration
- GuardianClawGuardrailsWrapper combined validation
...
class TestOpenGuardrailsValidatorErrorHandling:
    ...
    @pytest.mark.skipif(not REQUESTS_AVAILABLE, reason="requests not installed")
    @patch("guardianclaw.integrations.openguardrails.requests")
...
class TestGuardianClawOpenGuardrailsScannerInit:
    """Test GuardianClawOpenGuardrailsScanner initialization."""
    ...
    def test_scanner_definition_present(self):
        ...
        scanner = GuardianClawOpenGuardrailsScanner()
        assert "CLAW Protocol" in scanner.SCANNER_DEFINITION
        assert "CREDIBILITY" in scanner.SCANNER_DEFINITION
        assert "AVOIDANCE" in scanner.SCANNER_DEFINITION
        assert "LIMITS" in scanner.SCANNER_DEFINITION
        assert "WORTH" in scanner.SCANNER_DEFINITION


class TestGuardianClawOpenGuardrailsScannerRegister:
    """Test GuardianClawOpenGuardrailsScanner register/unregister."""
...
class TestGuardianClawGuardrailsWrapperInit:
    """Test GuardianClawGuardrailsWrapper initialization."""
...
class TestGuardianClawGuardrailsWrapperValidation:
    """Test GuardianClawGuardrailsWrapper validation."""
...
class TestGuardianClawGuardrailsWrapperResults:
    """Test GuardianClawGuardrailsWrapper result handling."""

    def test_validate_with_claw_only(self):
        """Should work with only GuardianClaw (LayeredValidator)."""
        from guardianclaw.validation import ValidationResult, ValidationLayer, RiskLevel as ValRiskLevel
        ...
        assert result["claw_result"]["is_safe"] is True
        assert result["openguardrails_result"] is None

    def test_validate_claw_blocks(self):
    ...
    def test_validate_claw_blocks_with_violations(self):
        ...
        assert result["safe"] is False
        assert result["claw_result"]["violations"] == ["Harmful content"]

    def test_validate_claw_exception_handled(self):
        ...
        assert result["claw_result"] is None
    ...
    def test_validate_with_openguardrails(self):
        ...
        wrapper = GuardianClawGuardrailsWrapper(openguardrails=mock_og)
        result = wrapper.validate("test content")
    ...
    def test_validate_require_both_permissive_mode(self):
        ...
        # Here only claw fails, so should be allowed
        assert result["safe"] is True
        assert "claw" in result["blocked_by"]  # claw failed but didn't block overall
...
class TestConvenienceFunctions:
    ...
    def test_register_claw_scanner_invalid_risk_level(self):
        """Should raise ValueError for invalid risk_level."""
        with pytest.raises(ValueError, match="Invalid risk_level"):
            register_claw_scanner(risk_level="invalid_level")

    def test_register_claw_scanner_valid_risk_levels(self):
        ...
        for level in valid_levels:
            ...
            try:
                register_claw_scanner(risk_level=level)
            except RuntimeError:
    ...
    def test_create_combined_validator(self):
        ...
        assert isinstance(wrapper, GuardianClawGuardrailsWrapper)
