...
from guardianclaw.detection.detectors.semantic import (
    SemanticDetector,
...
from guardianclaw.detection.types import AttackType, CheckFailureType, DetectionResult
...
@dataclass
class MockCLAWResult:
    """Mock CLAWResult for testing."""
    is_safe: bool = True
...
class TestSemanticDetector:
    ...
    @patch("guardianclaw.detection.detectors.semantic.SemanticDetector._get_validator")
    def test_safe_text_not_detected(self, mock_get_validator, semantic_detector_config):
        ...
        mock_validator = Mock()
        mock_validator.validate.return_value = MockCLAWResult(is_safe=True)
    ...
    @patch("guardianclaw.detection.detectors.semantic.SemanticDetector._get_validator")
    def test_attack_detected_avoidance_gate(self, mock_get_validator, semantic_detector_config):
        """Test detection when avoidance gate fails."""
        mock_validator = Mock()
        mock_validator.validate.return_value = MockCLAWResult(
            is_safe=False,
        ...
        assert "avoidance" in result.metadata.get("violated_gate", "").lower()

    @patch("guardianclaw.detection.detectors.semantic.SemanticDetector._get_validator")
    def test_attack_detected_limits_gate(self, mock_get_validator, semantic_detector_config):
        """Test detection when limits gate fails."""
        mock_validator = Mock()
        mock_validator.validate.return_value = MockCLAWResult(
            is_safe=False,
...
class TestSemanticChecker:
    ...
    @patch("guardianclaw.detection.checkers.semantic.SemanticChecker._get_validator")
    ...
    @patch("guardianclaw.detection.checkers.semantic.SemanticChecker._get_validator")
    def test_failure_detected_credibility_gate(self, mock_get_validator, semantic_checker_config):
        """Test detection when credibility gate fails."""
        mock_validator = Mock()
        mock_validator.validate.return_value = MockCLAWResult(
            is_safe=False,
    ...
    @patch("guardianclaw.detection.checkers.semantic.SemanticChecker._get_validator")
    def test_failure_detected_avoidance_gate(self, mock_get_validator, semantic_checker_config):
        """Test detection when avoidance gate fails."""
        mock_validator = Mock()
        mock_validator.validate.return_value = MockCLAWResult(
            is_safe=False,
...
class TestGateMappings:
    """Tests for CLAW gate to type mappings."""

    def test_detector_gate_mappings(self):
        """Test detector gate to attack type mappings."""
        assert GATE_TO_ATTACK_TYPE["credibility"] == AttackType.MANIPULATION
        assert GATE_TO_ATTACK_TYPE["avoidance"] == AttackType.HARMFUL_REQUEST
        assert GATE_TO_ATTACK_TYPE["limits"] == AttackType.JAILBREAK
        assert GATE_TO_ATTACK_TYPE["worth"] == AttackType.MANIPULATION
    ...
    def test_checker_gate_mappings(self):
        ...
        assert GATE_TO_FAILURE_TYPE["credibility"] == CheckFailureType.DECEPTIVE_CONTENT
        assert GATE_TO_FAILURE_TYPE["avoidance"] == CheckFailureType.HARMFUL_CONTENT
        assert GATE_TO_FAILURE_TYPE["limits"] == CheckFailureType.SCOPE_VIOLATION
        assert GATE_TO_FAILURE_TYPE["worth"] == CheckFailureType.PURPOSE_VIOLATION
...
class TestValidatorIntegration:
    ...
    def test_input_validator_config_has_semantic_options(self):
        ...
        from guardianclaw.detection.config import InputValidatorConfig
    ...
    def test_output_validator_config_has_semantic_options(self):
        ...
        from guardianclaw.detection.config import OutputValidatorConfig
