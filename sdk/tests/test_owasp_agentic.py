"""Tests for guardianclaw.compliance.owasp_agentic module (OWASP Top 10 for Agentic Applications)."""
...
from guardianclaw.compliance.owasp_agentic import Severity, CoverageLevel
...
class TestVulnerabilityCoverage:
    ...
    def test_not_covered_vulnerabilities(self):
        """Test vulnerabilities not covered by GuardianClaw."""
        not_covered_vulns = [
    ...
    def test_asi01_uses_worth_gate(self):
        """Test ASI01 Agent Goal Hijack uses Worth gate."""
        mapping = VULNERABILITY_COVERAGE[AgenticVulnerability.ASI01_AGENT_GOAL_HIJACK]
        assert "worth" in mapping["gates"]

    def test_asi02_uses_limits_gate(self):
        """Test ASI02 Tool Misuse uses Limits gate."""
        mapping = VULNERABILITY_COVERAGE[AgenticVulnerability.ASI02_TOOL_MISUSE_EXPLOITATION]
        assert "limits" in mapping["gates"]

    def test_asi10_uses_all_gates(self):
        """Test ASI10 Rogue Agents uses all 4 CLAW gates."""
        mapping = VULNERABILITY_COVERAGE[AgenticVulnerability.ASI10_ROGUE_AGENTS]
        gates = mapping["gates"]
        assert "credibility" in gates
        assert "avoidance" in gates
        assert "limits" in gates
        assert "worth" in gates
...
class TestAgenticFinding:
    ...
    def test_create_protected_finding(self):
        ...
        finding = AgenticFinding(
            ...
            component="CLAW Worth Gate",
            gates_relevant=["worth", "limits"],
        )
    ...
    def test_to_dict(self):
        ...
        assert d["component"] == "CLAW Worth Gate"
        assert d["gates_relevant"] == ["worth", "limits"]
...
class TestCLAWGateMapping:
    """Tests for CLAW gate mapping accuracy."""

    def test_worth_gate_vulnerabilities(self):
        """Test vulnerabilities using Worth gate."""
        purpose_vulns = [
            AgenticVulnerability.ASI01_AGENT_GOAL_HIJACK,
            AgenticVulnerability.ASI10_ROGUE_AGENTS,
        ]
        for vuln in purpose_vulns:
            mapping = VULNERABILITY_COVERAGE[vuln]
            assert "worth" in mapping["gates"], f"{vuln.value} should use Worth gate"

    def test_limits_gate_vulnerabilities(self):
        """Test vulnerabilities using Limits gate."""
        scope_vulns = [
            AgenticVulnerability.ASI01_AGENT_GOAL_HIJACK,
            AgenticVulnerability.ASI02_TOOL_MISUSE_EXPLOITATION,
            AgenticVulnerability.ASI03_IDENTITY_PRIVILEGE_ABUSE,
        ...
        for vuln in scope_vulns:
            mapping = VULNERABILITY_COVERAGE[vuln]
            assert "limits" in mapping["gates"], f"{vuln.value} should use Limits gate"

    def test_credibility_gate_vulnerabilities(self):
        """Test vulnerabilities using Credibility gate."""
        truth_vulns = [
            AgenticVulnerability.ASI06_MEMORY_CONTEXT_POISONING,
            AgenticVulnerability.ASI08_CASCADING_FAILURES,
            AgenticVulnerability.ASI09_HUMAN_AGENT_TRUST,
        ...
        for vuln in truth_vulns:
            mapping = VULNERABILITY_COVERAGE[vuln]
            assert "credibility" in mapping["gates"], f"{vuln.value} should use Credibility gate"

    def test_avoidance_gate_vulnerabilities(self):
        """Test vulnerabilities using Avoidance gate."""
        harm_vulns = [
            AgenticVulnerability.ASI09_HUMAN_AGENT_TRUST,
            AgenticVulnerability.ASI10_ROGUE_AGENTS,
        ]
        for vuln in harm_vulns:
            mapping = VULNERABILITY_COVERAGE[vuln]
            assert "avoidance" in mapping["gates"], f"{vuln.value} should use Avoidance gate"


class TestGuardianClawComponentMapping:
    """Tests for GuardianClaw component mapping."""

    def test_claw_worth_gate_component(self):
        """Test CLAW Worth Gate component for ASI01."""
        mapping = VULNERABILITY_COVERAGE[AgenticVulnerability.ASI01_AGENT_GOAL_HIJACK]
        assert "Worth Gate" in mapping["component"]

    def test_claw_limits_gate_component(self):
        """Test CLAW Limits Gate component for ASI02."""
        mapping = VULNERABILITY_COVERAGE[AgenticVulnerability.ASI02_TOOL_MISUSE_EXPLOITATION]
        assert "Limits Gate" in mapping["component"]
...
class TestIntegration:
    ...
    def test_components_used_aggregation(self):
        """Test GuardianClaw components are correctly aggregated."""
        checker = OWASPAgenticChecker()
        result = checker.get_coverage_assessment(include_metadata=True)

        components = result.metadata.get("claw_components_used", [])
        assert len(components) > 0
        assert any("CLAW" in c for c in components)
