"""
Tests for guardianclaw.core.exceptions module.
"""
...
class TestGuardianClawError:
    """Tests for base GuardianClawError exception."""

    def test_simple_message(self):
        """Test creating exception with just a message."""
        error = GuardianClawError("Something went wrong")
    ...
    def test_is_base_exception(self):
        """Test that GuardianClawError inherits from Exception."""
        error = GuardianClawError("test")
        assert isinstance(error, Exception)

    def test_can_be_raised_and_caught(self):
        """Test that exception can be raised and caught."""
        with pytest.raises(GuardianClawError) as exc_info:
            raise GuardianClawError("Test error")
...
class TestValidationError:
    ...
    def test_validation_error_with_violations(self):
        ...
        error = ValidationError(
            "Content blocked",
            violations=["Avoidance detected", "Jailbreak attempt"]
        )
        assert "Content blocked" in str(error)
        assert "Avoidance detected" in str(error)
        assert "Jailbreak attempt" in str(error)
        assert error.violations == ["Avoidance detected", "Jailbreak attempt"]
    ...
    def test_inherits_from_claw_error(self):
        """Test that ValidationError inherits from GuardianClawError."""
        error = ValidationError("test")
        assert isinstance(error, GuardianClawError)
...
class TestConfigurationError:
    ...
    def test_inherits_from_claw_error(self):
        """Test that ConfigurationError inherits from GuardianClawError."""
...
class TestIntegrationError:
    ...
    def test_integration_error_with_integration_name(self):
        ...
        assert "integration=openai_agents" in str(error)
        assert error.integration == "openai_agents"
    ...
    def test_integration_error_with_operation(self):
        ...
        assert error.integration == "coinbase"
        assert error.operation == "before_task_execution"
        assert "integration=coinbase" in str(error)
    ...
    def test_inherits_from_claw_error(self):
        """Test that IntegrationError inherits from GuardianClawError."""
...
class TestExceptionHierarchy:
    ...
    def test_catch_all_claw_errors(self):
        """Test that all specific errors can be caught as GuardianClawError."""
        errors = [
        ...
        for error in errors:
            with pytest.raises(GuardianClawError):
                raise error
