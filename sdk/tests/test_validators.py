"""Tests for guardianclaw.validators module."""
...
class TestCLAWValidator:
    """Tests for full CLAW validator with jailbreak pre-filter."""
...
class TestCLAWValidatorBugFixes:
    """Tests for bug fixes in CLAWValidator (Session 183)."""
    ...
    def test_validate_none_input_has_error_gates(self):
        ...
        validator = CLAWValidator()
        result = validator.validate(None)
        assert result["gates"]["credibility"] == "error"
        assert result["gates"]["avoidance"] == "error"
        assert result["gates"]["limits"] == "error"
        assert result["gates"]["worth"] == "error"
