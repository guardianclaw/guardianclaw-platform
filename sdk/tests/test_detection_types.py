"""
Tests for guardianclaw.detection.types module.
...
Test Categories:
    ...
    3. CLAW Gate Mapping: Correct mapping to Credibility/Avoidance/Limits/Worth
...
from guardianclaw.detection.types import (
    DetectionMode,
    AttackType,
    CheckFailureType,
    DetectionResult,
...
class TestCheckFailureType:
    ...
    def test_values_exist(self):
        ...
        expected = {
            ...
            "SCOPE_VIOLATION": "limits_violation",
...
class TestCheckFailureTypeGate:
    """Tests for CheckFailureType.gate property - CLAW mapping."""

    def test_credibility_gate_mapping(self):
        """DECEPTIVE_CONTENT maps to credibility gate."""
        assert CheckFailureType.DECEPTIVE_CONTENT.gate == "credibility"

    def test_avoidance_gate_mapping(self):
        """HARMFUL_CONTENT maps to avoidance gate."""
        assert CheckFailureType.HARMFUL_CONTENT.gate == "avoidance"

    def test_limits_gate_mapping(self):
        """SCOPE_VIOLATION, BYPASS_INDICATOR, POLICY_VIOLATION map to limits gate."""
        scope_types = [
        ...
        for failure_type in scope_types:
            assert failure_type.gate == "limits", \
                f"{failure_type} should map to limits gate"

    def test_worth_gate_mapping(self):
        """PURPOSE_VIOLATION maps to worth gate."""
        assert CheckFailureType.PURPOSE_VIOLATION.gate == "worth"
    ...
    def test_claw_coverage(self):
        """All four CLAW gates should be represented."""
        gates = {ft.gate for ft in CheckFailureType}
        assert "credibility" in gates, "Credibility gate not represented"
        assert "avoidance" in gates, "Avoidance gate not represented"
        assert "limits" in gates, "Limits gate not represented"
        assert "worth" in gates, "Worth gate not represented"
...
class TestOutputValidationResultGatesFailed:
    ...
    def test_gates_failed_multiple_failures(self):
        ...
        result = OutputValidationResult(
            seed_failed=True,
            failure_types=[
                CheckFailureType.HARMFUL_CONTENT,  # avoidance
                CheckFailureType.DECEPTIVE_CONTENT,  # credibility
                CheckFailureType.SCOPE_VIOLATION,  # limits
            ],
        )
    ...
    def test_gates_failed_with_duplicate_gates(self):
        ...
        gates = result.gates_failed
        # All map to limits, should be deduplicated
        assert len(gates) == 1
        assert gates[0] == "limits"
    ...
    def test_gates_failed_all_claw_gates(self):
        """gates_failed can represent all CLAW gates."""
        result = OutputValidationResult(
            seed_failed=True,
            failure_types=[
                CheckFailureType.DECEPTIVE_CONTENT,  # credibility
        ...
        assert gates == {"credibility", "avoidance", "limits", "worth"}
...
class TestOutputValidationResultSerialization:
    ...
    def test_to_dict_structure(self):
        ...
        assert d["gates_failed"] == ["avoidance"]
        assert d["confidence"] == 0.9
        assert d["blocked"] is True
...
class TestValidation360Integration:
    ...
    def test_full_360_flow_simulation(self):
        ...
        assert "avoidance" in output_result.gates_failed
