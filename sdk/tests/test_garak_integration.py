"""Tests for guardianclaw.integrations.garak module.
...
class TestProbeStructure:
    ...
    def test_worth_gate_has_required_attributes(self):
        ...
        assert probe.goal is not None
        assert len(probe.prompts) > 0
        assert "claw:claw:worth" in probe.tags

    def test_claw_combined_has_required_attributes(self):
        """CLAWCombined should have all required Garak probe attributes."""
        probe = CLAWCombined()
        assert probe.active is True
        assert probe.lang == "en"
        assert probe.goal is not None
        assert len(probe.prompts) > 0
        assert "claw:claw:combined" in probe.tags
...
class TestProbePromptCounts:
    ...
    def test_claw_combined_prompt_count(self):
        """CLAWCombined should have 9 prompts as documented."""
        probe = CLAWCombined()
...
class TestCombinedDetectorNoneHandling:
    """Tests for CLAWCombinedDetector None handling."""
    ...
    def test_max_with_mixed_scores(self):
        ...
        detector = CLAWCombinedDetector()
        # This triggers avoidance compliance (0.9) but refusal for others
...
class TestDefaultParams:
    ...
    def test_base_detector_has_case_sensitive_param(self):
        """CLAWBaseDetector.DEFAULT_PARAMS should include case_sensitive."""
        assert "case_sensitive" in CLAWBaseDetector.DEFAULT_PARAMS
        assert CLAWBaseDetector.DEFAULT_PARAMS["case_sensitive"] is False

    def test_combined_detector_has_case_sensitive_param(self):
        """CLAWCombinedDetector.DEFAULT_PARAMS should include case_sensitive."""
        assert "case_sensitive" in CLAWCombinedDetector.DEFAULT_PARAMS
        assert CLAWCombinedDetector.DEFAULT_PARAMS["case_sensitive"] is False

    def test_credibility_violation_inherits_case_sensitive(self):
        """TruthViolation should inherit case_sensitive from base."""
        assert "case_sensitive" in TruthViolation.DEFAULT_PARAMS

    def test_avoidance_violation_inherits_case_sensitive(self):
        """HarmViolation should inherit case_sensitive from base."""
        assert "case_sensitive" in HarmViolation.DEFAULT_PARAMS

    def test_limits_violation_inherits_case_sensitive(self):
        """ScopeViolation should inherit case_sensitive from base."""
        assert "case_sensitive" in ScopeViolation.DEFAULT_PARAMS

    def test_worth_violation_inherits_case_sensitive(self):
...
class TestModuleExports:
    ...
    def test_version_is_set(self):
        ...
        from guardianclaw.integrations.garak import __version__
    ...
    def test_min_garak_version_is_set(self):
        ...
        from guardianclaw.integrations.garak import MIN_GARAK_VERSION
