/**
 * GuardianClaw Guard - Content Script
 *
...
async function runBotCheck() {
  try {
    ...
    console.warn('[GuardianClaw Guard] Bot check failed:', err);
  }
}
...
function attachInputListener(input: HTMLElement) {
  if (input.dataset.clawProtected) return;
  input.dataset.clawProtected = 'true';
  ...
  input.addEventListener('keydown', (e) => {
    ...
    if (input.dataset.clawBypass === 'true') return;
  ...
  const setupSendButtonInterceptor = () => {
    document.querySelectorAll(sendSelector).forEach((btn) => {
      if ((btn as HTMLElement).dataset.clawProtected) return;
      (btn as HTMLElement).dataset.clawProtected = 'true';
...
function scanAndIndicate(text: string, input: HTMLElement) {
  ...
  if (totalIssues > 0 || walletThreats.hasRisk) {
    ...
    indicator.className = 'claw-indicator';

    if (walletCritical || criticalSecrets > 0) {
      indicator.classList.add('claw-critical');
      if (walletCritical) {
      ...
      indicator.classList.add('claw-warning');
      ...
      indicator.classList.add('claw-info');
...
function getOrCreateIndicator(input: HTMLElement): HTMLElement | null {
  ...
  let indicator = input.parentElement.querySelector('.claw-indicator') as HTMLElement | null;
...
function showWarning(text: string, input: HTMLElement) {
  ...
  chrome.runtime.sendMessage({
    ...
    console.warn('[GuardianClaw Guard] Failed to report threat:', err);
  });

  // C001: Create modal using safe DOM methods instead of innerHTML
  const modal = document.createElement('div');
  modal.className = 'claw-modal-overlay';

  const modalContent = document.createElement('div');
  modalContent.className = 'claw-modal';

  // Header
  const header = document.createElement('div');
  header.className = 'claw-modal-header';
  const logo = document.createElement('span');
  logo.className = 'claw-logo';
  logo.textContent = '🛡️';
  const title = document.createElement('span');
  title.textContent = 'GuardianClaw Guard';
  const closeBtn = document.createElement('button');
  closeBtn.className = 'claw-close';
  closeBtn.textContent = '×';
  header.append(logo, title, closeBtn);

  // Body
  const body = document.createElement('div');
  body.className = 'claw-modal-body';
  const h3 = document.createElement('h3');
  h3.textContent = `⚠️ ${t('warningTitle')}`;
  const p = document.createElement('p');
  p.textContent = t('sensitiveDataDetected');
  const ul = document.createElement('ul');
  ul.className = 'claw-matches';
  ...
  modal.addEventListener('click', (e) => {
    ...
    if (target.classList.contains('claw-close') || target.classList.contains('claw-modal-overlay')) {
      modal.remove();
      return;
    }
    ...
    if (action === 'remove') {
      ...
      input.dataset.clawBypass = 'true';
      triggerSend(input);
      setTimeout(() => {
        delete input.dataset.clawBypass;
      }, 100);
    }
  });
}
...
function triggerSend(input: HTMLElement): boolean {
  ...
  try {
    ...
    console.warn('[GuardianClaw Guard] Failed to trigger send:', err);
    return false;
  }
}
...
function setupConversationShield() {
  ...
  const inputObserver = new MutationObserver((mutations) => {
    for (const mutation of mutations) {
      if (mutation.type === 'attributes' && mutation.target instanceof HTMLElement) {
        ...
        if (target.dataset.clawProtected && mutation.attributeName === 'value') {
  ...
  if (document.body) {
    inputObserver.observe(document.body, {
      attributes: true,
      subtree: true,
      attributeFilter: ['value', 'data-claw-protected']
    });
  }
}
...
function logSuspiciousActivity(type: string, details: Record<string, unknown>) {
  console.warn(`[GuardianClaw Guard] Suspicious activity detected: ${type}`, details);
...
function createToast(message: string, type: 'warning' | 'success'): HTMLElement {
  ...
  toast.className = `claw-toast claw-toast-${type}`;
...
waitForBody()
  .then(() => init())
  .catch((err) => {
    console.error('[GuardianClaw Guard] Failed to initialize:', err);
  });
