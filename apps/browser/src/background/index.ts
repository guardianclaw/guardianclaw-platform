/**
 * @fileoverview GuardianClaw Guard - Background Service Worker
 *
 * Handles:
...
import { validateCLAW, ValidationContext } from '../lib/claw';
...
chrome.runtime.onInstalled.addListener(async (details) => {
  if (details.reason === 'install') {
    ...
    console.log('[GuardianClaw Guard] Extension installed');
  }
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  ...
  if (alarm.name === 'claw-approval-expiry-check') {
    const expired = await approvalEngine.processExpiredApprovals();
    if (expired > 0) {
      console.log(`[GuardianClaw Guard] Processed ${expired} expired approvals`);
    }
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  handleMessage(message, sender)
    .then(sendResponse)
    .catch((error) => {
      console.error('[GuardianClaw Guard] Message error:', error);
...
async function resetSettings(): Promise<Settings> {
  ...
  console.log('[GuardianClaw Guard] Settings reset to defaults');
...
async function clearAllData(): Promise<{ success: boolean }> {
  ...
  console.log('[GuardianClaw Guard] All data cleared');
  return { success: true };
}
...
async function reportThreat(
  ...
  if (settings.notifications) {
    chrome.notifications.create({
      ...
      title: 'GuardianClaw Guard Alert',
...
}): Promise<{ result: ReturnType<typeof validateCLAW>; blocked: boolean }> {
  const result = validateCLAW(payload.text, payload.context);
...
console.log('[GuardianClaw Guard] Background service worker started');
