...
describe('Agent Registry', () => {
  ...
  describe('registerAgent', () => {
    ...
    it('should register agent with metadata', async () => {
      ...
      const agent = await registerAgent('Test Agent', 'openai_agents', 'http://localhost:3001', metadata);
    ...
    it('should register different agents for different endpoints', async () => {
      const agent1 = await registerAgent('Agent 1', 'elizaos', 'http://localhost:3000');
      const agent2 = await registerAgent('Agent 2', 'openai_agents', 'http://localhost:3001');
  ...
  describe('getAgentConnections', () => {
    it('should return all agents', async () => {
      ...
      await registerAgent('Agent 2', 'custom', 'http://localhost:3001');
      await registerAgent('Agent 3', 'custom', 'http://localhost:3002');
  ...
  describe('getConnectedAgents', () => {
    it('should return only connected agents', async () => {
      const agent1 = await registerAgent('Agent 1', 'elizaos', 'http://localhost:3000');
      await registerAgent('Agent 2', 'openai_agents', 'http://localhost:3001');
  ...
  describe('getAgentsByType', () => {
    it('should filter agents by type', async () => {
      ...
      await registerAgent('Agent 3', 'openai_agents', 'http://localhost:3002');

      const elizaAgents = await getAgentsByType('elizaos');
      expect(elizaAgents).toHaveLength(2);

      const openaiAgents = await getAgentsByType('openai_agents');
