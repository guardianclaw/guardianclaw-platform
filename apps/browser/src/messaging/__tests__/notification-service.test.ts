...
describe('NotificationService', () => {
  ...
  describe('show', () => {
    it('creates a notification with basic options', async () => {
      ...
      expect(result.id).toMatch(/^claw-/);
      expect(mockCreate).toHaveBeenCalledWith(
        expect.stringMatching(/^claw-/),
  ...
  describe('clearAll', () => {
    it('clears all claw notifications', async () => {
      mockGetAll.mockImplementation((callback) => {
        callback({
          'claw-1': true,
          'claw-2': true,
      ...
      expect(mockClear).toHaveBeenCalledWith('claw-1');
      expect(mockClear).toHaveBeenCalledWith('claw-2');
