...
Database Guard protects databases from AI agent abuse by validating SQL queries before
execution. It addresses **OWASP ASI03 (Identity and Privilege Abuse)** and prevents data
...
- Industry surveys and AI-incident trackers report a non-trivial share of organisations
  experiencing AI-assisted data leaks; specific rates vary by source and report year.
- Financial data, PII, and credentials can be exfiltrated when the agent has broad
  read permissions.
- Destructive operations (e.g. `DROP TABLE`, `TRUNCATE`, unfiltered `DELETE`) can corrupt
  or destroy data.
