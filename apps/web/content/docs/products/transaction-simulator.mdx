...
description: "Validate Solana transactions before execution - detect risks and prevent losses"
category: "products"
order: 5
...
```bash
pip install guardianclaw httpx
```
...
```python
import asyncio
from guardianclaw.safety.simulation import TransactionSimulator
...
```python
from guardianclaw.safety.simulation import PreflightValidator

async def validate():
    # Combines CLAW validation + transaction simulation
...
Combined validator that integrates with GuardianClaw CLAW.
