...
description: "ISO/TS 15066 compliant safety validation for humanoid robots"
category: "products"
order: 3
...
Safety validation module for humanoid robots implementing ISO/TS 15066-aligned contact force
limits, balance monitoring, and CLAW validation.

## Overview

The Humanoid Safety Protocol provides a safety layer for humanoid-robot operations as a Python
SDK module (`guardianclaw.safety.humanoid`). It exposes biomechanical limits, balance
monitoring, and CLAW validation against pre-defined robot constraints; integration with any
specific simulation environment (ROS 2, Isaac Lab, MuJoCo, etc.) is the caller's
responsibility.
...
```python
from guardianclaw.safety.humanoid import BalanceMonitor, IMUReading
...
```python
from guardianclaw.safety.humanoid import HumanoidSafetyValidator, tesla_optimus
...
result = validator.validate_motion(velocity=1.2, force=50.0)
if not result.is_safe:
    print(f"Motion blocked: {result.violations}")
```
