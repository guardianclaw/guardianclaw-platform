---
title: "REST API"
description: "HTTP REST API reference for GuardianClaw"
category: "api"
order: 7
...
Complete reference for the GuardianClaw REST API.
...
The GuardianClaw REST API provides HTTP endpoints for AI safety validation, seed retrieval, and chat functionality.

**Features:**
- CLAW (Credibility, Limits, Avoidance, Worth) validation
- Jailbreak detection
...
```
Production: https://api.guardianclaw.org
...
**Request:**
```bash
curl https://api.guardianclaw.org/
```

**Response:**
```json
{
  "name": "GuardianClaw AI API",
  "version": "0.1.0",
  ...
  "endpoints": {
    ...
    "POST /validate/request": "Pre-validate a user request",
...
**Request:**
```bash
curl https://api.guardianclaw.org/seed/standard
```
...
{
  "level": "standard",
  "content": "You are a helpful AI assistant operating under the GuardianClaw alignment framework...",
  "token_estimate": 1024
}
```
...
Validate text through CLAW gates with jailbreak detection.
...
**Request:**
```bash
curl -X POST https://api.guardianclaw.org/validate \
...
| `gates.credibility` | boolean | Credibility gate passed (factual accuracy, role / roleplay manipulation) |
| `gates.avoidance` | boolean | Avoidance gate passed (potential harm) |
| `gates.limits` | boolean | Limits gate passed (boundaries, instruction override, prompt extraction, filter bypass, system injection) |
| `gates.worth` | boolean | Worth gate passed (legitimate purpose) |
...
```json
{
  "is_safe": false,
  "violations": [
    "Limits (instruction_override): jailbreak attempt detected",
    "Limits: request outside authorized boundaries"
  ],
  "gates": {
    "credibility": true,
    "avoidance": false,
...
**Request:**
```bash
curl -X POST https://api.guardianclaw.org/validate/request \
  -H "Content-Type: application/json" \
...
**Request:**
```bash
curl -X POST https://api.guardianclaw.org/benchmark \
  -H "Content-Type: application/json" \
  -d '{
    "seed_level": "standard",
    "suite": "basic"
...
```json
{
  ...
  "pass_rate": 0.94,
  "by_category": {
    "credibility": {"passed": 18, "failed": 2},
    "avoidance": {"passed": 15, "failed": 0},
    "limits": {"passed": 14, "failed": 1}
  },
  "results": null
}
```
...
BASE_URL = "https://api.guardianclaw.org"
...
```javascript
const BASE_URL = "https://api.guardianclaw.org";
...
- **Swagger UI:** https://api.guardianclaw.org/docs
- **ReDoc:** https://api.guardianclaw.org/redoc

## See Also

- [Python SDK Reference](/docs/api/validation)
- [Seeds Reference](/docs/api/seeds)
- [Validators Reference](/docs/api/validators)
