...
Official GuardianClaw safety plugin for [ElizaOS](https://elizaos.ai) autonomous agents.
Implements the CLAW (Credibility, Limits, Avoidance, Worth) protocol to validate agent actions
and outputs.

Validated against `@elizaos/core@2.0.0-alpha.223` on 2026-04-24. The peer dependency is pinned
to that exact alpha — upstream is still pre-stable and shape changes between alphas are common.
The current plugin version is **`@guardianclaw/elizaos-plugin@2.0.0-rc.1`**.
...
|------|----------|--------|
| **CREDIBILITY** | Is this factually accurate? | Fake documents, impersonation, misinformation, role / roleplay manipulation |
| **LIMITS** | Is this within authorized boundaries? | Jailbreaks, instruction overrides, prompt extraction, filter bypass, system injection |
| **AVOIDANCE** | Could this cause harm? | Violence, weapons, hacking, malware |
| **WORTH** | Does this serve legitimate benefit? | Purposeless destruction, waste |
...
| Gate | Detects |
|------|---------|
| **CREDIBILITY** | Deception attempts, role manipulation, fake identity claims |
| **LIMITS** | Jailbreak attempts, instruction overrides, prompt extraction, system injection |
| **AVOIDANCE** | Violence, hacking, malware, weapons, dangerous substances |
| **WORTH** | Purposeless destruction patterns |
