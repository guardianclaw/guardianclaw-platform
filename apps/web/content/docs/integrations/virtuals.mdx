...
Validated against `game-sdk@0.1.5` with floor `>=0.1.1` on 2026-04-24. Imports from
`game_sdk.game.agent` and `game_sdk.game.custom_types` are stable across the supported range.
...
| Gate | Blocks When |
|------|-------------|
| **Credibility** | Context manipulation, misleading function names, impersonation |
| **Limits** | Transaction amount exceeds limit, blocked function call, privilege overreach |
| **Avoidance** | Blocked functions, private-key exposure, destructive operations |
| **Worth** | Sensitive actions without a documented purpose |
