...
description: "Model Context Protocol server exposing GuardianClaw safety tools"
...
- `mcp>=1.8.0` — [GitHub](https://github.com/modelcontextprotocol/python-sdk).
  The minimum floor is `1.8.0` because `mcp.client.streamable_http` (used by
  `GuardianClawMCPClient` for HTTP transport) landed in that release. Validated against
  `mcp@1.27.0` on 2026-04-24.
...
> **Note:** HTTP transport requires `streamable_http_client` from the MCP SDK, which may not be available in all versions. Use stdio transport for maximum compatibility.
